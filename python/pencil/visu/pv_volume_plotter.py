...
REASONS_FOR_TERMINATION = {
    ...
    pv._vtk.vtkStreamTracer.FIXED_REASONS_FOR_TERMINATION_COUNT: "FIXED_REASONS_FOR_TERMINATION_COUNT",
}

SAMPLING_METHODS = ["surface points", "points", "slice", "slice orthogonal", "default"]
...
def meshSamplingMethods(
    ...
    if method == "surface points":
        src = randomSampleMeshPoints(
            n_points, surface_mesh, set_seed_1=set_seed_1, get_arrays=get_arrays
        )

    elif method == "points":
        src = randomSampleMeshPoints(
            n_points, mesh, set_seed_1=set_seed_1, get_arrays=get_arrays
        )

    elif method == "slice":
        src = mesh.slice(normal=normal, origin=origin)
        src = randomSampleMeshPoints(
            n_points, src, set_seed_1=set_seed_1, get_arrays=get_arrays
        )
        ...
        for i in range(len(src)):
            src[i] = randomSampleMeshPoints(
                n_points, src[i], set_seed_1=set_seed_1, get_arrays=get_arrays
            )
        ...
        raise ValueError(
            f"Unknown sampling method {method}. Should be one of"
    ...
    * At the time of development pyvista.add_volume (volume rendering) method supported
    ...
    def __init__(
        ...
        assert self.datadir.exists(), (
            f"Given data directory does not exist!"
    ...
    def saveToVTK(self, filename):
        ...
        assert (
            "." not in filename
        ), "Filename contains '.'. Note filename should not contain a suffix."
    ...
    def loadSettings(self, settings):
        ...
        if isinstance(settings, str):
            if self.debug:
                print(
                    f"String given as Plot3DSettings, interpreting as path to settings"
                    "json"
                )
            assert settings.endswith(".json"), (
                "If parameter settings is a string " 'it should contain suffix ".json"'
            )
            ...
            assert isinstance(settings, Plot3DSettings), (
        ...
        if self.param_test:
            ...
            print(
                f"NOTE! parameter test run detected! Number of settings"
    ...
    def contour(
        ...
        values=None,
        isosurfaces=3,
        filename=f"contour_{int(time.time())}",
        cmap=None,
        use_vector_magnitude_as_scalars=False,
    ):
        """
        ...
        assert (
            not self.param_test
        ), "Only vectors and streamlines support parameter tests!"
        ...
        if values == None:
            values = np.linspace(
                self.mesh[scalars].min(), self.mesh[scalars].max(), isosurfaces
            )
    ...
    def scalars(self):
        ...
        self.plotter = pv.Plotter(
            off_screen=self.settings.off_screen, window_size=self.settings.window_size
        )
    ...
    def movingIsovalue(
        self,
        values=None,
        filename=f"moving_isovalue_{time.time()}.gif",
        isosurfaces=30,
        show_outline=True,
        ...
        scalar_sbar_args = create_sbar_args(
            self.settings,
            self.settings.scalar_sbar_title,
            self.settings.scalar_sbar_pos_x,
            self.settings.scalar_sbar_pos_y,
        )
    ...
    def __streamlines(self, settings: Plot3DSettings):
        assert (
            settings.method in SAMPLING_METHODS
        ), f"Unkown sampling method {settings.method}."
        ...
        self.plotter = pv.Plotter(
            off_screen=settings.off_screen, window_size=settings.window_size
        )
        ...
        if settings.method == "default":
            ...
            stream, self.src = self.mesh.streamlines(
                "vectors",
                return_source=True,
    ...
    def __vectors(self, settings: Plot3DSettings):
        assert settings.method != "default", (
            f"Sampling method default is only" "available for streamlines!"
        )
        ...
        if settings.vector_scaling == "magnitude":
            ...
            self.mesh["scale"] = np.array(
                [
                    settings.vector_max if a > settings.vector_max else a
                    for a in magnitude
                ]
            )
            scale = "scale"
            ...
            raise ValueError(
                f"[__add_vectors] Unknown scaling method scaling = {settings.vector_scaling}"
            )
        ...
        for source in self.src:
            self.field.append(
                source.glyph(
                    orient="vectors", scale=scale, factor=settings.vector_factor
                )
            )
    ...
    def __3D_data_loader(self, scalar_key, vector_key):
        ...
        if isinstance(vector_key, list):
            assert (
                len(vector_key) == 3
            ), f"Parameter vector_key should contain 3 elements"
        ...
        if self.coordinates == "cartesian":
            ...
            if isinstance(vector_key, list):
                ...
                vectors = (
                    self.var.__getattribute__(vector_key).reshape(3, -1).swapaxes(0, 1)
                )
            ...
            raise ValueError(
                f"Coordinate system {self.coordinates} is not a valid coordinate system!"
            )
        ...
        self.mesh["vector magnitude"] = np.sqrt(
    ...
    def __addWidgets(self, settings):
        print(
            f">>> WARNING! with larger datasets some of these widgets might be"
            ' laggy! Try "clip plane" might be generally faster than other widgets.'
        )

        field_sbar_args = create_sbar_args(
            settings,
        ...
        if settings.widget_type == "clip slice":
            ...
            print(
                f'> NOTE! "plane vectors" widget can be tuned using Plot3DSettings vector parameters!'
            )
            ...
            def callback(normal, origin):
                ...
                if self.settings.n_points != None:
                    sampled = randomSampleMeshPoints(
                        settings.n_points, slice, get_arrays=True
                    )
                else:
                    sampled = slice
                ...
                self.plotter.add_mesh(
                    slice,
                    opacity=settings.mesh_opacity,
                    name="scalars",  # clim=lims,
                    cmap=settings.mesh_cmap,
                    scalar_bar_args=scalar_sbar_args,
                )

                if "vectors" in settings.widget_type:
                    arrows = sampled.glyph(
                        orient="vectors", scale=scale, factor=settings.vector_factor
                    )
                    self.plotter.add_mesh(
                        arrows,
                ...
                if "streamlines" in settings.widget_type:
                    stream = slice.streamlines_from_source(
                        sampled,  # vectors='vectors',
        ...
        if settings.imageformat is not None:
            self.plotter.screenshot(
                self.outputdir
                / f"widget_image_{int(time.time())}.{settings.imageformat}"
            )
    ...
    def __handlePlotter(self, settings):
        ...
        if settings.show_mesh:
            if settings.mesh_type == "volume":
                ...
                ), "At the moment only cartesian supports volume rendering!"

                # Possible mappers for add_volume are: 'fixed_point', 'gpu',
                #   'open_gl', 'smart'
                va = self.plotter.add_volume(
                    self.mesh,  # self._scalars
                ...
                if settings.add_volume_opacity_slider:
                    f = lambda val: va.GetProperty().SetScalarOpacityUnitDistance(
                        self.mesh.length * val
                    )
                    self.plotter.add_slider_widget(
                        f, [0, 1], title="Volume opacity", color="black"
                    )
                ...
                raise ValueError(
                    f"Unknown mesh_type == {settings.mesh_type}!"
        ...
        if self.field != None:
            for tmp in self.field:
                if tmp.n_points == 0:
                    print(
                        f"WARNING! This field (vectors | streamlines) has zero points"
        ...
        if settings.orbit_gif:
            ...
            orbit = self.plotter.generate_orbital_path(
                n_points=settings.orbit_points, shift=self.mesh.length
            )
            ...
            self.plotter.orbit_on_path(
                orbit, step=settings.orbit_step, write_frames=True
            )
            self.plotter.close()
        ...
        if not settings.off_screen:
            ...
            self.plotter.show(
                title=f"Scalar: {self.scalar_key}, vector: {self.vector_key}",
                auto_close=False,
            )

        if settings.imageformat is not None:
            self.plotter.screenshot(
                filename=self.outputdir
                / f"{self.id_counter:04d}_streamline_{int(time.time())}.{settings.imageformat}"
            )
    ...
    def __set_camera(self, settings):
        ...
        if settings.camera_centre != None and settings.focal_point != None:
            ...
            self.plotter.camera_position = (pos, settings.focal_point, settings.view_up)
