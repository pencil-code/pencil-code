...
class Tensors(object):
    ...
    def calc(
        ...
        try:
            ...
            if lskip_zeros:
                ...
                izero = np.array(
                    np.where(
                        aver.z.__getattribute__(index)[
                            ...
                            int(aver.z.__getattribute__(index).shape[-1] / 2),
                        ]
                        == 0
                    )
                )[0]
                ...
                if izero.size > 0:
                    ...
                    if rank == 0:
                        print(
                            "Removed {0} zeros from {1} resets".format(
        ...
        if l_correction:
            ...
            aver.z.__getattribute__(index)[itcorr] *= -dim.nprocz / (dim.nprocz - 2.0)
            for j in range(0, 3):
                index = alpformat.format(3, j + 1)
                aver.z.__getattribute__(index)[itcorr] *= -dim.nprocz / (
                    dim.nprocz - 2.0
                )
        ...
        alp = np.zeros(
            [3, 3, len(imask), aver.z.__getattribute__(index).shape[-2], iy.size]
        )
        eta = np.zeros(
            [3, 3, 3, len(imask), aver.z.__getattribute__(index).shape[-2], iy.size]
        )
        if rank == 0:
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                ...
                if iy.size > 1:
                    ...
                    eta[0, j, i, :, :, 0] = aver.z.__getattribute__(index1)[
                        imask, :, iy
                    ]
                    eta[1, j, i, :, :, 0] = (
                        aver.z.__getattribute__(index2)[imask, :, iy] * r[:, 0]
                    )
        ...
        self.alpha[irr, iph, :, :, :, 0] = 0.5 * (
            alp[iph, irr, :, :, :]
            + alp[irr, iph, :, :, :]
            - eta[ith, ith, iph, :, :, :] / r
        )
        self.alpha[ith, irr, :, :, :, 0] = self.alpha[irr, ith, :, :, :, 0]
        self.alpha[ith, ith, :, :, :, 0] = (
            alp[ith, ith, :, :, :] + eta[ith, irr, ith, :, :, :] / r
        )
        self.alpha[ith, iph, :, :, :, 0] = 0.5 * (
            alp[iph, ith, :, :, :]
            + alp[ith, iph, :, :, :]
            + eta[ith, irr, iph, :, :, :] / r
        )
        ...
        self.gamma[irr, :, :, :, 0] = -0.5 * (
            alp[iph, ith, :, :, :]
            - alp[ith, iph, :, :, :]
            - eta[ith, irr, iph, :, :, :] / r
        )
        self.gamma[ith, :, :, :, 0] = -0.5 * (
            alp[irr, iph, :, :, :]
            - alp[iph, irr, :, :, :]
            - eta[ith, ith, iph, :, :, :] / r
        )
        ...
        self.beta[irr, ith, :, :, :, 0] = 0.25 * (
            eta[irr, iph, irr, :, :, :] - eta[ith, iph, ith, :, :, :]
        )
        self.beta[irr, iph, :, :, :, 0] = 0.25 * (
            eta[ith, irr, irr, :, :, :]
            - eta[ith, iph, iph, :, :, :]
            - eta[irr, ith, irr, :, :, :]
        )
        ...
        self.beta[iph, iph, :, :, :, 0] = 0.5 * (
            eta[ith, irr, iph, :, :, :] - eta[irr, ith, iph, :, :, :]
        )
        ...
        self.delta[irr, :, :, :, 0] = 0.25 * (
            eta[irr, ith, ith, :, :, :]
            - eta[ith, irr, ith, :, :, :]
            + eta[irr, iph, iph, :, :, :]
        )
        self.delta[ith, :, :, :, 0] = 0.25 * (
            eta[ith, irr, irr, :, :, :]
            - eta[irr, ith, irr, :, :, :]
            + eta[ith, iph, iph, :, :, :]
        )
        self.delta[iph, :, :, :, 0] = -0.25 * (
            eta[irr, iph, irr, :, :, :] + eta[ith, iph, ith, :, :, :]
        )
        # Sign convention to match with meanfield_e_tensor
        ...
        for i in range(0, 3):
            ...
            self.kappa[ith, irr, i, :, :, :, 0] = -0.5 * (
                eta[ith, irr, i, :, :, :] + eta[irr, ith, i, :, :, :]
            )
