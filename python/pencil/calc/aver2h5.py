...
def zav2h5(
    ...
    timereducers = {
        ...
        # np.std(x,axis=-3)),
        "mean_last": lambda x, args: np.mean(
            np.take(x, np.arange(-int(args[0]), 0, 1), axis=-3), axis=-3, keepdims=True
        ),
    ...
    if os.path.exists(os.path.join(folder, "averages/z.h5")):
        ...
        for ipy in range(dim.nprocy):  # over all y processors of the PC run
            for ipx in range(dim.nprocx):  # over all x processors of the PC run
                ...
                iproc = (
                    dim.nprocx * ipy + ipx
                )  # proc rank of the PC run (0 ... nprocx*nprocy-1)
                yndx = yndx_tmp[ipy] - ipy * int(dim.nygrid / dim.nprocy)

                zav = aver(
                    proc=iproc, plane_list=["z"]
                )  # read averages from proc iproc
        ...
        for field, comp in fvars:
            ...
            if len(comp) == 1:
                ...
                ds[dsname][:, :, :, :, yndx_tmp[ipy], :] = tensor.__getattribute__(
                    field
                )
        ds.close()
