...
def load_dataset(src, key, lindx, lindy, lindz, nghost):
    ...
    if var.shape == src[key][()].shape:
        ...
        if l3:
            var[:,nn1:nn2,mm1:mm2,ll3:ll4] = src["bb"][:,n1:n2,m1:m2,l3:l4]
            if m3:
                var[:,nn1:nn2,mm3:mm4,ll3:ll4] = src["bb"][:,n1:n2,m3:m4,l3:l4]
                var[:,nn1:nn2,mm3:mm4,ll1:ll2] = src["bb"][:,n1:n2,m3:m4,l1:l2]
                if n3:
                    var[:,nn3:nn4,mm3:mm4,ll3:ll4] = src["bb"][:,n3:n4,m3:m4,l3:l4]
                    var[:,nn3:nn4,mm1:mm2,ll3:ll4] = src["bb"][:,n3:n4,m1:m2,l3:l4]
                    var[:,nn3:nn4,mm3:mm4,ll1:ll2] = src["bb"][:,n3:n4,m3:m4,l1:l2]
                    var[:,nn3:nn4,mm1:mm2,ll1:ll2] = src["bb"][:,n3:n4,m1:m2,l1:l2]
        elif m3:
            var[:,nn1:nn2,mm3:mm4,ll1:ll2] = src["bb"][:,n1:n2,m3:m4,l1:l2]
            if n3:
                var[:,nn3:nn4,mm3:mm4,ll1:ll2] = src["bb"][:,n3:n4,m3:m4,l1:l2]
...
# ======================================================================
def gauss_3Dsmooth(arr, sigma=1., typ="all", mode=list()):
...
def smoothed_data(
    ...
    # ======================================================================
    def mean_flow(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode):
        if key == "meanuu":                      
            ...
            var = uu.copy()
            for j in range(3):
                var[j] = gauss_3Dsmooth(uu[j], sigma=sigma, typ=typ, mode=mode)
            return var

    # ======================================================================
    def mean_density(src, dst, key, par, gd, lindx,lindy, lindz, nghost, sigma, typ, mode):
        if key == "meanrho":
            if 'rho' in src.keys():
                ...
                rho = np.exp(var)
            var = gauss_3Dsmooth(rho, sigma=sigma, typ=typ, mode=mode)
            return var

    # ======================================================================
    def mean_mag_pressure(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode):
        if key == "meaneb":
            if "eb" in dst.keys():
    ...
    # ======================================================================
    def mean_bfield(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode):
        if key == "meanbb":
            if "bb" in dst.keys():
            ...
            for j in range(3):
                ...
                var[j] = gauss_3Dsmooth(bb[j], sigma=sigma, typ=typ, mode=mode)
            print('meanbb range {:.2e} to {:.2e}'.format(var.min(),var.max())) 

            return var

    # ======================================================================
    def mean_mag_helicity(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode):
        if key == "meanab":
            if 'ab' in dst.keys():
    ...
    # ======================================================================
    def mean_cur_helicity(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode):
        if key == "meanjb":
            if 'jb' in dst.keys():
    ...
    # ======================================================================
    def mean_kin_helicity(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode):
        if key == "meanou":
            if 'ou' in dst.keys():
    ...
    def calc_smoothed_item(key):
        case = {
            "meanuu":         mean_flow(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode),
            "meanrho":     mean_density(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode),
            "meanbb":       mean_bfield(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode),
            "meaneb": mean_mag_pressure(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode),
            "meanab": mean_mag_helicity(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode),
            "meanjb": mean_cur_helicity(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode),
            "meanou": mean_kin_helicity(src, dst, key, par, gd, lindx, lindy, lindz, nghost, sigma, typ, mode),
        }
