...
class Draglift(object):
    ...
    def compute(self, simulation, d_cylinder=0.1, u_0=1.0, flow_dir="y", t_start=-1):
        ...
        if flow_dir == "y":
            ...
            print(
                "ERROR: Incorrect flow direction specified. Must be x- or y-direction"
            )
            return False
        ...
        rms_drag = np.sqrt(
            np.mean(
                np.square(drag[t_0_ind + maximum_drag[0] : t_0_ind + minimum_drag[-1]])
            )
        )
        rms_lift = np.sqrt(
            np.mean(
                np.square(lift[t_0_ind + maximum_lift[0] : t_0_ind + minimum_lift[-1]])
            )
        )

        freq = (maximum_lift.size - 1) / (
            time[t_0_ind + maximum_lift[-1]] - time[t_0_ind + maximum_lift[0]]
        )
