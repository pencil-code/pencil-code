...
#                    else:
    ...
    def __poincare_index(self, field, sx, sy, diff):
        poincare = 0
        poincare += self.__edge(field, [sx[0], sx[1]], [sy[0], sy[0]],
                                diff[0, :], diff[1, :], 0)
        poincare += self.__edge(field, [sx[1], sx[1]], [sy[0], sy[1]],
                                diff[1, :], diff[2, :], 0)
        poincare += self.__edge(field, [sx[1], sx[0]], [sy[1], sy[1]],
                                diff[2, :], diff[3, :], 0)
        poincare += self.__edge(field, [sx[0], sx[0]], [sy[1], sy[0]],
                                diff[3, :], diff[0, :], 0)
        return poincare
    ...
    def __sub_fixed_series(self, queue, t_idx, field, var, i_proc):
        ...
        for i, point in enumerate(self.fixed_points[t_idx-1][i_proc::self.params.n_proc]):
            ...
            fixed_tentative[1] <= self.params.Oy+self.params.Ly:
                ...
                fixed_sign.append(self.fixed_sign[t_idx-1][i_proc+i*self.params.n_proc])
