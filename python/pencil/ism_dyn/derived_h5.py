...
def derive_data(sim_path, src, dst, magic=['pp','tt'], par=[], comm=None,
    ...
    if isinstance(magic, list):
        ...
        magic = [magic]
    # initialise group
    group = group_h5(dst, 'data', status='a', overwrite=grp_overwrite,
    ...
    def Rm_number(src, dst, key, par, gd, l1, l2, m1, m2, n1, n2, nghost):
        if key == 'Rm':
            ...
            if 'bb' in dst.keys():
                ...
                bb = bfield(src, dst, 'bb', par, gd, l1shift, l2shift, m1shift, m2shift, n1shift, n2shift, nghost)
            if 'jj' in dst.keys():
                jj = dst['jj'][:,n1shift:n2shift,m1shift:m2shift,l1shift:l2shift]
            else:
                jj = current(src, dst, 'jj', par, gd, l1shift, l2shift, m1shift, m2shift, n1shift, n2shift, nghost)
    ...
    def Mach_cs(src, dst, key, par, gd, l1, l2, m1, m2, n1, n2, nghost):
        if key == 'Ms':
            ...
            if 'tt' in dst.keys():
                ...
                tt = temperature(src, dst, 'tt', par, gd, l1, l2, m1, m2, n1, n2, nghost)
            if not par.gamma == 1:
    ...
    def kin_helicity(src, dst, key, par, gd, l1, l2, m1, m2, n1, n2, nghost):
        if key == 'ou':
            ...
            if 'vort' in dst.keys():
                ...
                oo = vorticity(src, dst, 'vort', par, gd, l1, l2, m1, m2, n1, n2, nghost)
            var = dot(uu, oo)
            return var
