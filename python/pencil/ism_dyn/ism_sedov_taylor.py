...
class SedovTaylor(object):
    ...
    def get_st(self,
        ...
        *lcioffi:
        ...
        rdot = 2./(self.dims+2.)*(self.xi*self.E0/self.rho0)**(
               1./(self.dims+2.))*self.t**(2./(self.dims+2.)-1)
        ...
        def get_snpl(self, quiet):
            ...
            vtrans=2.e2*self.n0**(2./17.)*self.E0**(1./17.)
            ttrans=(2./(2.+self.dims)*(self.xi*self.E0/self.rho0)**(
                    1./(self.dims+2.))*vtrans**(-1))**(1./(1-2./(self.dims+2.)))
            rtrans=(self.xi*self.E0/self.rho0)**(1./(self.dims+2.)
            ...
            rsnpl[isnpl]=self.rWolt*(8./(self.dims+2.
                         )*self.t[isnpl]/self.tWolt-3./(self.dims+2.))**(1./4.)
            vsnpl[isnpl]=self.vWolt*(8./(self.dims+2.
                         )*self.t[isnpl]/self.tWolt-3./(self.dims+2.))**(-3./4.)
        ...
        def get_cioffi(self, quiet):
            ...
            tpds=(3.61e-5*self.E0**(3./14.)/self.n0**(4./7.))/2.718281828
            rpds=(self.xi*self.E0/self.rho0)**(1./(self.dims+2.)
                 )*tpds**(2./(self.dims+2.))
            vpds=(2./(2+self.dims))*(self.xi*self.E0/self.rho0)**(
                  2./(2+self.dims)-1)
            if not quiet:
            ...
            tmcs=61*self.vejecta**3/self.n0**(3./7.)/self.E0**(3./14.)*self.tpds
            ...
            rcioffi[icioffi]=self.rpds*(4./3.*self.t[icioffi]/
                             self.tpds-1./3.)**(3./10.)
            vcioffi[icioffi]=0.3*self.rpds*4./3./self.tpds*(4./3.*
                             self.t[icioffi]/self.tpds-1./3.)**(-7./10.)
            jcioffi=np.where(self.t>self.tmcs)[0]
            rcioffi[jcioffi]=self.rpds*(4.66*(self.t[jcioffi]-self.tmcs)/
                             self.tpds*(1.-0.779/(self.tmcs/self.tpds)**0.17)+(
                             self.rmcs/self.rpds)**4)**0.25
            vcioffi[jcioffi]=0.25*self.rpds*4.66/self.tpds*(1.-0.779/(
                                  self.tmcs/self.tpds)**0.17)*(
                             4.66*(self.t[jcioffi]-self.tmcs)/self.tpds*(1.-
                            0.779/(self.tmcs/self.tpds)**0.17)+(
                                   self.rmcs/self.rpds)**4)**(-0.75)
