...
def poisson_vector_cartesian(bx, by, bz, x, y, z, hx, hy, hz, niter=1000):
    ...
    *niter*:
    ...
    while iteration < niter:
        iteration += 1
        Aux = ux.copy()
        Aux = m*(1/dx**2*(np.roll(ux, -1, 2) + np.roll(ux, 1, 2)) +
                 1/dy**2*(np.roll(ux, -1, 1) + np.roll(ux, 1, 1)) +
                 1/dz**2*(np.roll(ux, -1, 0) + np.roll(ux, 1, 0)) - hx)
        ux[1:-1, 1:-1, 1:-1] = Aux[1:-1, 1:-1, 1:-1]

        Auy = uy.copy()
        Auy = m*(1/dx**2*(np.roll(uy, -1, 2) + np.roll(uy, 1, 2)) +
                 1/dy**2*(np.roll(uy, -1, 1) + np.roll(uy, 1, 1)) +
                 1/dz**2*(np.roll(uy, -1, 0) + np.roll(uy, 1, 0)) - hy)
        uy[1:-1, 1:-1, 1:-1] = Auy[1:-1, 1:-1, 1:-1]

        Auz = uz.copy()
        Auz = m*(1/dx**2*(np.roll(uz, -1, 2) + np.roll(uz, 1, 2)) +
                 1/dy**2*(np.roll(uz, -1, 1) + np.roll(uz, 1, 1)) +
                 1/dz**2*(np.roll(uz, -1, 0) + np.roll(uz, 1, 0)) - hz)
...
def poisson_scalar_cylindrical(bc, r, theta, z, h, niter=200):
    ...
    *niter*:
    ...
    while iteration < niter:
        iteration += 1
        Au = u.copy()
        Au = m*((1/dx**2 + 1/(2*dx*radius_matrix)) * np.roll(u, -1, 2) +
                (1/dx**2 - 1/(2*dx*radius_matrix)) * np.roll(u, 1, 2) +
                1/(dy**2*radius_matrix**2)*(np.roll(u, 1, 1) + np.roll(u, -1, 1)) +
                1/(dz**2)*(np.roll(u, 1, 0) + np.roll(u, -1, 0)) - h)
        u[:, :, 1:-1] = Au[:, :, 1:-1]
...
def poisson_vector_cylindrical(br, btheta, bz, r, theta, z, hr, htheta, hz, niter=1000):
    ...
    *niter*:
    ...
    dy = theta[1] - theta[0]
    dz = z[1] - z[0]
    m = 1/(2/dx**2 + 2/(radius_matrix**2*dy**2) + 2/dz**2 + 1/radius_matrix**2)
    ...
    while iteration < niter:
        ...
        R = br.copy()
        Theta = btheta.copy()
        R = m*((1/dx**2 + 1/(2*dx*radius_matrix))*(np.roll(R, -1, 2) + np.roll(R, 1, 2)) +
               1/(radius_matrix**2*dy**2)*(np.roll(R, -1, 1) + np.roll(R, 1, 1)) +
               1/dz**2*(np.roll(R, -1, 0) + np.roll(R, 1, 0)) -
               1/(dy*radius_matrix**2)*(np.roll(Theta, -1, 1) + np.roll(Theta, 1, 1)) - hr)
        Theta = m*((1/dx**2 + 1/(2*dx*radius_matrix))*(np.roll(Theta, -1, 2) + np.roll(Theta, 1, 2)) +
                   1/(radius_matrix**2*dy**2)*(np.roll(Theta, -1, 1) + np.roll(Theta, 1, 1)) +
                   1/dz**2*(np.roll(Theta, -1, 0) + np.roll(Theta, 1, 0)) +
                   1/(dy*radius_matrix**2)*(np.roll(R, -1, 1) + np.roll(R, 1, 1)) - htheta)
...
def poisson_vector_spherical(br, btheta, bphi, r, theta, phi, hr, htheta, hphi, niter=200):
    '''
    Solve the vector form of the Poisson equation, $\nabla^2 u = h$,
    ...
    *niter*:
    ...
    fraction_r = 1/(2*(1/dx**2 + 1/(dy*radius_matrix)**2 +
                       1/(dz*radius_matrix*np.sin(theta_matrix))**2 + 1/radius_matrix**2))
    fraction_theta = 1/(2*(1/dx**2 + 1/(dy*radius_matrix)**2 + 1/(dz*radius_matrix*np.sin(theta_matrix))**2 +
    ...
    def pre(coord):
        ...
        return np.nan_to_num((1/(dx*radius_matrix)*(np.roll(coord, -1, 2) - np.roll(coord, 1, 2)) +
                              1/dx**2*(np.roll(coord, -1, 2) + np.roll(coord, 1, 2)) +
                              1/(dy*radius_matrix)**2*(np.roll(coord, -1, 1) + np.roll(coord, 1, 1)) +
                              1/(dz*radius_matrix*np.sin(theta_matrix))**2*(np.roll(coord, -1, 0) + np.roll(coord, 1, 0)) +
                              1/(2*dy*radius_matrix**2*np.tan(theta_matrix))*(np.roll(coord, -1, 1) + np.roll(coord, 1, 1))))
    ...
    while iteration <= niter:
        ...
        R = fraction_r*np.nan_to_num((pre(R) - 1/(dy*radius_matrix**2)*(np.roll(Theta, -1, 1) - np.roll(Theta, 1, 1)) -
                                      1/(dz*radius_matrix**2*np.sin(theta_matrix))*(np.roll(Phi, -1, 1) - np.roll(Phi, 1, 1)) -
                                      2/(radius_matrix**2*np.nan_to_num(np.tan(theta_matrix)))*Theta - hr))
        Theta = fraction_theta*np.nan_to_num((pre(Theta) - 1/(np.nan_to_num(np.tan(theta_matrix))*dz*radius_matrix**2*np.sin(theta_matrix))*(np.roll(Phi, -1, 0) - np.roll(Phi, 1, 0)) +
                                              1/(radius_matrix**2*dy)*(np.roll(R, -1, 1) - np.roll(R, 1, 1)) - htheta))
        Phi = fraction_phi*np.nan_to_num((pre(Phi) - 1/(dz*radius_matrix**2*np.sin(theta_matrix))*(np.roll(R, -1, 0) - np.roll(R, 1, 0)) +
                                          1/(np.nan_to_num(np.tan(theta_matrix))*radius_matrix**2*dz*np.sin(theta_matrix))*(np.roll(Theta, -1, 1) - np.roll(Theta, 1, 1)) - hphi))
