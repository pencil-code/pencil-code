...
def laplace_vector_spherical(br, btheta, bphi, r, theta, phi, niter=200):
    ...
    *niter*: int
    ...
    bz[1:, :, :] = 0
    solution = pc.math.laplace_vector_spherical(bx, by, bz, x, y, z, niter=100)
    '''
    ...
    fraction_r = 1/(2*(1/dr**2 + 1/(dtheta*radius_matrix)**2 +
                       1/(dphi*radius_matrix*np.sin(theta_matrix))**2 + 1/radius_matrix**2))
    fraction_theta = 1/(2*(1/dr**2 + 1/(dtheta*radius_matrix)**2 + 1/(dphi*radius_matrix*np.sin(theta_matrix))**2 +
                           1/(radius_matrix*np.sin(theta_matrix))**2))
    fraction_phi = fraction_theta

    def pre(coord):
        '''
        This function simply returns the first five terms in each of the differentialequations
        The parameter coord is simply the most recent iteration of the unknown.
        '''

        return np.nan_to_num((1/(dr*radius_matrix)*(np.roll(coord, -1, 0) - np.roll(coord, 1, 0)) +
                              1/dr**2 * (np.roll(coord, -1, 0) + np.roll(coord, 1, 0)) +
                              1/(dtheta*radius_matrix)**2 * (np.roll(coord, -1, 1) + np.roll(coord, 1, 1)) +
                              1/(dphi*radius_matrix*np.sin(theta_matrix))**2 * (np.roll(coord, -1, 2) + np.roll(coord, 1, 2)) +
                              1/(2*dtheta*radius_matrix**2*np.tan(theta_matrix))*(np.roll(coord, -1, 1) + np.roll(coord, 1, 1))))
    ...
    while iteration <= niter:
        ...
        R = fraction_r*np.nan_to_num((pre(R) - 1/(dtheta*radius_matrix**2)*(np.roll(Theta, -1, 1) - np.roll(Theta, 1, 1)) -
                                      1/(dphi*radius_matrix**2*np.sin(theta_matrix))*(np.roll(Phi, -1, 1) - np.roll(Phi, 1, 1)) -
                                      2/(radius_matrix**2*np.nan_to_num(np.tan(theta_matrix)))*Theta))
        Theta = fraction_theta * np.nan_to_num((pre(Theta) - 1/(np.nan_to_num(np.tan(theta_matrix))*
                                                 dphi*radius_matrix**2*np.sin(theta_matrix))*(np.roll(Phi, -1, 2) - np.roll(Phi, 1, 2)) +
                                                1/(radius_matrix**2 * dr) * (np.roll(R, -1, 1) - np.roll(R, 1, 1))))
        Phi = fraction_phi * np.nan_to_num((pre(Phi) - 1/(dphi*radius_matrix**2*np.sin(theta_matrix))*(np.roll(R, -1, 2)-np.roll(R, 1, 2)) +
                                            1/(np.nan_to_num(np.tan(theta_matrix))*radius_matrix**2*dphi*np.sin(theta_matrix))*(np.roll(Theta, -1, 1) - np.roll(Theta, 1, 1))))
