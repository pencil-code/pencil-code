...
def space_struct( arr, dims=[[],[],[]], Dorder=2, dirs='zyx',
                 lperi=(True,True,True), lplot=True,
                 InitialGuess = [1.0,1.0],
                 deltay=0., chunksize=1000.0,
                 figname = None, dlabel = 'data',
                 quiet=True, downsample=[1,1,1], maxl=[-1,-1,-1],
                 loopsample=[1,1,1] ):
    """
    ...
    *quiet*
    ...
    if 'z' in dirs:
        ...
        if not ldirmatch:
            print('Please correct size dims(z) '+
    ...
    if 'y' in dirs:
        ...
        if not ldirmatch:
            print('Please correct size dims(y) '+
    ...
    if 'x' in dirs:
        ...
        if not ldirmatch:
            print('Please correct size dims(x) '+
    ...
    if len(Dshape) > len(arrshape):
        print('N dirs {} cannot exceed N dims {} of arr'.format(
    ...
    if D1:
        ...
        ell[0] = 0.
    if D2:
        lmax = np.min([lmax,np.sqrt((
                     (Dshape[0]+1)/2/Dshape[0]*abs(dims[0][-1]-dims[0][0]))**2+
                     ((Dshape[1]+1)/2/Dshape[1]*abs(dims[1][-1]-dims[1][0]))**2)])
        for j in range(2):
            if maxl[j] < 0:
                maxl[j] = Dshape[j]
        D[0,0] = 0.
    ...
    if D3:
        lmax = np.min([lmax,np.sqrt((
                 (Dshape[0]+1)/2/Dshape[0]*abs(dims[0][-1]-dims[0][0]))**2+
                 ((Dshape[1]+1)/2/Dshape[1]*abs(dims[1][-1]-dims[1][0]))**2+
                 ((Dshape[2]+1)/2/Dshape[2]*abs(dims[2][-1]-dims[2][0]))**2)])
        for j in range(3):
            if maxl[j] < 0:
                maxl[j] = Dshape[j]
        D[0,0,0] = 0.
    ...
    for iz in range(1,Dshape[0],zskip):
        ...
        if not len(Dshape) > 1:
            ...
            if D1:
                N[iz] = marr[iz0:iz0+maxl[0]][marr[iz0:iz0+maxl[0]].mask==False].size/arr[::zstep][iz0:iz0+maxl[0]].size
            elif D2:
                N[iz] = marr[iz0:iz0+maxl[0]][marr[iz0:iz0+maxl[0]].mask==False].size/arr[::zstep,::ystep][iz0:iz0+maxl[0]].size
            elif D3:
                N[iz] = marr[iz0:iz0+maxl[0]][marr[iz0:iz0+maxl[0]].mask==False].size/arr[::zstep,::ystep,::xstep][iz0:iz0+maxl[0]].size
        else:
            marr = np.ma.array(arr[::zstep])
            ...
            for iy in range(0,Dshape[1],yskip):
                ...
                if not len(Dshape) > 2:
                    ...
                    if ell[iz,iy] <= lmax:
                        ...
                        yshift = np.roll(zshift[:,::ystep,::ystep], int(iy/ystep), axis=Daxis[1])
                        # account for shear if D2 'yx' 
                        if 'x' in dirs and iy > 0 and not deltay == 0.:
                            ishear = round(2*ny*deltay/
                                         (max(dims[0])-min(dims[0]))/ystep)
                            yshift[:,:int(iy/ystep)] = np.roll(yshift[:,:int(iy/istep)], ishear,
                                                                         axis=0)
                        shift_diff = yshift[:,iy0:iy0+maxl[1]] - marr[iz0:iz0+maxl[0],iy0:iy0+maxl[1]]
                        shift_power = np.power(shift_diff, Dorder)
                        D[iz,iy] = np.ma.mean(shift_power[
                                          np.logical_not(np.isnan(shift_diff))])
                        if D2:
                            N[iz,iy] = marr[iz0:iz0+maxl[0],iy0:iy0+maxl[1]][marr[iz0:iz0+maxl[0],iy0:iy0+maxl[1]].mask==False].size/arr[::zstep,::ystep][iz0:iz0+maxl[0],iy0:iy0+maxl[1]].size
                        elif D3:
                            N[iz,iy] = marr[iz0:iz0+maxl[0],iy0:iy0+maxl[1]][marr[iz0:iz0+maxl[0],iy0:iy0+maxl[1]].mask==False].size/arr[::zstep,::ystep,::xstep][iz0:iz0+maxl[0],iy0:iy0+maxl[1]].size
                else:
                    marr = np.ma.array(arr[::zstep,::ystep])
                    yshift = np.roll(zshift[:,::ystep], int(iy/ystep), axis=Daxis[1])[:,iy0:iy0+maxl[1]]
                    ...
                    for ix in range(0,Dshape[2],xskip):
                        ...
                        if ell[iz,iy,ix] <= lmax:
                            ...
                            if ix > 0 and not deltay == 0.:
                                ...
                                xshift[:,:,:int(ix/xstep)] = np.roll(xshift[:,:,:int(ix/xstep)],
                                                                 ishear, axis=1)
                            shift_diff = xshift[:,:,ix0:ix0+maxl[2]] - marr[iz0:iz0+maxl[0],
                                                                            iy0:iy0+maxl[1],
                                                                            ix0:ix0+maxl[2]]
                            shift_power = np.power(shift_diff, Dorder)
                            D[iz,iy,ix] = np.mean(shift_power[
                                          np.logical_not(np.isnan(shift_diff))])
                            ...
                            N[iz,iy,ix] = shift_power[shift_power.mask==False].size/shift_power.size#marr[iz0:iz0+maxl[0],iy0:iy0+maxl[1],ix0:ix0+maxl[2]][marr[iz0:iz0+maxl[0],iy0:iy0+maxl[1],ix0:ix0+maxl[2]].mask==False].size/arr[::zstep,::ystep,::xstep][iz0:iz0+maxl[0],iy0:iy0+maxl[1],ix0:ix0+maxl[2]].size
    ...
    if D1d.size > 1: 
        ...
        popt, pcov = [0,1], [0,1]
    print('Fit parameters of sigma {:.2e} and L0 {:.2e}'.format(
           popt[0],popt[1]))
   
    print('Calculated spatial structure function of '+
    ...
    if lplot:
        if D1d.size > 1:
            ...
            plt.xlabel(r'$\ell$')
            plt.ylabel(r'$D(\ell)$')
            plt.plot(l1d, fit_gaussm1(l1d, *popt),'.',
...
def space_struct_mpi( arr, dims=[[],[],[]], Dorder=2, dirs='zyx',
                 lperi=(True,True,True), lplot=True,
                 InitialGuess = [1.0,1.0],
                 deltay=0., chunksize=1000.0,
                 figname = None, dlabel = 'data',
    ...
    *quiet*
    ...
    lmax = 0
    if D1:
        lmax = max(lmax, (Dshape[0]+1)/2/Dshape[0]*abs(dims[0][-1]-dims[0][0]))
    if D2:
        lmax = np.max([lmax,
    ...
    for iz in range(nz):
        ...
        if l_mpi:
            comm.send(bzshift[-np.mod(iz,indz.size):],np.mod(rank-int(iz/indz.size),size),tag=rank)
            fzshift[:np.mod(iz,indz.size)] = comm.recv(np.mod(rank+int(iz/indz.size),size),tag=np.mod(rank+int(iz/indz.size)))
            print('rank {} sent to {} data size {}'.format(rank, np.mpd(rank+int(iz/indz.size)),iz))
            print('rank {} received from to {} data size {}'.format(rank, np.mpd(rank-int(iz/indz.size)),iz))
        bzshift = fzshift    
        if not len(Dshape) > 1:
            ell[iz] = np.sqrt((dims[Daxis[0]][iz]-dims[Daxis[0]][0])**2)
            ...
            for iy in range(0,Dshape[1]):
                ...
                if not len(Dshape) > 2:
                    ...
                    if ell[iz,iy] <= lmax:
                        ...
                        if 'x' in dirs and iy > 0 and not deltay == 0.:
                            ishear = round(2*ny*deltay/
                                         (max(dims[0])-min(dims[0])))
                            yshift[:,:iy] = np.roll(yshift[:,:iy], ishear,
                                                                         axis=0)
...
#------------------------------------------------------------------------------
def time_struct(arr, time=[], order=2,
                lplot=True,
                InitialGuess = [1.0,1.0],
                figname = None, dlabel = 'data',
                quiet=False):
    ...
    *quiet*
    ...
    if np.size(time) > 0:
        ...
        if not ltime_corr:
            print('arr shape {} does not match time shape {}'.format(
