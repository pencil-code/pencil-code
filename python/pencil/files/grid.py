...
class Grid(object):
    ...
    def __init__(self, datadir='data/', proc=-1, ivar=-1, quiet=False,
        ...
        if not allprocs:
            ...
            filename = os.path.join(datadir+'/allprocs/grid.dat')
            infile = npfile(filename, endian=format)
            grid_raw = infile.fort_read(precision)
            dx, dy, dz = tuple(infile.fort_read(precision))
            Lx, Ly, Lz = tuple(infile.fort_read(precision))
            ...
            z=grid_raw[dim.mx+dim.my+1:dim.mx+dim.my+dim.mz+1]
            dx_1 = dx_1_raw[1:dim.mx+1]
            dy_1 = dx_1_raw[dim.mx+1:dim.mx+1+dim.my]
            dz_1 = dx_1_raw[dim.mx+dim.my+1:dim.mx+dim.my+dim.mz+1]
            dx_tilde = dx_tilde_raw[1:dim.mx+1]
            dy_tilde = dx_tilde_raw[dim.mx+1:dim.mx+1+dim.my]
            dz_tilde = dx_tilde_raw[dim.mx+dim.my+1:dim.mx+dim.my+dim.mz+1]
