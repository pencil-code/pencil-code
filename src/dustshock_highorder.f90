...
!
       use Cdata, only: ishock,iudx,iudy,iudz,lroot,bcx,bcy,bcz
...
!
      if (.not. lstarting .and. .not. lforce_periodic_shockviscosity) then
        if (bcx(ishock)=='p' .and. .not. all((/bcx(iudx(1):iudz(1))/)=='p')) then
          if (lroot) then
        ...
        if (bcy(ishock)=='p' .and. .not. all((/bcy(iudx(1):iudz(1))/)=='p')) then
          if (lroot) then
        ...
        if (bcz(ishock)=='p' .and. .not. all((/bcz(iudx(1):iudz(1))/)=='p')) then
          if (lroot) then
...
!
      ...
      use Cdata, only: iuud,iudx,iudy,iudz,ishock
...
!  Compute divergence
!
      call boundconds_x(f,iudx(1),iudx(1))
!
      call initiate_isendrcv_bdry(f,iudy(1),iudz(1))
!
      do imn=1,ny*nz
!
        n = nn(imn)
...
!
        if (necessary(imn)) then
          call finalize_isendrcv_bdry(f,iudy(1),iudz(1))
          call boundconds_y(f,iudy(1),iudy(1))
          call boundconds_z(f,iudz(1),iudz(1))
        endif
! The following will calculate div u for any coordinate system. 
        call div(f,iuud(1),penc)
