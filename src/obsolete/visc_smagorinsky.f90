...
!
        ...
        if (.not.ldensity) &
          call fatal_error('initialize_viscosity',"ldensity better be .true. for ivisc='smagorinsky*'")
!
        ...
        if (idiag_nu_LES /= 0) then
          ...
          do mcount=m1,m2
            do ncount=n1,n2
               ...
               call sum_mn_name(nu_smag,idiag_nu_LES)      ! mean nu_smag?
            enddo
          enddo
        endif
      endif
...
!
      ...
      real, dimension (nx) :: murho1,rho,rho1,divu,shock,SS12,sij2,diffus_nu
      integer :: i
!
...
!
      ...
      case default
         call fatal_error('calc_viscous_forcing','no such ivisc: '//trim(ivisc))
!
      endselect
      !
      ! Add ordinary viscosity if nu /= 0
      !
      diffus_nu=f(l1:l2,m,n,ismagorinsky)
      if (nu /= 0.) then
        !
        ...
        diffus_nu=diffus_nu+nu
      endif
!
      df(l1:l2,m,n,iux:iuz)=df(l1:l2,m,n,iux:iuz)+fvisc
!
!  for viscous time step
!
      diffus_nu=diffus_nu*dxyz_2
      if (lfirst.and.ldt) maxdiffus=max(maxdiffus,diffus_nu)
