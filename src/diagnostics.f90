...
!  Communicate over all processors.
!
      call mpireduce_max(fmax_tmp,fmax,nmax_count,MPI_COMM_DIAG)
      call mpireduce_sum(fsum_tmp,fsum,nsum_count,comm=MPI_COMM_DIAG)  !,nonblock=maxreq)        ! wrong for Yin-Yang due to overlap
      if (lweight_comm) call mpireduce_sum(fweight_tmp,fweight,nsum_count,comm=MPI_COMM_DIAG)!   ~
      !call mpiwait(maxreq)
!
...
!
        call mpireduce_sum(fnamez,fsumz,(/nz,nprocz,nnamez/), MPI_COMM_DIAG)
...
!
      if (nnamey>0) then
        call mpireduce_sum(fnamey,fsumy,(/ny,nprocy,nnamey/), MPI_COMM_DIAG)
...
!
      if (nnamex>0) then
        call mpireduce_sum(fnamex,fsumx,(/nx,nprocx,nnamex/), MPI_COMM_DIAG)
...
!
      if (nnamer>0) then
        ...
        if (ipz==0) call mpireduce_sum(phiavg_norm,norm,nrcyl,comm=MPI_COMM_XYPLANE_DIAG)
        if (lroot) then
          do iname=1,nnamer
...
!
      if (nnamexz>0) then
        call mpireduce_sum(fnamexz,fsumxz,(/nx,nz,nnamexz/),idir=IXBEAM_DIAG)
...
!
      if (nnamerz>0) then
        ...
        if (ipz==0) call mpireduce_sum(phiavg_norm,norm,nrcyl,comm=MPI_COMM_XYPLANE_DIAG)  ! avoid double comm!
        if (lroot) then
          do i=1,nnamerz
