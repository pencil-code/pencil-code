...
! initial spectrum k^power
        ...
        case('random-isotropic-KS')
          call random_isotropic_KS(initpower,f,iux,iuz,N_modes_uu)
...
!
      if (lwr) then
        ...
        write(3,*) 'i_ursphmhi=',idiag_ursphmphi
...
!
        case ('uu')
          if (slices%index>=3) then
            ...
            if (lwrite_slice_xy3) &
                 slices%xy3=f(l1:l2,m1:m2,slices%iz3,iux-1+slices%index)
            if (lwrite_slice_xy4) &
                 slices%xy4=f(l1:l2,m1:m2,slices%iz4,iux-1+slices%index)
