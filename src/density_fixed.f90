...
!
      ldensity_fixed = .true.
!
      ilnrho = mvar+naux+1           ! indix to access lam
      naux = naux+1             ! added 1 variable
!
      if ((ip<=8) .and. lroot) then
        print*, 'register_density: naux = ', naux
...
!
      if (naux > maux) then
        ...
        call stop_it('register_density: naux > maux')
      endif
!
!  Writing files for use with IDL
!
      if (naux < maux) aux_var(aux_count)=',lnrho $'
      if (naux == maux) aux_var(aux_count)=',lnrho'
      aux_count=aux_count+1
      if (lroot) write(15,*) 'lnrho = fltarr(mx,my,mz)*one'
!
...
!
!  Density auxiliary variables and diagnostics
!
!   9-jun-04/anders: adapted from density
!
      use Sub
!
      real, dimension (mx,my,mz,mvar+maux) :: f
...
!
      ...
      intent(out) :: glnrho,lnrho
!
!  identify module
!
      if (headtt.or.ldebug) print*,'dlnrho_dt: fixed density'
!
!  define lnrho; calculate density gradient
!
