! $Id$
!
! This module is a copy of the 'deriv_2nd' module, but implements all
! derivatives, including up to 6th order, with 2nd order accuracy.
! In 'deriv_2nd' the 3rd or higher derivatives are otherwise unavailable.
! To provide higher derivatives in this module, nghost must be 3.
!
...
!
!  16-Oct-2024/PABourdin: adapted from deriv module
!
      real, dimension (mx,my,mz,mfarray), intent(in) :: f
...
!
      if (present(upwind)) &
          call fatal_error('der4','NOT IMPLEMENTED for upwinding')
!
      if (j==1) then
        if (nxgrid/=1) then
          if (igndx) then
...
!
      if (present(upwind)) then
        if (.not. lequidist(j).and..not.lignore_nonequi) &
          call fatal_error('der6_main','upwind cannot be used with non-equidistant grid.')
        upwnd = upwind
      else
        upwnd = .false.
      endif
      ...
      elseif (j==3) then
        if (nzgrid/=1) then
          if (igndx) then
            ...
            fac=(1./60.)*dz_1(n)
            if (lcoarse_mn) fac=fac*nphis1(m)
          else
            if (lcoarse_mn) then
              fac=(dz_1(n)*nphis1(m))**6
            else
              fac=dz_1(n)**6
            endif
          endif
...
!
      if (present(upwind)) then
        if (.not. lequidist(j).and..not.lignore_nonequi) &
          call fatal_error('der6_other','upwind cannot be used with non-equidistant grid.')
        upwnd = upwind
      else
        upwnd = .false.
      endif
