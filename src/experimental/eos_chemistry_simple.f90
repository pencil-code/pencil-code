...
!          p%cp = 0.
          ...
          p%cv = 0.
!
!          if ((lpencil(i_cv)) .and. (lpencil(i_cp))) then
 ...
 !               p%cp = p%cp + cp_k(k)*f(l1:l2,m,n,ichemspec(k))
                p%cv = p%cv + cv_k(k)*f(l1:l2,m,n,ichemspec(k))
              enddo
...
! By the way, why is there grad(lnT)*T instead of grad(T)?
! Ask for that!!!!
