...
! Root part of the analysis, read config and co-ordinate other programs
        ...
        if ((nanalyzers>0) .and. (navgs > 0) .and. (nprocs_needed > 0) &
          ...
          do i=1,nprocs-1
            call MPI_SEND(command, 1, &
                          MPI_INTEGER, i, 0, MPI_COMM_WORLD, mpierr)
          end do
          call BCastInfo()
          call OpenH5Groups()
        end if
    ...
    if (.not. initerror) then
      ...
      do
        ...
        if (command == command_hdfopen) then
          ...
          ! Open logfile
          logfile = trim(directory)//'/mfcollect.log'
          ...
          do ianalyzer=1,nanalyzers
            ...
            do iavg=1,navgs
              do j=1,ndim2runs
                ...
                write(*,*) iproc, j, (j-1)*ndim2read+1, j*ndim2read
  ...
  contains
    ...
    ! Initialize HDF5
      ...
      phdffile = trim(datadir)//'/mfcollect.h5'
      write(*,*) phdffile
      ! Open hdf5-file
