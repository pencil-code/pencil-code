...
!
  ...
  logical :: linertial_frame=.true., lvectorgravity=.false.
...
!
  namelist /particles_nbody_init_pars/ &
      ...
      linertial_frame, eccentricity, semimajor_axis, lvectorgravity
!
  namelist /particles_nbody_run_pars/ &
      ...
      ltempering, lgas_gravity, ldust_gravity, linertial_frame, lvectorgravity
!
...
!
      ...
      real, dimension (mpar_loc,mparray) :: fp
      real, dimension (mpar_loc,mpvar) :: dfp
      real, dimension (mpar_loc) :: x1,y1,z1,dists2
      real, dimension (mpar_loc,3) :: evr_cart
      integer, dimension (mpar_loc,3) :: ineargrid
!
...
!
      if (lvectorgravity) then
        if (lcartesian_coords) then
          x1(1:npar_loc) = fp(1:npar_loc,ixp)
          y1(1:npar_loc) = fp(1:npar_loc,iyp)
          z1(1:npar_loc) = fp(1:npar_loc,izp)
        elseif (lcylindrical_coords) then
          x1(1:npar_loc) = fp(1:npar_loc,ixp)*cos(fp(1:npar_loc,iyp))
          y1(1:npar_loc) = fp(1:npar_loc,ixp)*sin(fp(1:npar_loc,iyp))
          z1(1:npar_loc) = fp(1:npar_loc,izp)
        elseif (lspherical_coords) then
          x1(1:npar_loc) = fp(1:npar_loc,ixp)*sin(fp(1:npar_loc,iyp))*cos(fp(1:npar_loc,izp))
          y1(1:npar_loc) = fp(1:npar_loc,ixp)*sin(fp(1:npar_loc,iyp))*sin(fp(1:npar_loc,izp))
          z1(1:npar_loc) = fp(1:npar_loc,ixp)*cos(fp(1:npar_loc,iyp))
        endif
        do ks=1,mspar
          if (lcartesian_coords) then
            x2 = fsp(ks,ixp)
            ...
            z2 = fsp(ks,izp)
          elseif (lcylindrical_coords) then
            x2 = fsp(ks,ixp)*cos(fsp(ks,iyp))
            y2 = fsp(ks,ixp)*sin(fsp(ks,iyp))
            z2 = fsp(ks,izp)
          elseif (lspherical_coords) then
            x2 = fsp(ks,ixp)*sin(fsp(ks,iyp))*cos(fsp(ks,izp))
            y2 = fsp(ks,ixp)*sin(fsp(ks,iyp))*sin(fsp(ks,izp))
            z2 = fsp(ks,ixp)*cos(fsp(ks,iyp))
          endif
          dists2(1:npar_loc) = sqrt((x1(1:npar_loc)-x2)**2.0+(y1(1:npar_loc)-y2)**2.0+(z1(1:npar_loc)-z2)**2.0+r_smooth(ks)**2.0)
          evr_cart(1:npar_loc,1) = x1(1:npar_loc)-x2
          evr_cart(1:npar_loc,2) = y1(1:npar_loc)-y2
          evr_cart(1:npar_loc,3) = z1(1:npar_loc)-z2
          if (lcartesian_coords) then
            where(ipar(1:npar_loc) /= ks) dfp(1:npar_loc,ivpx) = &
              dfp(1:npar_loc,ivpy) - GNewton*pmass(ks)*dists2(1:npar_loc)**(-1.5)*evr_cart(1:npar_loc,1)
            where(ipar(1:npar_loc) /= ks) dfp(1:npar_loc,ivpy) = &
              dfp(1:npar_loc,ivpy) - GNewton*pmass(ks)*dists2(1:npar_loc)**(-1.5)*evr_cart(1:npar_loc,2)
            where(ipar(1:npar_loc) /= ks) dfp(1:npar_loc,ivpy) = &
              dfp(1:npar_loc,ivpy) - GNewton*pmass(ks)*dists2(1:npar_loc)**(-1.5)*evr_cart(1:npar_loc,3)
          else
            where(ipar(1:npar_loc) /= ks) dfp(1:npar_loc,ivpx_cart) = &
              dfp(1:npar_loc,ivpx_cart) - GNewton*pmass(ks)*dists2(1:npar_loc)**(-1.5)*evr_cart(1:npar_loc,1)
            where(ipar(1:npar_loc) /= ks) dfp(1:npar_loc,ivpy_cart) = &
              dfp(1:npar_loc,ivpy_cart) - GNewton*pmass(ks)*dists2(1:npar_loc)**(-1.5)*evr_cart(1:npar_loc,2)
            where(ipar(1:npar_loc) /= ks) dfp(1:npar_loc,ivpz_cart) = &
              dfp(1:npar_loc,ivpz_cart) - GNewton*pmass(ks)*dists2(1:npar_loc)**(-1.5)*evr_cart(1:npar_loc,3)
          endif
        enddo
      else
        do k=npar_loc,1,-1
