...
!
      ...
      logical :: initial
! dNds stuff
      real, dimension (10000000) :: radii, beta, e0
      integer :: Nadded, iradius
      real :: prob, rando, randoo, radnorm
!
      initial=.false.
...
!
        case ('strubbechiang')
          !radii = (/ (2.000001+(real (i-1))*4.999e-08, i = 1, 10000000) /)
          radii = (/ (2.2+(real (i-1))*9.5455e-07, i = 1, 10000000) /)
          radnorm = ((radii(1)**(-3.5))*(1.0-e0(1))**(-1.5))
          beta = 1.0/radii
          e0 = beta/(1.0-beta)
          Nadded = 0
          do while (Nadded<=(npar_high-npar_low))
           call random_number_wrapper(randoo)
           iradius = min(nint(1e7*randoo)+1,10000000)
           prob = ((radii(iradius)**(-3.5))*(1.0-e0(iradius))**(-1.5))/radnorm
           call random_number_wrapper(rando)
           if (rando<prob) then
             fp(npar_low+Nadded,iap) = radii(iradius)
             Nadded = Nadded+1
           endif
          enddo
!
