! $Id: radiation_ray_new.f90,v 1.2 2004-10-27 14:21:47 ajohan Exp $
...
!
  ...
  real, dimension (mx,my,mz) :: Srad,lnchi,tau,dtau,Qrad,Qrad0
  real, dimension (mx,my,mz) :: emdtau,emdtau1st,emdtau2nd
  integer, dimension (maxdir/2,3) :: dir
...
!
      weight=1./(2.*ndir2)
!
      if (lroot.and.ip<14) print*,'initialize_radiation: ndir=',2*ndir2
!
...
!
      ...
      do facdir=1,-1,-2
        ...
        call raydirection(facdir)

        if (facdir==1) then
          call Qintrinsic_common
          call Qintrinsic_forward
        else
          call Qintrinsic_backwards
        endif
        ...
        f(:,:,:,iQrad)=f(:,:,:,iQrad)+weight(idir2)*Qrad
...
!***********************************************************************
    subroutine raydirection(facdir)
!
...
!
      lrad=facdir*dir(idir2,1)
      mrad=facdir*dir(idir2,2)
      nrad=facdir*dir(idir2,3)
!
...
!
      ...
      use IO, only: output
!
      real :: Srad1st,Srad2nd,dlength
      real :: dSdtau_m,dSdtau_p,dtau_m,dtau_p
      integer :: l,m,n
      character(len=3) :: raydir
!
!  identifier
!
      if(ldebug.and.headt) print*,'Qintrinsic_common'
!
!  line elements
!
...
!
      ...
      do l=l1,l2+lrad
        ...
        dtau(l,m,n)=sqrt(exp(lnchi(l-lrad,m-mrad,n-nrad)+lnchi(l,m,n)))*dlength

        if (dtau(l,m,n)>dtau_thresh1) then
          emdtau(l,m,n)=0.0
          emdtau1st(l,m,n)=1.0
          emdtau2nd(l,m,n)=-1.0
        elseif (dtau(l,m,n)<dtau_thresh2) then
          emdtau1st(l,m,n)=dtau(l,m,n)*(1-0.5*dtau(l,m,n)*(1-0.33333333*dtau(l,m,n)))
          emdtau2nd(l,m,n)=-dtau(l,m,n)**2*(0.5+0.33333333*dtau(l,m,n))
          emdtau(l,m,n)=1-emdtau1st(l,m,n)
        else
          emdtau(l,m,n)=exp(-dtau(l,m,n))
          emdtau1st(l,m,n)=1-emdtau(l,m,n)
          emdtau2nd(l,m,n)=emdtau(l,m,n)*(1+dtau(l,m,n))-1
        endif
...
!
    endsubroutine Qintrinsic_common
!***********************************************************************
    subroutine Qintrinsic_forward
!
!  Integration radiation transfer equation along rays
!
!  This routine is called before the communication part
...
!  identifier
!
      if(ldebug.and.headt) print*,'Qintrinsic_forward'
!
!  set heating rate and optical depth initially to zero
!
      Qrad=0
      tau=0
!
...
!
      ...
      do l=l1,l2,lsign
        ...
        dtau_m=dtau(l     ,m     ,n     )
        dtau_p=dtau(l+lrad,m+mrad,n+nrad)
        ...
        Qrad(l,m,n)=Qrad(l-lrad,m-mrad,n-nrad)*emdtau(l,m,n) &
                   -Srad1st*emdtau1st(l,m,n) &
                   -Srad2nd*emdtau2nd(l,m,n)
        tau(l,m,n)=tau(l-lrad,m-mrad,n-nrad)+dtau(l,m,n)
...
!
    endsubroutine Qintrinsic_forward
!***********************************************************************
    subroutine Qintrinsic_backwards
!
!  Integration radiation transfer equation along rays
!
!  This routine is called before the communication part
...
!  identifier
!
      if(ldebug.and.headt) print*,'Qintrinsic_backwards'
!
!  set heating rate and optical depth initially to zero
!
      Qrad=0
...
!
      ...
      do l=l2,l1,lsign
        ...
        dtau_m=dtau(l-lrad,m-mrad,n-nrad)
        dtau_p=dtau(l     ,m     ,n     )
        dSdtau_m=(Srad(l,m,n)-Srad(l-lrad,m-mrad,n-nrad))/dtau_m
        dSdtau_p=(Srad(l+lrad,m+mrad,n+nrad)-Srad(l,m,n))/dtau_p
        Srad1st=(dSdtau_p*dtau_m+dSdtau_m*dtau_p)/(dtau_m+dtau_p)
        ...
        Qrad(l,m,n)=Qrad(l-lrad,m-mrad,n-nrad)*emdtau(l-lrad,m-mrad,n-nrad) &
                   -Srad1st*emdtau1st(l-lrad,m-mrad,n-nrad) &
                   -Srad2nd*emdtau2nd(l-lrad,m-mrad,n-nrad)
        tau(l,m,n)=tau(l-lrad,m-mrad,n-nrad)+dtau(l-lrad,m-mrad,n-nrad)
...
!
    endsubroutine Qintrinsic_backwards
!***********************************************************************
...
!
      if (mrad/=0) then
        ...
        if (ipy/=ipystart) call radboundary_zx_recv(mrad,idir2,Qrad0_zx)
...
!
      if (nrad/=0) then
        ...
        if (ipz/=ipzstart) call radboundary_xy_recv(nrad,idir2,Qrad0_xy)
...
!
       ...
       *exp(-tau(llstart-lrad:llstop:lsign,mmstop,nnstart-nrad:nnstop:nsign)) &
        ...
        call radboundary_zx_send(mrad,idir2,Qrad0_zx)
      endif
!
       ...
       *exp(-tau(llstart-lrad:llstop:lsign,mmstart-mrad:mmstop:msign,nnstop)) &
        ...
        call radboundary_xy_send(nrad,idir2,Qrad0_xy)
      endif
!
