...
!  
!*********************************************************************** 
  subroutine get_grid_pos(phys_pos, grid_pos)
!
...
!
    ...
    integer :: l, loop_count
    real, dimension (1024,7) :: tracers
    integer :: n_tracers, tracer_idx
    real, dimension (nx,ny,nz,3) :: bb_aux ! the auxiliary magnetic field B
...
!       (a) Single step (midpoint method):
        ...
        call get_grid_pos(x_mid,grid_pos)
        if (any(grid_pos <= 0) .or. (grid_pos(1) > nx) .or. &
            (grid_pos(2) > ny) .or. (grid_pos(3) > nz)) exit
...
!       (b) Two steps with half stepsize:
        ...
        call get_grid_pos(x_half,grid_pos)
        if (any(grid_pos <= 0) .or. (grid_pos(1) > nx) .or. &
            (grid_pos(2) > ny) .or. (grid_pos(3) > nz)) exit
