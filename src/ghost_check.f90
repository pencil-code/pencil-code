          ...
          do py = 1, num_y
            do px = 1, num_x
              if (a(px,py,pz,pa) /= b(px,py,pz,pa)) then
                write (200+iproc,*) msg, ' => ', px, py, pz, pa, ' : ', a(px,py,pz,pa), b(px,py,pz,pa)
                blocks_equal = .false.
              endif
            enddo
          enddo
...
!
      if (lroot) then       ! to avoid compile-time length calculation in allocation of "global" below
...
!
      ! check in X:
      write (200+iproc,*) 'Checking X-direction:'
      do py = 1, nprocy
        do pz = 1, nprocz
          if (nprocx == 1) then
            if (lperi(1)) then
              write (200+iproc,*) 'Subdomain ', 1, py, pz
            ...
            do px = 1, nprocx
              write (200+iproc,*) 'Subdomain ', px, py, pz
              lx = px - 1
...
!
      ! check in Y:
      write (200+iproc,*) 'Checking Y-direction:'
      do px = 1, nprocx
        do pz = 1, nprocz
          if (nprocy == 1) then
            if (lperi(2)) then
              write (200+iproc,*) 'Subdomain ', px, 1, pz
...
!
      ! check in z:
      write (200+iproc,*) 'Checking Z-direction:'
      do px = 1, nprocx
        do py = 1, nprocy
          if (nprocz == 1) then
            if (lperi(3)) then
              write (200+iproc,*) 'Subdomain ', px, py, 1
