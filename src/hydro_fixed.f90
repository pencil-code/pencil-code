...
!  equations: iuu, etc; increase naux accordingly.
!
!  6-nov-01/wolf: coded
!
      use Cdata
...
!
      lhydro_fixed = .true.
!
      iuu = mvar+naux+1             ! indices to access uu
      iux = iuu
      iuy = iuu+1
      iuz = iuu+2
      naux = naux+3             ! added 3 variables
!
      if ((ip<=8) .and. lroot) then
        print*, 'register_hydro: naux = ', naux
...
!
      if (naux > maux) then
        ...
        call stop_it('register_hydro: naux > maux')
      endif
!
!  Writing files for use with IDL
!
      if (naux < maux) aux_var(aux_count)=',uu $'
      if (naux == maux) aux_var(aux_count)=',uu'
      aux_count=aux_count+3
      if (lroot) write(15,*) 'uu = fltarr(mx,my,mz,3)*one'
!
...
!  11-jun-04/anders: call init_uu again for hydro_fixed with not lstarting
!
...
!  
!  Rewrite initial condition into f for fixed hydro
!        
      if (.not. lstarting) call init_uu(f,spread(spread(x,2,my),3,mz), &
...
!
!  Velocity auxiliary variables and diagnostics
!
!   9-jun-04/anders: adapted from hydro
!
      use Cdata
      use Sub
      use Slices
!
      ...
      real, dimension (nx) :: u2,divu,o2,ou,rho1,rho,ux,uy,uz,shock,u2u13
      integer :: i,j
!
      intent(in) :: f,rho1
      intent(out) :: uu,glnrho,divu,u2
!
!  identify module
!
      if (headtt.or.ldebug) print*,'duu_dt: fixed velocity field'
!
!  abbreviations
!
