...
module Global
!
!  Read in global current and magentic field.
!
!  28-may-06/tobi: Adapted from noglobal.f90
!
  use Cparam
  use Messages
  ...
  real, dimension (mx,my,mz,3) :: jj_global,bb_global
...
!
      ...
      case ('jj_ext')
        var = jj_global(l1:l2,m,n,:)

      case ('bb_ext')
        var = bb_global(l1:l2,m,n,:)

      case default
        call fatal_error('get_global_vect','No such value for label '//&
                                           trim(label))
...
!
      ...
      call input(trim(directory)//'/jj_ext.dat',jj_global,3,0)
      call input(trim(directory)//'/bb_ext.dat',bb_global,3,0)
