...
!
      ...
      integer :: icount,i
      real :: residual,aout,alpha,beta,dTTdx      
!
!  Define r.h.s.
!
...
!
      nu=assert_eq((/size(u,1),size(u,2)/),'successive_over_relaxation')
      nr=assert_eq((/size(r,1),size(r,2)/),'successive_over_relaxation')
...
!     
           if (lsolver_highorder) then
             ...
             call solve_highorder(u,0.,0.,ufactor,vterm,ii,nn)
           else
              !call solve_loworder(psi,alpha,beta,u,v,i,n)
              if (present(h)) then
                 call solve_loworder(u,0.,0.,ufactor,vterm,ii,nn)
              else
                 call solve_loworder(u,0.,0.,ufactor,vterm,ii,nn,h)
              endif
           endif
!     
...
!
      integer :: m,n,ng,ngrid,j,nn,i
!
      m=assert_eq((/size(psi,1),size(psi,2)/),'mglin')
      n=assert_eq((/size(rhs,1),size(rhs,2)/),'mglin')
      ngrid=nint(log(nx-1.0)/log(2.0))
      if (nx /= 2**ngrid+1) call fatal_error("mglin","n-1 must be a power of 2 in mglin")
!
      allocate(grid(ngrid))
...
!
      grid(1)%u=0.
      call slvsml(grid(1)%u,grid(1)%r,.false.)
!
...
!        
      if (j/=1) then
!           
         ngrid=nint(log(nx-1.0)/log(2.0)) !-1
!
         do jpre=1,npre
            if (j==ngrid .and. (.not.lpoisson_test)) call update_bounds_psi(u)
            call relax(u,rhs)
            if (j==ngrid .and. (.not.lpoisson_test)) call update_bounds_psi(u)
         enddo
!
...
!
      real, dimension(:,:) :: v
      real, dimension(2*size(v,1)-2*nghost-1,2*size(v,1)-2*nghost-1) :: u
      integer :: nv,nu,mv,mu,lu1,lu2,lv1,lv2,jpost,j,ngrid
!
      ngrid=nint(log(nx-1.0)/log(2.0)) !-1
!
      u=grid(j)%u
      nv=assert_eq((/size(v,1),size(v,2)/),'upward'); mv=nv+2; lv1=nghost+1; lv2=mv-nghost
      nu=assert_eq((/size(u,1),size(u,2)/),'upward'); mu=nu+2; lu1=nghost+1; lu2=mu-nghost
      u(lu1:lu2,lu1:lu2)=u(lu1:lu2,lu1:lu2)+interp(v(lv1:lv2,lv1:lv2))
!
!  Post-smoothing.
!
      do jpost=1,npost
         if (j==ngrid .and. (.not.lpoisson_test)) call update_bounds_psi(u)
         call relax(u,grid(j)%r)
         if (j==ngrid .and. (.not.lpoisson_test)) call update_bounds_psi(u)
      enddo
      grid(j)%u=u
!        
...
!  the coarse-grid solution is returned in the Nc × Nc array rstrct.
!
!  13-jan-16/wlad: from numerical recipes.
!
      implicit none
      real, dimension(:,:), intent(in) :: uf
      real, dimension((size(uf,1)+1)/2,(size(uf,1)+1)/2) :: rstrct
      integer :: nc,nf
!
...
!  Interior points
!      
      rstrct(2:nc-1,2:nc-1) =   0.5 *uf(3:nf-2:2,3:nf-2:2) + &
...
!  solution is returned in interp.
!
!  13-jan-16/wlad: from numerical recipes.
!
      implicit none
      real, dimension(:,:), intent(in) :: uc
      real, dimension(2*size(uc,1)-1,2*size(uc,1)-1) :: interp
      integer :: nc,nf
!
...
!
      interp(2:nf-1:2,1:nf:2) = 0.5*(interp(3:nf:2,1:nf:2) + interp(1:nf-2:2,1:nf:2))
!
! Do even-numbered columns, interpolating horizontally.
!
      interp(1:nf,2:nf-1:2) = 0.5*(interp(1:nf,3:nf:2) + interp(1:nf,1:nf-2:2))
!
    endfunction interp
!********************************************************************
    subroutine slvsml(u,rhs,lboundary)
!
...
!
      ...
      real, dimension(size(rhs,1),size(rhs,1)) :: u_old
      real :: h,fac
      logical :: lboundary
!
      real :: res,tol
...
!
      nu=assert_eq((/size(u,1),size(u,2)/),'slvsml')
      nr=assert_eq((/size(rhs,1),size(rhs,2)/),'slvsml')
...
!
         do while (res > tol)
!
            if (lboundary.and..not.lpoisson_test) then
               u(nn1,:) = 0; u(:,nn1)=0
               do i=1,nghost
                  u(:,nn1-i) = - u(:,nn1+i)
                  u(:,nn2+i) = - u(:,nn2-i)
...
!
            ...
            if (ldebug) print*,'slvsml',icount,res
         enddo
!
      endif
!         
    endsubroutine slvsml
!********************************************************************
...
!
      ...
      integer :: ii,nn
!
      m=assert_eq((/size(u,1),size(u,2)/),'relax')
      n=assert_eq((/size(rhs,1),size(rhs,2)/),'relax')
!
      h=1.0/(n-1)
!
! Convection
      ...
      if (.not.lpoisson_test) then 
         do ii=2,n-1
            ...
            do nn=2,n-1
               k=nn+nghost
               cc = rhs(ii,nn)
               call solve_loworder(u,0.0,0.0,ufactor,vterm,l,k,h)
...
!
      ...
      integer :: ii,nn
!
      m=assert_eq((/size(u,1),size(u,2)/),'resid')
      n=assert_eq((/size(rhs,1),size(rhs,2)/),'resid')
!
      h=1.0/(n-1)
!
!  Interior points.
