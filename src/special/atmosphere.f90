...
!
      ...
      !    ind_cloud=k
      !  endif
        if (trim(varname(ichemspec(k)))=='H2O') ind_H2O=k
        if (trim(varname(ichemspec(k)))=='N2') ind_N2=k
!
      enddo
!
      if (lroot) then
        print*,'special/atmosphere: water index', ind_H2O
        print*,'special/atmosphere: N2 index', ind_N2
      endif
!
...
!
      if (lgrav_LES) df(l1:l2,m,n,iuz)=df(l1:l2,m,n,iuz) - gg
...
!              if ((z(n) >= zgrid(nn2-3)) .and. (z(n) <= zgrid(nn2-1))) &
!                df(l1:l2,m,n,iuz)=df(l1:l2,m,n,iuz)-(f(l1:l2,m,n,iuz)-0.)*dt1
...
!
      df(l1:l2,m,n,ilnTT) = df(l1:l2,m,n,ilnTT) + 2.5e6/1005.*p%ccondens*p%TT1
!
      call keep_compiler_quiet(df)
      call keep_compiler_quiet(p)
!
      ...
      if (lbuffer_zone_chem) then
        do j=1,2
          ...
          if ((j==1) .and. (x(l2)==xyz0(1)+Lxyz(1))) then
            ...
            lzone_right=.true.
!            df(ll1:ll2,m,n,iuy)=  &
!            df(ll1:ll2,m,n,iuy)+(f(ll1:ll2,m,n,iuy)-0.)*dt1/5.
!
...
!
      if (lpscalar) then
        do i=1,nx
          do k=1,ndustspec
           ff_tmp(k)= (p%ppwater(i)-p%ppsf(i,k))*f(l1+i-1,m,n,ind(k))/dsize(k)
          enddo
!
...
!
     if (lACTOS) call dustspec_normalization_(f)
!
...
!
      ...
      real, dimension(2), parameter :: jet_center=0.
      real, dimension(my,mz) :: u_profile
!
      do jjj=1,my
      do kkk=1,mz
        rad_2=((y(jjj)-jet_center(1))**2+(z(kkk)-jet_center(2))**2)
...
!
        ...
        print*,"bc_satur_x: ",bc%location," should be `top(", iBC_X_TOP,")' or `bot(",iBC_X_BOT,")'"
      endif
!
...
!
      if (bc%location==iBC_Z_BOT) then
        ...
        do i = 1,59
          if ( t>=time_bot(i) .and. t<time_bot(i+1) ) time_position_bot=i
!         if ( t>=time_top(i) .and. t<time_top(i+1) ) time_position=i
        enddo
!
...
!
        do i = 1,59
          if (t>=time_top(i) .and. t<time_top(i+1)) time_position_top=i
        enddo
!
...
!           f(j,i,n2,vr)=2481.*bc_T_final_top/exp(f(j,i,n2,ilnTT))
            ...
            if (nxgrid>1) f(j,i,n2,vr)=sin(Period*PI*x(j)/Lxyz(1))*uz_bc
!
          enddo
          enddo
!
