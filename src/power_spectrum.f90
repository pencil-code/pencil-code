...
!
        ...
        enddo
        enddo
      elseif (trim(sp)=='gph') then
        !$omp do collapse(2)
        do n_loc=n1,n2
        do m_loc=m1,m2
          m=m_loc;n=n_loc
          call der(f,ispecialvar,a1(:,m-nghost,n-nghost),ivec)
        enddo
        enddo
...
!  Need m=m_loc;n=n_loc to make gij etc work correctly.
!
    !$omp do collapse(2)
    do m_loc=m1,m2
    do n_loc=n1,n2
      m=m_loc;n=n_loc
      if (ijxb>0) then
        a_vec_re(:,m-nghost,n-nghost,:)=f(l1:l2,m,n,ijxbx:ijxbz)
      else
...
!
    ...
    !$ thread_id = omp_get_thread_num()+1
!
!  Lorentz force spectra (spectra of L*L^*=a^2) and work against Lorentz (a*b).
!
      if (sp=='Lor') then
        !$omp workshare
        b_re=a_vec_re(:,:,:,ivec)
...
!
    ...
    do n_loc=n1,n2
      ...
      a_vec_re(:,m-nghost,n-nghost,:)=oxu
    enddo
    enddo
    !$omp end parallel
!
...
!
    ...
    !$ thread_id = omp_get_thread_num()+1
!
!  oxu force spectra (spectra of L*L^*=a^2) and work against oxu (a*b).
!
      if (sp=='OmU') then
        !$omp workshare
        b_re=a_vec_re(:,:,:,ivec)
...
!
!  Calculate spectra of (phi'*B) E^*
!
!  18-jun-26/axel: adapted from powerEMF
!
    use Fourier, only: fft_xyz_parallel
    use Mpicomm, only: mpireduce_sum
    use Sub, only: gij, gij_etc, curl_mn, cross_mn
...
!  integration over shells
!
      if (ip<10) call information('poweraBE','fft done, now integrate over shells')
      !$omp do collapse(3) reduction(+:spectrum,spectrumhel,k2m,nks)
      do ikz=1,nz
        do iky=1,ny
          do ikx=1,nx
