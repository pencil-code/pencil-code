...
!
                  do izz=izz0,izz1; do iyy=iyy0,iyy1; do ixx=ixx0,ixx1
                    if ( ((ixx-ix0)==-1) .or. ((ixx-ix0)==+1) ) then
                      weight_x=1.125-1.5* abs(fp(k,ixp)-xb(ixx,ib))*dx1+ &
                                     0.5*(abs(fp(k,ixp)-xb(ixx,ib))*dx1)**2
                    else
                      if (nxgrid/=1) &
                           weight_x=0.75-   ((fp(k,ixp)-xb(ixx,ib))*dx1)**2
                    endif
                    if ( ((iyy-iy0)==-1) .or. ((iyy-iy0)==+1) ) then
                      weight_y=1.125-1.5* abs(fp(k,iyp)-yb(iyy,ib))*dy1+ &
                                     0.5*(abs(fp(k,iyp)-yb(iyy,ib))*dy1)**2
                    else
                      if (nygrid/=1) &
                          weight_y=0.75 -   ((fp(k,iyp)-yb(iyy,ib))*dy1)**2
                    endif
                    if ( ((izz-iz0)==-1) .or. ((izz-iz0)==+1) ) then
                      weight_z=1.125-1.5* abs(fp(k,izp)-zb(izz,ib))*dz1+ &
                                     0.5*(abs(fp(k,izp)-zb(izz,ib))*dz1)**2
                    else
                      if (nzgrid/=1) &
