...
!
!  ===> WARNING! <===
!  This module is experimental and the data format may change any time.
!  There may be sudden errors, missing functionality, and data losses.
!  Future versions of this module may be incompatible with this version.
!  ===> Use it only if you agree with the above terms!
!
!
module HDF5_IO
!
...
!
  interface input_hdf5
    ...
    module procedure input_local_hdf5_int_1D_0D
...
!
  interface output_hdf5
    ...
    module procedure output_local_hdf5_1D_0D
    module procedure output_local_hdf5_int_1D_0D
...
!
  ...
  integer, dimension(:), allocatable :: ts_offsets
!
...
!
      if (allocated (ts_offsets)) deallocate (ts_offsets)
!
...
!***********************************************************************
    subroutine input_local_hdf5_int_1D_0D(name, data, pos)
!
!  Read integer scalar from local HDF5 data array at given position.
!
!  19-Apr-2026/PABourdin: coded
!
      use General, only: loptest
!
      character(len=*), intent(in) :: name
...
!
      ...
      call check_error (h5_err, 'close dataset', name)
!
    endsubroutine input_local_hdf5_int_1D_0D
!***********************************************************************
...
!
      ...
      call h5screate_simple_f (1, glob_dim, h5_fspace, h5_err)
      call check_error (h5_err, 'create file space', name, caller='output_local_hdf5_1D_0D')
!
      ! define 'memory-space' to indicate a scalar in memory
      call h5screate_f (H5S_SCALAR_F, h5_mspace, h5_err)
      call check_error (h5_err, 'create memory space', name)
!
      if (exists_in_hdf5 (name)) then
        ! open dataset
        call h5dopen_f (h5_file, trim (name), h5_dset, h5_err)
...
!
      ! define 'hyper-slab' in the file
      h5_stride(:) = 1
      h5_count(:) = 1
      call h5sselect_hyperslab_f (h5_fspace, H5S_SELECT_SET_F, loc_start, h5_count, h5_err, h5_stride, loc_dim)
      call check_error (h5_err, 'select hyperslab within file', name)
!
      ! write dataset
      call h5dwrite_f (h5_dset, h5_ntype, data, glob_dim, h5_err, h5_mspace, h5_fspace)
      call check_error (h5_err, 'write dataset', name)
!
      ! close data spaces, dataset, and the property list
      call h5sclose_f (h5_fspace, h5_err)
...
!
      ! define 'file-space' to indicate the data portion in the file
      call h5screate_simple_f (1, glob_dim, h5_fspace, h5_err)
      call check_error (h5_err, 'create file space', name, caller='output_local_hdf5_int_1D_0D')
!
      ! define 'memory-space' to indicate a scalar in memory
      call h5screate_f (H5S_SCALAR_F, h5_mspace, h5_err)
      call check_error (h5_err, 'create memory space', name)
...
!
      ! write dataset
      call h5dwrite_f (h5_dset, H5T_NATIVE_INTEGER, data, glob_dim, h5_err, h5_mspace, h5_fspace)
      call check_error (h5_err, 'write dataset', name)
!
      ! close data spaces, dataset, and the property list
      call h5sclose_f (h5_fspace, h5_err)
      ...
      call check_error (h5_err, 'close dataset', name)
!
    endsubroutine output_local_hdf5_int_1D_0D
!***********************************************************************
...
!
!  18-Apr-2026/PABourdin: adapted from hdf5_io_parallel
!
      use File_io, only: file_exists
      use General, only: itoa
!
...
!
      if (.not. lroot) return
!
      if (.not. allocated (ts_offsets)) then
        allocate (ts_offsets(2*nname))
        ts_offsets(:) = -1
      endif
!
