...
module Entropy
  ...
  logical :: l1D_cooling=.false.,l1D_heating=.false., lheatc_diffusion=.false.
...
!AB: mixinglength_flux is used as flux in mixing length initial condition
      ...
      T0,ampl_TT,kx_ss,beta_glnrho_global,ladvection_entropy, lviscosity_heat, l1D_cooling,l1D_heating
  ! run parameters
...
!AB: They are used to re-calculate the radiative conductivity profile.
      ...
      tdown, allp,beta_glnrho_global,ladvection_entropy, lviscosity_heat, l1D_cooling,l1D_heating
...
!
      ...
      do i=1,nheatc_max
        ...
        case('diffusion')
          lheatc_diffusion=.true.
          if (lroot) print*, 'heat conduction: diffusion of radiation'
        case default
          if (lroot) then
...
!
      ...
      real :: beta1
!
      beta1 = gamma/gamma1*gravz/(mpoly+1)  ! gamma1*cp/gamma=R_{*}
!
      do m=m1,m2
      do n=n1,n2
    ...
    !     endif 
     ...
     !       -1./(5.*dt)*(p%TT(:)/(cs0**2/(gamma1*cp))-1.)
      ...
      !      -1./(5.*dt)*(p%TT(l_sz_1:nxgrid)-TT_cs0)/TT_cs0
         ...
         !  df(l_sz:l2,m,n,iss)=df(l_sz:l2,m,n,iss) &
         !  -1./(5.*dt)*(f(l_sz:l2,m,n,iss)-log(TT_cs0)/gamma)/p%rho(l_sz_1:nxgrid)/p%TT(l_sz_1:nxgrid) 
...
!***************************************************************
!***************************************************************
    subroutine entropy_step(f,xx,zz,Tstar)
!Natalia
