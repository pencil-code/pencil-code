...
!
  include 'energy.h'
!
  public :: denergy_dt_copy,denergy_dt_copy_2,denergy_dt_copy_3,calc_heatcond,calc_heatcond_copy
...
!
!  Register slot for running average of entropy if required.
!
...
!***********************************************************************
subroutine denergy_dt_copy_3(f,df,p)
...
real, dimension(nxgrid/nprocx) :: tmp1
integer :: j
logical :: notanumber_2_return_value_1
real, dimension (nxgrid/nprocx,3)::glnthcond_15
real, dimension (nxgrid/nprocx,3)::glhc_15
real, dimension (nxgrid/nprocx) :: chix_15
real, dimension (nxgrid/nprocx)::thdiff_15
real, dimension (nxgrid/nprocx)::g2_15
real, dimension (nxgrid/nprocx)::del2ss1_15
real, dimension (nxgrid/nprocx) :: glnrhoglnt_15
real, dimension (nxgrid/nprocx,3) :: gradchit_prof_15
real, dimension (nxgrid/nprocx,3,3) :: tmp_15
real::s2_15
real::c2_15
real::sc_15
integer :: j_15
real, dimension(nxgrid/nprocx)::r_mn_3_15
real, dimension(nxgrid/nprocx)::r_mn1_3_15
integer :: j_3_15
logical :: notanumber_2_return_value_4_15
integer :: i_6_15
logical :: loptest_return_value_5_6_15
logical :: notanumber_1_return_value_7_15
logical :: notanumber_1_return_value_8_15
logical :: notanumber_1_return_value_9_15
logical :: notanumber_2_return_value_10_15
logical :: notanumber_1_return_value_11_15
logical :: notanumber_2_return_value_12_15
logical :: notanumber_1_return_value_13_15
logical :: notanumber_1_return_value_14_15
logical :: notanumber_return_value_14_15
hmax = 1./3.9085e37
ssmax = 1./3.9085e37
...
glhc_15(:,3)=dlnhcond_prof(n-3)
glhc_15(:,1:2)=0.
chix_15 = p%rho1*hcond*p%cp1
glnthcond_15 = p%glntt + glhc_15
loptest_return_value_5_6_15=.false.
g2_15=p%glntt(:,1)*glnthcond_15(:,1)
do i_6_15=2,3
g2_15=g2_15+p%glntt(:,i_6_15)*glnthcond_15(:,i_6_15)
enddo
thdiff_15 = p%rho1*hcond * (p%del2lntt + g2_15)
endif
if(hcond0/=0..or.lread_hcond.or.0.00000000/=0.) then
df(1+3:l2,m,n,iss) = df(1+3:l2,m,n,iss) + thdiff_15
endif
endsubroutine denergy_dt_copy_3

    subroutine denergy_dt_copy_2(f,df,p)
!
...
!
    ...
    endsubroutine denergy_dt_copy_2
...
!***********************************************************************
    subroutine denergy_dt_copy(f,df,p)
    ...
    integer :: j
    logical :: notanumber_2_return_value_1
    real, dimension (nxgrid/nprocx)::tmp_4
    real, dimension (nxgrid/nprocx)::heat_4
    real, dimension (nxgrid/nprocx)::tt_drive_4
    real, dimension (nxgrid/nprocx)::prof_4
    real, dimension (nxgrid/nprocx)::prof_3_4
    real::zbot_3_4
    real::ztop_3_4
    logical :: save_name
    hmax = 1./3.9085e37
    ...
    print*,'denergy_dt: solve denergy_dt'
    endif
    df(1+3:l2,m,n,iux:iuz) = df(1+3:l2,m,n,iux:iuz) + p%fpres
    if(any(beta_glnrho_scaled/=0.0)) then
    if(headtt) then
    print*, 'denergy_dt: adding global pressure gradient force'
    endif
    do j=1,3
    df(1+3:l2,m,n,(iux-1)+j) = df(1+3:l2,m,n,(iux-1)+j) - p%cs2*beta_glnrho_scaled(j)
    enddo
    endif
    df(1+3:l2,m,n,iss) = df(1+3:l2,m,n,iss) - p%ugss
    do j=1,3
    if(grads0_imposed(j)/=0.) then
    df(1+3:l2,m,n,iss)=df(1+3:l2,m,n,iss)-grads0_imposed(j)*p%uu(:,j)
    endif
    enddo
    diffus_chi=0.
    diffus_chi3=0.
    df(1+3:l2,m,n,iss) = df(1+3:l2,m,n,iss) + p%tt1*p%visc_heat
    if(headtt) then
    print*, 'calc_heat_cool: lgravz, lgravr, lgravx, lspherical_coords=',  .true., .false., .false., .false.
    endif
    heat_4=0.0
    prof_3_4 = spread(exp(-0.5*((ztop_3_4-z(n))/0.200000003)**2), 1, nx)
    heat_4 = heat_4 - 15.0000000*prof_3_4*(p%cs2-0.333332986)/0.333332986
    df(1+3:l2,m,n,iss) = df(1+3:l2,m,n,iss) + p%tt1*p%rho1*heat_4
    call calc_diagnostics_energy(f,p)
    endsubroutine denergy_dt_copy
...
! from calc_heatcond, calc_heatcond_kramers
...
!
    ...
    subroutine calc_heatcond_copy(f,df,p)
real, dimension (nxgrid/nprocx+2*3,nygrid/nprocy+2*3,nzgrid/nprocz+2*3,5+0+0+0) :: f
real, dimension (nxgrid/nprocx+2*3,nygrid/nprocy+2*3,nzgrid/nprocz+2*3,5) :: df
type (pencil_case) :: p
intent(in) :: f,p
intent(inout) :: df
real, dimension (nxgrid/nprocx,3) :: glnthcond,glhc
real, dimension (nxgrid/nprocx) :: chix
real, dimension (nxgrid/nprocx) :: thdiff,g2,del2ss1
real, dimension (nxgrid/nprocx) :: glnrhoglnt
real, dimension (nxgrid/nprocx,3) :: gradchit_prof
real, dimension (nxgrid/nprocx,3,3) :: tmp
real :: s2,c2,sc
integer :: j
real, dimension(nxgrid/nprocx)::r_mn_0
real, dimension(nxgrid/nprocx)::r_mn1_0
integer :: j_0
logical :: notanumber_2_return_value_1
integer :: i_3
logical :: loptest_return_value_2_3
logical :: notanumber_1_return_value_4
logical :: notanumber_1_return_value_5
logical :: notanumber_1_return_value_6
logical :: notanumber_2_return_value_7
logical :: notanumber_1_return_value_8
logical :: notanumber_2_return_value_9
logical :: notanumber_1_return_value_10
logical :: notanumber_1_return_value_11
logical :: notanumber_return_value_11
if(hcond0/=0..or..false.) then
hcond=hcond_prof(n-3)
glhc(:,3)=dlnhcond_prof(n-3)
glhc(:,1:2)=0.
chix = p%rho1*hcond*p%cp1
glnthcond = p%glntt + glhc
loptest_return_value_2_3=.false.
if(.true.) then
g2=p%glntt(:,1)*glnthcond(:,1)
do i_3=2,3
g2=g2+p%glntt(:,i_3)*glnthcond(:,i_3)
enddo
else
do i_3=1,3
g2=g2+p%glntt(:,i_3)*glnthcond(:,i_3)
...
! df(1+3:l2,m,n,iss) = df(1+3:l2,m,n,iss) + thdiff
endif
endsubroutine calc_heatcond_copy
!***********************************************************************
...
!
      ...
      real, pointer :: gravx        ! ,xc,xb   ! are not put shared!
...
! Hydrostatic equilibrium relations
      dTTdxc = gravx_xpencil*cv1 / (gamma_m1*(mpoly_xprof+1.))
