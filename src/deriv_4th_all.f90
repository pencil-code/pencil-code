! $Id$
!
! This module is a copy of the 'deriv_4th' module, but implements all
! derivatives, including up to 6th order, with 4th order accuracy.
! In 'deriv_4th' the 5th or higher derivatives are otherwise unavailable.
! To provide higher derivatives in this module, nghost must be 4.
!
...
!
      ...
      real, parameter :: der2_coef0=-30., der2_coef1=16.
      real :: facs
!
      if (j==1) then
        if (nxgrid/=1) then
          fac=(1.0/12)*dx_1(l1:l2)**2
...
!
!  17-Oct-2024/PABourdin: adapted from deriv_6th
!
      real, dimension (mx,my,mz,mfarray), intent(in) :: f
      real, dimension (nx), intent(out) :: df
      integer, intent(in) :: j, k
...
!  the ratios dx:dy:dz.
!
!  17-Oct-2024/PABourdin: adapted from deriv_8th
!
      real, dimension (mx,my,mz,mfarray), intent(in) :: f
      real, dimension (nx), intent(out) :: df
      integer, intent(in) :: j,k
...
!
      ...
      integer, intent(in) :: j, k
!
      call fatal_error('der2_minmod','Not implemented for deriv_4th')
