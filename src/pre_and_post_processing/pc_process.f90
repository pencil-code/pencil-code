...
!
  real(:,:,:,:,:), allocatable :: global_output
  real(nx,3) :: tmp
  integer :: imn, pos
  integer, parameter :: num_quantities = 2
  integer :: num_components
  integer(num_quantities), parameter :: global_start = (/ 1, 4 /)
  integer(num_quantities), parameter :: global_size = (/ 3, 3 /)
  integer(num_quantities), parameter :: output_luns = (/ 71, 72, /)
  character(num_quantities,:) :: output_names = (/ "curl_a", "b", /)
!
...
!
  if (IO_strategy == "collect") call fatal_error ('pc_process', &
      "Snapshots are already collected, when using the 'io_collect' module.")
  if (IO_strategy == "MPI-IO") call fatal_error ('pc_process', &
...
!
  if (lwrite_aux .and. .not. lread_aux) then
    if (lroot) then
      ...
      print *, ''
      call fatal_error("pc_process","Stop and check")
    endif
  endif
!
...
!
  ...
  if (alloc_err /= 0) call fatal_error ('pc_process', 'Failed to allocate memory for gf.', .true.)
  allocate (global_output (nxgrid,nygrid,nz,num_components), stat=alloc_err)
  if (alloc_err /= 0) call fatal_error ('pc_process', 'Failed to allocate memory for global quantities.', .true.)
!
...
!
  ...
  if (.not. ex) call fatal_error ('pc_process', 'File not found: '//trim(directory_dist)//'/'//filename, .true.)
  ...
  do pos = 1, num_quantities
    open (output_luns(pos), FILE=trim(directory_out)//'/'//trim(filename)//'_'//output_names(pos), status='replace', access='direct', recl=mxgrid*mygrid*io_len)
  enddo
!
...
!
        do imn = 1, ny*nz
          ...
          do pos = 1, num_quantities
            select case (pos)
              case 1:
                ! compute curl A manually:
                call curl (f, iax, tmp)
              case 2:
                ! get B from the pencils:
                tmp = p%bb
            endselect
            global_output(1+ipx*nx:nx+ipx*nx,m-nghost,n-nghost,global_start(pos):global_start(pos)-1+global_size(pos)) = tmp(:,1:global_size(pos))
        end
!
...
!
  ...
  close (lun_output)
  do pos = 1, num_quantities
    open (output_luns(pos), FILE=trim(directory_out)//'/'//trim(filename)//'_'//output_names(pos), FORM='unformatted', position='append', status='old')
