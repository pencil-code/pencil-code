...
__global__ void 
__launch_bounds__(RK_THREADS_PER_BLOCK, 5)
...
//printf("d_DT= %f \n", d_DT);
	...
	//------printing SMs vs Blocks-----------
	/*if(threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0 && blockIdx.z == 0){
		if(smid == 7){
			//printf("SM = %d consuming block(%d,%d,%d), %d\n", smid, v_blk_x, v_blk_y, blockIdx.z, d_sm_counter[smid] );
			printf("SM = %d consuming block(%d,%d,%d), %d\n", smid, blockIdx.x, blockIdx.y, blockIdx.z, d_sm_counter[smid] );
		}
	}*/
...
/*
	volatile float mom_x[BOUND_SIZE+1];
	volatile float mom_y[BOUND_SIZE+1];
	volatile float mom_z[BOUND_SIZE+1];
...
*/
	...
	for(int zplane = -3 ; zplane < RK_ELEMS_PER_THREAD_FIRST + 3; zplane++) {
		...
		//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			...
			/*const float d2xz_uu_z = der2_scalxz(sid_row, sid_col, s_uu_z, diagonal_result_xz_uuz);		
			const float d2yz_uu_z = der2_scalyz(sid_row, sid_col, s_uu_z, diagonal_result_yz_uuz);
			const float d2xz_uu_x = der2_scalxz(sid_row, sid_col, s_uu_x, diagonal_result_xz_uux);
			const float d2yz_uu_y = der2_scalyz(sid_row, sid_col, s_uu_y, diagonal_result_yz_uuy);*/
			...
			if(zplane >= 0 && zplane < RK_ELEMS_PER_THREAD_FIRST){
				...
				mom_y[(threadIdx.y*RK_THREADS_X)+threadIdx.x][0] = - (s_uu_x[sid_row][sid_col] * ddx_uu_y +               //vec_dot_nabla_scal
				...
				mom_z[(threadIdx.y*RK_THREADS_X)+threadIdx.x][0] = - (s_uu_x[sid_row][sid_col] * ddx_uu_z +               //vec_dot_nabla_scal
				...
				mom_x[(threadIdx.y*RK_THREADS_X)+threadIdx.x][3] = mom_x[(threadIdx.y*RK_THREADS_X)+threadIdx.x][2];
				mom_x[(threadIdx.y*RK_THREADS_X)+threadIdx.x][2] = mom_x[(threadIdx.y*RK_THREADS_X)+threadIdx.x][1];
				mom_x[(threadIdx.y*RK_THREADS_X)+threadIdx.x][1] = mom_x[(threadIdx.y*RK_THREADS_X)+threadIdx.x][0];
				
				mom_y[(threadIdx.y*RK_THREADS_X)+threadIdx.x][3] = mom_y[(threadIdx.y*RK_THREADS_X)+threadIdx.x][2];
				mom_y[(threadIdx.y*RK_THREADS_X)+threadIdx.x][2] = mom_y[(threadIdx.y*RK_THREADS_X)+threadIdx.x][1];
				mom_y[(threadIdx.y*RK_THREADS_X)+threadIdx.x][1] = mom_y[(threadIdx.y*RK_THREADS_X)+threadIdx.x][0];

				mom_z[(threadIdx.y*RK_THREADS_X)+threadIdx.x][3] = mom_z[(threadIdx.y*RK_THREADS_X)+threadIdx.x][2];
				mom_z[(threadIdx.y*RK_THREADS_X)+threadIdx.x][2] = mom_z[(threadIdx.y*RK_THREADS_X)+threadIdx.x][1];
				mom_z[(threadIdx.y*RK_THREADS_X)+threadIdx.x][1] = mom_z[(threadIdx.y*RK_THREADS_X)+threadIdx.x][0];
