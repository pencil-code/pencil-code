...
static __device__ real momentum(const int smem_idx, HydroStencil& stncl)
{
    ...
    //S_grad_lnrho  //Eq(.9)
    const real Sxx = (2.0f/3.0f)*ddx_uux - (1.0f/3.0f)*(ddy_uuy + ddz_uuz);
    const real Sxy = 0.5f*(ddy_uux + ddx_uuy);
    const real Sxz = 0.5f*(ddz_uux + ddx_uuz);
    const real Syy = (2.0f/3.0f)*ddy_uuy - (1.0f/3.0f)*(ddx_uux + ddz_uuz);
    const real Syz = 0.5f*(ddz_uuy + ddy_uuz);
    const real Szz = (2.0f/3.0f)*ddz_uuz - (1.0f/3.0f)*(ddx_uux + ddy_uuy);
    ...
    if (axis == X_AXIS) {
        ...
        const real res =   - stncl.r_uux[3] * ddx_uux //vec_dot_nabla_scal
                           ...
                           + 2.0f*d_NU_VISC*(Sxx*ddx_lnrho + Sxy*ddy_lnrho + Sxz*ddz_lnrho)
                           + d_NU_VISC*(1.0f/3.0f)*(d2x_uux + d2xy_uuy); //S_grad_lnrho
        return res;
    } else if (axis == Y_AXIS) {
        ...
        const real res =   - stncl.r_uux[3] * ddx_uuy //vec_dot_nabla_scal
                           ...
                           + d_NU_VISC*(1.0f/3.0f)*(d2xy_uux + d2y_uuy); //S_grad_lnrho
        return res;
    } else {
                        ...
                        const real dt)
{
    ...
    const int tz = threadIdx.z + blockIdx.z*blockDim.z*RK_ELEMS_PER_THREAD;//Start from bound zone
    ...
    for (int k=0; k < RK_ELEMS_PER_THREAD + 2*ZBOUND_SIZE; ++k) {
        ...
        if (k+BOUND_SIZE < RK_ELEMS_PER_THREAD + 2*ZBOUND_SIZE && tz + k + BOUND_SIZE < d_mz) {
        ...
        mom_x[0] -= d_NU_VISC*(1.0f/3.0f)*der2_scalxz<3>(smem_idx, s_uuz);
        mom_x[1] -= d_NU_VISC*(1.0f/3.0f)*der2_scalxz<2>(smem_idx, s_uuz);
        mom_x[2] -= d_NU_VISC*(1.0f/3.0f)*der2_scalxz<1>(smem_idx, s_uuz);
        mom_x[4] += d_NU_VISC*(1.0f/3.0f)*der2_scalxz<1>(smem_idx, s_uuz);
        mom_x[5] += d_NU_VISC*(1.0f/3.0f)*der2_scalxz<2>(smem_idx, s_uuz);
        mom_x[6]  = d_NU_VISC*(1.0f/3.0f)*der2_scalxz<3>(smem_idx, s_uuz);

        mom_y[0] -= d_NU_VISC*(1.0f/3.0f)*der2_scalyz<3>(smem_idx, s_uuz);
        mom_y[1] -= d_NU_VISC*(1.0f/3.0f)*der2_scalyz<2>(smem_idx, s_uuz);
        mom_y[2] -= d_NU_VISC*(1.0f/3.0f)*der2_scalyz<1>(smem_idx, s_uuz);
        mom_y[4] += d_NU_VISC*(1.0f/3.0f)*der2_scalyz<1>(smem_idx, s_uuz);
        mom_y[5] += d_NU_VISC*(1.0f/3.0f)*der2_scalyz<2>(smem_idx, s_uuz);
        mom_y[6]  = d_NU_VISC*(1.0f/3.0f)*der2_scalyz<3>(smem_idx, s_uuz);

        mom_z[0] -= d_NU_VISC*(1.0f/3.0f)*(der2_scalxz<3>(smem_idx, s_uux) + der2_scalyz<3>(smem_idx, s_uuy));
        mom_z[1] -= d_NU_VISC*(1.0f/3.0f)*(der2_scalxz<2>(smem_idx, s_uux) + der2_scalyz<2>(smem_idx, s_uuy));
        mom_z[2] -= d_NU_VISC*(1.0f/3.0f)*(der2_scalxz<1>(smem_idx, s_uux) + der2_scalyz<1>(smem_idx, s_uuy));
        mom_z[4] += d_NU_VISC*(1.0f/3.0f)*(der2_scalxz<1>(smem_idx, s_uux) + der2_scalyz<1>(smem_idx, s_uuy));
        mom_z[5] += d_NU_VISC*(1.0f/3.0f)*(der2_scalxz<2>(smem_idx, s_uux) + der2_scalyz<2>(smem_idx, s_uuy));
        mom_z[6]  = d_NU_VISC*(1.0f/3.0f)*(der2_scalxz<3>(smem_idx, s_uux) + der2_scalyz<3>(smem_idx, s_uuy));
...
template<int step_number>
static void rk3_step_cuda_generic(Grid* d_grid, Grid* d_grid_dst, const real dt, CParamConfig* cparams)
{
    ...
    const dim3 bpg   = {(unsigned int) ceil(cparams->nx / (real)(tpb.x)),
                        ...
                        (unsigned int) ceil(cparams->nz / (real)(tpb.z*RK_ELEMS_PER_THREAD))};

    //cudaDeviceSetSharedMemConfig(cudaSharedMemBankSizeEightByte);
    //cudaDeviceSetCacheConfig(cudaFuncCachePreferL1);

    cudaStream_t stream;
    //cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking);//Doesn't work, creates a race cond
                      ...
                      const int step_number, const real dt, CParamConfig* cparams)
{
    switch(step_number) {
        case 0:
            rk3_step_cuda_generic<0>(d_grid, d_grid_dst, dt, cparams);
            break;
        case 1:
            rk3_step_cuda_generic<1>(d_grid, d_grid_dst, dt, cparams);
            break;
        case 2:
            rk3_step_cuda_generic<2>(d_grid, d_grid_dst, dt, cparams);
            break;
        default :
