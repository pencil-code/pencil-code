...
!
      if (dtforce==0.) then
        ...
        c%aforce=sin(   pi*(tsforce-t)/dtforce)**2*dtforce_ampl+(1.-dtforce_ampl)
        c%pforce=cos(.5*pi*(tsforce-t)/dtforce)**2*c%aforce
        c%qforce=sin(.5*pi*(tsforce-t)/dtforce)**2*c%aforce
      endif
