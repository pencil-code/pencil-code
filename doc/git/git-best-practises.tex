...
\end{lstlisting}
because that
% [PB]: The SVN bridge also broke already between two consecutive SVN commits with no other commits interfering.
% [PB]: Commenting the misleading comments on the SVN bridge:
% breaks the SVN bridge and
...
and, voilà:
% [PB]: Commenting the misleading comments on the SVN bridge (see details above):
%after half year of disciplined commits by everybody, you
...
% [PB]: Commenting the misleading comments on the SVN bridge (see details above):
%Apart from avoiding problems with the SVN bridge, merging
