...
\begin{itemize}
\item Don't panic.
\item Don't pull from the remote branch into the tracking branch.
...
sense and formulate and format your log messages appropriately.\footnote{The first line of your commit message is a heading summarizing what
has happened.
The second line is traditionally kept empty, and more details can follow
on lines 3, 4, 5, etc. of the log message.}


\subsection{The different levels of a Git repository's state}
\label{sec-2-5}

There are (at least) four different variants of the \emph{Pencil Code} that
you are interacting with:
\begin{enumerate}
\item The upstream branch that you occasionally update by querying the
...
represent the last state of the server branch you \emph{fetched}.
\item Your tracking branch.
This is where [\ldots{}]
\item The \emph{index} where changes are recorded before they get committed to
your local branches.
\item The file system.
\end{enumerate}

[Many possibilities for discord, but not that bad\ldots{}]
...
\end{verbatim}
Immediately after this, both \emph{master} and \emph{feature} point to the same
...
In terms of narrating, \emph{rebasing} suggests letter style, where you
...
\bottomrule
\end{tabular}
\end{center}\footnotetext[3]{Less safe in the sense that conflicts can put you in a
detached-head state.}
...
applying a change G' in the first place.\footnote{But of course this is not true: you can use \texttt{‘git reflog’} and
...
of mind (i.e. \emph{before} you test your changes, because after testing
...
branch (\emph{detached head}).
This is not really something to worry about:
Just fix the conflicts and do '\texttt{git rebase -{}-continue}' to finish the
rebase; and in case you get lost, do '\texttt{git rebase -{}-abort}' and start
afresh.
Even if you get completely lost and resort to measures like
'\texttt{git reset}', \texttt{you needn't be afraid} to lose history.
...
Git will try hard to preserve your changes:\footnote{There are corner cases where other git commands (like
...
\item Files unknown to Git will only get lost with
\begin{itemize}
...
\bottomrule
\end{tabular}
\end{table}\footnotetext[8]{Leaving important Git commits dangling (≈ unused) for more than two
...
push, you will get an error message\footnote{My notation in the graph is adopted from
...
either do this \hyperref[Rebase]{using rebase}, or \hyperref[Merge]{using merge}.
...
git pull --rebase
\end{lstlisting}
This is equivalent to \verb~git fetch; git rebase origin/master~, so this is
...
See also \href{https://asciinema.org/a/dauj562l4uwr7bpyohqyewkj5}{Tobi's screencast} where he demonstrates a variant of that
approach.
...
git push
\end{lstlisting}
(possibly in connection with \verb~rebase.autostash=true~),
or maybe
\lstset{frame=single,basicstyle=\ttfamily\footnotesize,language=sh,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
It is tempting to just call (after the \texttt{git fetch})
\lstset{frame=single,basicstyle=\ttfamily\footnotesize,language=sh,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
git merge origin/master  # DON'T DO THIS
\end{lstlisting}
...
\includegraphics[width=.98\linewidth]{git-wrong-history.pdf}
...
\includegraphics[width=.98\linewidth]{git-better-history.pdf}
...
\emph{
When developing on \emph{master}, if you cannot directly push your changes
due to upstream commits, either use \emph{rebase}, or use \emph{git pc
reverse-merge}.
}
