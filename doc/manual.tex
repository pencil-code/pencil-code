...
%%  CVS: $Id: manual.tex,v 1.365 2007-08-18 08:42:41 brandenb Exp $
%%  Description:
...
%\date{$ $Date: 2007-08-18 08:42:41 $ $,~ $ $Revision: 1.365 $ $}
...
\begin{titlepage}
  ...
  \emph{$ $Date: 2007-08-18 08:42:41 $ $,~ $ $Revision: 1.365 $ $}\\
...
rise in the number of sample problems (Fig.~\ref{pfile-size}).
In order to keep the development of the code going, it it important
that the users are able to understand and modify the code.
...
\begin{figure}[htb]
  ...
  since the end of 2001. The glitch in the Summer of 2005 was the
...
We quickly arrive at the following piece of code,
\begin{verbatim}
!
!  continuity equation
!
...
equation should indeed be solved and whether we do indeed want to
...
Resetting is only done by the timestepping routine.
Next, the pieces \code{p\%ugrho} and \code{p\%rho*p\%divu} are being
subtracted.
These are \emph{pencils}, which are organized in the \emph{structure}
...
For example \code{p\%ugrho} is calculated in \file{density.f90}; see
\begin{verbatim}
          call u_dot_grad(f,ilnrho,p%grho,p%uu,p%ugrho,UPWIND=lupw_rho)
\end{verbatim}
So this is a special subroutine to calculate the $\uv\cdot\nabla$ operator,
...
in the \file{magnetic.f90} file. The right hand side of
...
We leave it with this and encourage the user to do similar
inspection work on an number of other examples.
...
\section*{APPENDIX
  \hfill\normalsize\rm$ $Date: 2007-08-18 08:42:41 $ $,~ $ $Revision: 1.365 $ $}
...
$ $Id: manual.tex,v 1.365 2007-08-18 08:42:41 brandenb Exp $ $}
