...
Copyright \copyright{} 2001--2014 Wolfgang Dobler \& Axel Brandenburg
\bigskip
...
You may compile the code using a default compiler-specific configuration file:
\begin{enumerate}
\item Single-processor using the GNU compiler collection: \code{pc_build -f GNU-GCC}
\item Multi-processor using GNU with MPI support: \code{pc_build -f GNU-GCC_MPI}
\end{enumerate}
Many compilers are supported already, see available config files in \file{\$PENCIL_HOME/config/compilers/*.conf}.
For example you will also find there configuration files for \code{Intel} and \code{Intel_MPI}.

If you have to set up something specific to a certain host system you work on,
...
you can clone an existing host-configuration file, include there an existing
compiler configuration, and simply only apply the changes you need.
A good example of a host-configuration file is \file{\$PENCIL_HOME/config/hosts/IWF/host-andromeda-GNU_Linux-Linux.conf},
you may clone this, change it according to your needs, and save it unter \file{\$PENCIL_HOME/config/hosts/<ID>.conf},
...
\item Install the matplotlib library (a package often called
  \name{python-matplotlib}), which should install \name{numpy} as a
  dependency.\\
  You may also want to install ipython.

\item Initialize you PYTHONPATH environment variable by sourcing 'sourceme.*'.

\item Load all of the Python pencil modules using 'import pencil as pc'

\item Load data using 'pc.read_ts()', etc.
...
\begin{alltt}
  \prompt{unix> } ipython -pylab
\end{alltt}

\begin{alltt}
  \prompt{In [1]: } import pencil as pc
  \prompt{In [2]: } ts = pc.read_ts()
\end{alltt}

\noindent While in IDL you would would type ``print,tag_names(ts)'' in
the prompt to examine the structure, in python you type
``ts.'' and  hit tab to see what is inside the class.

\begin{alltt}
  \prompt{In [3]: } ts.
  ts.dt      ts.plot    ts.rhomax  ts.t       ts.xpar2   ts.ypar2
  ts.it      ts.rhom    ts.rhomin  ts.xpar1   ts.ypar1
\end{alltt}

Next, for plotting, type, for instance:

\begin{alltt}
\prompt{In [4]: } plot(ts.t,ts.rhomax)
\end{alltt}
...
$x$ parallelization is ppossible.
...
but many of the currently 98 registered people on the repository have
...
This $h$ index is currently 28, i.e., 28 users have done at least
28 check ins; see Figure~\ref{pencil-hindex}.
...
1152&Beskow&0.0028&$1152^3$&magn/noentro&1x32x36&20-mar-15& AB-gnu \\
...
\begin{figure}[htb]
  ...
  \caption{Scaling results on Kraken at fixed load per processor, for a magnetized disk model in cylindrical coordinates.
    The figure shows, after determing that 16$^3$ is the best load per processor for strong scaling, how far one can push
...
hyperdiffuvity is an important factor in the choice of this scheme.

To decide whether the effective hyperdiffuvity from the diffusive
