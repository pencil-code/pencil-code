...
Some clusters use Lua modules\footnote{
\url{https://lmod.readthedocs.io/en/latest/index.html}
} instead of the usual Tcl modules;\footnote{
\url{https://modules.readthedocs.io/en/stable/modulefile.html}
} in that case, the syntax for the module file (the name of which must end in \code{.lua}) is
\begin{verbatim}
whatis("Environment setup for the Pencil code")

-- change the following line according to the location of your local copy of Pencil
setenv("PENCIL_HOME", os.getenv("HOME") .. "/.software/pencil-local")

setenv("_sourceme_quiet", "1")
source_sh("bash", os.getenv("PENCIL_HOME") .. "/sourceme.sh")
unsetenv("_sourceme_quiet")
\end{verbatim}
...
\begin{verbatim}
  cmake version >= 3.23,bison,flex,CUDA or HIP,python
\end{verbatim}
...
Note that the mpicc compiler work poorly with CMake, thus it is preferable to have
set the CC compiler explicity in your compilation config something different from mpicc.

By default the code assumes communication is done with CUDA-aware MPI, which means the GPUs speak directly to each other.
If you have to ask is your MPI CUDA-aware enabled then it most likely is not.\footnote{
If Openmpi has been built with CUDA support, you will see \code{cuda} in the output of \command{ompi\_info | grep "MPI extensions"}.
}
Thus trying to run the code in CUDA-aware mode will most likely cause a segmentation fault and the code to crash. 

To turn off CUDA-aware MPI add the following to your build config
\begin{verbatim}
  CUDA_AWARE_MPI=OFF
\end{verbatim}
.
...
All of these settings are incorporated inside \cmd{pc_run}, so when using it
there is no difference in running the GPU version, except the slurm settings.
...
For Fourier transform by default the vendor versions are used (cufft or rocfft). However, these are not usually configured to be MPI-enabled. Because of this one has to install the HeFFTe library for
distributed Fourier transforms on the GPUs. At the moment these are needed for advancement of gravitational waves in the hTXk-module and for solving the Poisson equation in periodic domains.
In `utils/install\_scripts/install\_HeFFTe\_LUMI.sh` you can find an example script for installing HeFFTe for rocm-enabled environment.
In addition to the installation also add the following to your build config
\begin{verbatim}
  GPU_FFT=HEFFTE
\end{verbatim}, to make the build choose HeFFTe.
If you installed HeFFTe instead of using an existing module for it, you have to also export the environment variable HEFFTE\_DIR, which should point to a directory that contains the `include` folder of HeFFTe and the built library under `build`.

\subsubsection{prep rhs subroutines}
Often time-dependent variables are calculated in before or after boundary.
To avoid excessive loading of parameters to the GPUs, we would prefer these calculations to be in prep\_rhs functions.
The functionality will be the same on CPUs (not calculated inside the mn-loop), but on GPUs it is calculated on each thread.
Since compute is cheap compared to loading of variables, this is preferable, and more importantly less likely to break because of missing load statements.

\subsubsection{Runtime compilation}
\label{runtime-compilation}
As stated normal usage of GPUs requires compilation during the running of the setup.
This is because the code as is with its large number of conditionals would not be performant on the GPUs because for performance Astaroth needs to know at compile-time which stencils are used. Compiling after the parameters have been chosen fixes the problem. 

However it is inconvenient if you want to run a spectrum of runs while varying some numerical parameters if each choice requires separate runtime compilation. Because of this it is possible to tell via code comments does a parameter need recompilation or not.
The relevant subroutines are called \code{pushpars2c}. The following has examples for boolean and real variables where the former would force recompilation and the latter not.
\begin{verbatim}
  call copy_addr(lklein_gordon,p_par(1)) ! bool
  call copy_addr(wfreeze,p_par(2))
  call copy_addr(lklein_gordon,p_par(1)) ! bool dconst
  call copy_addr(wfreeze,p_par(2)) ! real dconst
\end{verbatim}

If control-flow (if statements/other conditionals) depends on a variable it should trigger recompilation so Astaroth can optimise the code.
If a variable does not affect control-flow it is often preferable to have the variables not trigger recompilation and it would be helpful to add these cases to the code. So, whenever you are annoyed by the constant need of recompilation because of a certain variable, please do change the variable to not need the recompilation if you have made sure it does not affect control-flow (and is not likely to do so in the future).
\subsubsection{Performance tips}
To hide the cost of communication as much as possible, it is preferable to have as large subdomains as possible.
Commonly used subdomain (= per-process) sizes for accelerated setups are of the order of $256^3$ or bigger.
Another aspect worth considering is the cadence of the diagnostic outputs.
Since the diagnostics are performed on the CPUs each diagnostic output needs to copy the mesh from the device, which is on most hardware is quite slow, and has to compute the diagnostics themselves on the slower hardware of CPUs and/or perform I/O, which is even slower. Thus an overly small cadence of diagnostic output will prevent any speedup. With a high enough cadence the diagnostics are computed/stored alongside the time integration and the time required for them is completely hidden.
This is especially doing for power spectra, which requires a lot of communication and thus cannot be totally hidden since it competes with the time integration for access to the network.
% ---------------------------------------------------------------------- %
...
\begin{verbatim}
TIMESTEP  =   timestep_rkf_lowsto
\end{verbatim}
or
\begin{verbatim}
TIMESTEP  =   timestep_rkf
\end{verbatim}
in \file{src/Makefile.local}. The low-storage version is more up to date and the preferred one.
...
We start from the general induction equation for the 
vector potential,
...
where $\Av$ is the magnetic vector potential\index{Vector potential},
$\Bv = \curl\Av$ the magnetic
flux density, $\jv = \mu_0^{-1}\boldsymbol{\nabla}\times\Bv$
the current density, $\Phi$ is the scalar potential,
$\eta = 1/(\mu_0\sigma)$ is the magnetic diffusivity
($\sigma$ denoting the electrical conductivity), and $\mu_0$ the
magnetic vacuum permeability.

In the Weyl gauge, $\Phi=0$, and the induction equation reduces to
\begin{equation}
  \frac{\partial\Av}{\partial t}
  = \uv\times\Bv - \eta\mu_0\jv \; .
\label{dAdt}
...
However, we do not normally use the Weyl gauge, but rather the resistive gauge,
\begin{equation}
  \frac{\partial\Av}{\partial t}
  = \uv\times\Bv + \eta\nabla^2\Av \; .
\label{resistive-gauge}
\end{equation}
In the \name{resistive gauge}, we have $\Phi=-\eta\nab\cdot\Av$.
It has the advantage that the terms $A_{x,xx}$, $A_{y,yy}$, and $A_{z,zz}$
are retained, providing direct diffusion of the corresponding components
of the vector potential and thereby suppressing small-scale numerical
oscillations. Although the longitudinal derivatives $A_{x,x}$, $A_{y,y}$,
and $A_{z,z}$ do not contribute directly to $\Bv = \boldsymbol{\nabla}\times\Av$,
large values of these terms can adversely affect the numerical solution.
...
is proportional to the positive flow convergence, maximum over five zones, and smoothed to second order,
\begin{equation}
...
$2\langle\Strain^2\rangle=\langle\omv^2\rangle+{4\over3}\langle(\nab\cdot\uv)^2\rangle$.\footnote{
More generally, we have $2\SSSS^2=-\nab\cdot\FFF^{\omega}-\ov^2+\frac{4}{3}(\nab\cdot\uv)^2$,
where $\FFF^{\omega}=\ov\times\uv-\frac{4}{3}\uv\,\nab\cdot\uv$ is proportional to a viscous flux term.
The 4/3 factor in front of the $\langle(\nab\cdot\uv)^2\rangle$ contribution to the heating was incorrectly given as 2/3 in Ref.~\cite{BJNRST96}.}
