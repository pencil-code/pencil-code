...
\begin{alltt}
  ...
  \prompt{IDL> }  .r pc_gui
\end{alltt}
If you like to load only 2D-slices from the given data snapshots,
instead of using previously written video files (see \ref{S-slices}),
you can also type:
\begin{alltt}
  \prompt{unix> } idl
  \prompt{IDL> }  cut_z = 27
  \prompt{IDL> }  .r pc_gui
\end{alltt}
The Pencil Code GUI offers many more options to be set on the command-line,
please refer to the description in the source code.
...
as the velocity of the particles are equal to the instantanious fluid velocity
...
All particle infrastructure is controlled and organised by the
...
\begin{enumerate}
  ...
  shared with neighbouring processors (including neighbours over periodic
  ...
  between the processors. One can then try to change the parallelisation
...
the processors. One can as a first approach attempt to not parallelise
...
\begin{enumerate}
  ...
  to the nearest grid point, but also to the neighbouring grid points.
...
neeed the option \option{-qextname}.
