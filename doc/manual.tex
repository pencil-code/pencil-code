...
The Astaroth library and the code translator to it come together with Pencil Code as git submodules.
...
Then the compilation and running is done the same as for normal CPU setups.
By default the code assumes communication is done with CUDA-aware MPI, which means the GPUs speak directly to each other.
If you have to ask is your MPI CUDA-aware enabled then it most like is not.
...
Additionally if you are using CUDA-aware MPI with MPICH (for example on the LUMI supercomputer) but the following export before calling run.x but after calling start.x
\begin{verbatim}
...
This is most predominant when regarding the particles either in the Particle-in-Cell (PIC) method or with the N-body solver of Pencil Code, neither of which are GPU accelerated. Additionally more complicated computations outside of the rhss are not included like the Navier Stokes Characteristic Boundary Condition (NSCBC) and granulation calculations in the coronae module. To see which of the commited samples are known to run on the GPUs check the list gpu\_test in \file{bin/pc_autotest}. As a rule of thumb if the sample only requires the following features it can be run on GPUs:
\begin{verbatim}
integration of PDEs and ODEs. 
Boundary conditions that can be formulated as local operations. (For not yet covered boundary conditions requires explicit use of code tools to generate the GPU code)
Calculations of averages or other reductions before time integration. (For not yet covered cases requires explicit use of code tools to generate the GPU code).
Fourier transforms (Requires handwritten modifications to add new physics)
\end{verbatim}
% ---------------------------------------------------------------------- %
