...
%%  CVS: $Id: manual.tex,v 1.265 2004-07-17 16:25:28 brandenb Exp $
%%  Description:
...
%\date{$ $Date: 2004-07-17 16:25:28 $ $,~ $ $Revision: 1.265 $ $}
...
\begin{titlepage}
  ...
  \emph{$ $Date: 2004-07-17 16:25:28 $ $,~ $ $Revision: 1.265 $ $}\\
  \url{http://www.nordita.dk/pencil-code/}
...
Contributors to the code (in inverse alphabetical order according to
their user name):\\

  vpar\hfill\htmladdnormallink{Vladimir Pariev}{http://www.pas.rochester.edu/~vpariev/}\\
  tork\hfill\htmladdnormallink{Ulf Torkelsson}{http://fy.chalmers.se/~torkel/}\\
  thei\hfill\htmladdnormallink{Tobias (Tobi) Heinemann}{http://www.nordita.dk/~theine/}\\
  tare\hfill\htmladdnormallink{Tarek A. Yousef}{http://www.pvv.ntnu.no/~tarek/}\\
  snod\hfill\htmladdnormallink{Andrew Snodin}{http://www.ncl.ac.uk/math/postgrad/postgrads.htm}\\
  pkap\hfill\htmladdnormallink{Petri K\"apyl\"a}{http://cc.oulu.fi/~pkapyla/}\\
  nils\hfill\htmladdnormallink{Nils Erland L. Haugen}{http://www.phys.ntnu.no/~nilshau/index2.html}\\
  ngrs\hfill\htmladdnormallink{Graeme R. Sarson}{http://www.mas.ncl.ac.uk/~ngrs/home.html}\\
  mee \hfill\htmladdnormallink{Antony (tOnY) Mee}{http://www.mas.ncl.ac.uk/~n7026413/pencil-code/movies/}\\
  mcmi\hfill\htmladdnormallink{David McMillan}{http://www.mas.ncl.ac.uk/~ndgm5/}\\
  matt\hfill\htmladdnormallink{Mattias Christensson}{http://www.nordita.dk/~mattias/}\\
  dorc\hfill\htmladdnormallink{Bertil Dorch}{http://www.astro.ku.dk/~dorch/}\\
  dobl\hfill\htmladdnormallink{Wolfgang Dobler}{http://www.kis.uni-freiburg.de/~dobler/}\\
  chri\hfill\htmladdnormallink{Christer Sandin}{http://www.astro.uu.se/~christer/CS_index.html}\\
  bran\hfill\htmladdnormallink{Axel Brandenburg}{http://www.nordita.dk/~brandenb/}\\
  bing\hfill\htmladdnormallink{Sven Bingert}{http://www.kis.uni-freiburg.de/~peter/corona/}\\
  amje\hfill\htmladdnormallink{Amjed Mohammed}{http://ehf.uni-oldenburg.de/member.php?nav=staff&sprache=english&show=43}\\
  ajoh\hfill\htmladdnormallink{Anders Johansen}{http://www.astro.ku.dk/~ajohan/}\\
...
(\file[.cshrc]{\~{}/.cshrc} or \file[.bashrc]{\~{}/.bashrc}, bcause it
outputs a few lines of diagnostics for each subshell, which will break
many applications.
...
So far, one can choose between \code{grid_function='sinh'} and
...
\end{equation}
where $z_0$ and $z_0+Lz$ are the lowest and uppermost levels, 
...
To apply that profile, you can set the following in \file{start.in}:
\begin{Verbatim}
  &init_pars
    [...]
    lequidist = T, T, F         ! non-equidistant grid in z
    coef_grid = , 0.05          ! (z_*, a) -- note the comma!
    grid_func = 'sinh'          ! sinh function: linear for small, but
                                ! exponential for large z
    ! zeta_grid0 = 0.           ! old and soon to be removed
  /
\end{Verbatim}
The parameter array \var{coef_grid} represents $z_*$ and $a$; the bottom
height $z_0$ and the total box height $L_z$ are taken from \var{xyz0} and
\var{Lxyz} as in the equidistant case.
If you want the inflection point of the $\sinh$ profile (the part where it
is linear) at a non-zero height, you can set \var[coef_grid]{coef_grid(1)}
(i.\,e.~$z_*$ to a value different from $z_0$.

%
\paragraph{Alternative formulation:}
Equations~(\ref{Eq-timeavg-exp-alg}) and (\ref{Eq-timeavg-exp-int}) take a
somewhat more compact form when working with the inverse function
$\zeta(z)$:
\begin{eqnarray} \label{Eq-nonequi-fprime-zeta-z}
...
The code treats gas and dust as two separate fluids. The dust and the gas
...
{\em
  ...
  Make sure you have tha latest patches applied to the compiler.
...
\url{http://www.nordita.dk/pencil-code/tests.html}.
\begin{Verbatim}
...
%% [wd: I understand this is extremely annoying, but this has no place
%% in a user manual]
%% {\em
...
> have introduced it, right? Pretty irritating..
\end{Verbatim}
...
{\em
  ...
  yes! My experience of some 5 days ago was quite enormous. It started
...
{\bf A}:
{\em
The sys admins must have changed something
I guess because LANG=en_US.UTF-8 now for some reason.
Put \cmd{LANG=POSIX}.
}
...
\section*{APPENDIX
  \hfill\normalsize\rm$ $Date: 2004-07-17 16:25:28 $ $,~ $ $Revision: 1.265 $ $}
...
%    \begin{tabular}{rllrlrll}
  ...
  16 & giga  & 0.47 & $128^3$ & chiral       & 4x4/ifc& 29-may-04 & AB \\
...
\var{ny}$\times$\var{nz}=$2\times1$ CPUs) that you want to
...
\item Create another run directory with current \file{VAR1} as
  ...
  \prompt{unix> } reprodir -l ../run_64_restart {\sl or } new run_64_restart
  \prompt{unix> } (cd ../run_64_restart/data ; mkproc-tree 2)
  \prompt{unix> } restart-new-dir-VAR ../run_64_restart 1
  \end{alltt}

\item Create the new run directory:
  \begin{alltt}
  \prompt{unix> } cd ../run_64_restart
  \prompt{unix> } reprodir -l ../run_128 {\sl or } new run_128
  \prompt{unix> } vi ../run_128/src/cparam.local
  {\sl{}# set nxgrid=128, ncpus=16, nprocy=4}
  \prompt{unix> } (cd ../run_128; crtmp; setup-src; make)
\end{alltt}

\item Setup and do remeshing
  \begin{alltt}
  \prompt{unix> } setup-remesh
  \prompt{unix> } (cd remesh; make)
  \prompt{unix> } vi remesh/common.local
  {\sl{}# set muly=2, mulz=4, remesh_par=2}
  \prompt{unix> } vi remesh.in
  {\sl{}# Replace line by ../run_128}
  \prompt{unix> } remesh.csh
  {\sl{}# Answer `yes'}  
  \end{alltt}
\end{enumerate}
...
\subsection{Upwind derivatives and relation to boundary conditions}
\index{Upwind}
\label{S-upwind}
\newcommand{\Order}[1]{O\left(#1\right)}

%AB: Wolfgang, this is a cut+paste version of your text
%AB: in kompendien/numerik/FiniteDiff.tex
%AB: If your checked-in version was not up-to-date, you can overwrite this.

The ``wiggles'' in $\ln\varrho$ in (high-order centred-difference)
convection simulations are apparently caused by a velocity profile where
the velocity approaches zero at the boundary.
This causes the density profile to be squeezed into a boundary layer where
eventually numerical resolution is insufficient and, for centred
differences, a spurious Nyquist signal is generated that almost
instantaneously propagates into the whole box.

This behaviour is hardly influenced by the boundary conditions on
$\ln\varrho$ at all.
...
\end{equation}
the fourth-order upwind scheme would be
\begin{equation}
  f'_0
...
spreading away from the boundary.
...
\begin{equation}
  ...
  = \frac{-f_{-3} + 6 f_{-2} - 15 f_{-1} + 20 f_{0}
          - 15 f_{1} + 6 f_{2} - f_{3}}
         {60\,\delta x}
  = -\frac{\delta x^5}{60} f^{(6)}_{\rm 2nd} \; .
\end{equation}
...
\midrule
  ...
  \var{rumax} & $\max(\varrho \uv)$ \quad(maximum modulus of momentum) \\
%
\midrule
...
$ $Id: manual.tex,v 1.265 2004-07-17 16:25:28 brandenb Exp $ $}
...
% LocalWords:  phiaver pE pF datafiles PNG MPEG mpeg Secs mphi chiral
