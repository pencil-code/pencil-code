...
%%  CVS: $Id: manual.tex,v 1.226 2004-01-09 15:29:37 dobler Exp $
%%  Description:
...
%\date{$ $Date: 2004-01-09 15:29:37 $ $,~ $ $Revision: 1.226 $ $}
...
\begin{titlepage}
  ...
  \emph{$ $Date: 2004-01-09 15:29:37 $ $,~ $ $Revision: 1.226 $ $}\\
...
\name[Fortran record]{Fortran records} (a \name{Fortran record} is marked
by the 4-byte integer byte count of the data in the record at the
beginning and the end, i.e.~has the form
...
\name{Video files} and \name{slice files} are used interchangeably.
The output interval is given by \var{dvid} in code time units.
The quantities to be written should be entered into the file \file{video.in}.
The slice position is set in \name{run_pars} and can be either
\var{slice_position='p'} (default, for periphery) or
\var{slice_position='m'} (for middle of the box).
Afterwards, one runs \file{src/read_videofiles.x} interactively
in the run directory. This procedure assembles the right files from
the different processors from \file{data/proc}$N$/ and puts them
into \file{data/}.
Note: remember to delete unused slices \cmd{rm data/proc*/slice*}.
For visualization, you can use the idl procedure, e.g.\ as
\file{rvid_box,'lnrho',amin=-.1,amax=.1}.
It accepts the \file{/png} argument for writing png files for making videos.
...
\section*{APPENDIX
  \hfill\normalsize\rm$ $Date: 2004-01-09 15:29:37 $ $,~ $ $Revision: 1.226 $ $}
...
\subsection{List of startup parameters for \file{start.in}}
...
\subsection{List of runtime parameters for \file{run.in}}
...
\subsection{List of parameters for \file{print.in}}
...
\midrule
  ...
  \var{dtu}   & advective time step
                $c_{\delta t}\,\delta x/\max|\mathbf{u}|$
                (see \S~\ref{time-step})\\
  \var{dtb}   & Alfv{\'e}n time step
                $c_{\delta t}\,\delta x/v_{\rm A,max}$
                (see \S~\ref{time-step})\\
  \var{dteta} & resistive time step
                $c_{\delta t,{\rm v}}\,
                 \delta x^2/\eta_{\rm max}$
                (see \S~\ref{time-step})\\
  \var{dtchi} & time step based on heat conductivity
                $c_{\delta t,{\rm v}}\,
                 \delta x^2/\chi_{\rm max}$
                (see \S~\ref{time-step})\\
  \var{dtnu}  & viscous time step
                $c_{\delta t,{\rm v}}\,
                 \delta x^2/\nu_{\rm max}$
                (see \S~\ref{time-step})\\
...
\subsection{List of parameters for \file{phiaver.in}}\
...
$ $Id: manual.tex,v 1.226 2004-01-09 15:29:37 dobler Exp $ $}
...
% LocalWords:  Milano IECR ILNTT NOERASE checkin cvsstat pkeff
