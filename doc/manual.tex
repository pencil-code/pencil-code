...
%%  CVS: $Id: manual.tex,v 1.210 2003-11-20 11:59:17 ajohan Exp $
%%  Description:
...
%\date{$ $Date: 2003-11-20 11:59:17 $ $,~ $ $Revision: 1.210 $ $}
...
\begin{titlepage}
  ...
  \emph{$ $Date: 2003-11-20 11:59:17 $ $,~ $ $Revision: 1.210 $ $}\\
...
\subsubsection{Problems with the \code{ifc} compiler}

Leonardo J. Milano (\url{http://udel.edu/~lmilano/}) asks:
``What f90 compilers are you guys using ? 
gcc-g95 is not there yet, and there are a bunch of commercial
options. I wonder which ones are known to work well.''

{\it On Linux machines the ifc compiler from intel is the fastest, but
version 7.1 has a problem, so try to get 7.0 or earlier.}

Leo provided the following useful info:
\begin{Verbatim}
1) I had to download an older version (ifc 7.0), as Axel said.
   I found it at:
   ftp://download.intel.com/software/products/compilers/downloads

2) I needed this trick from Wolfgang:

> use
>   FC=ifc -i_dynamic
> for compiling and setting the environment variable
...
4) When using ifc 7.1 version 7.1 build 008, there is an
   * interal compiler error * while compiling the "hydro" module.
   The compilation goes well up to this point (in the turbulence
   sample for instance). I guess this is the known issue Axel mentioned.
   I hope a newer ifc will fix the issue, so we don't have to stick
   to an old version !
\end{Verbatim}
...
anoying error messages that one gets, when running e.g. ".r rall" after
...
\section*{APPENDIX
  \hfill\normalsize\rm$ $Date: 2003-11-20 11:59:17 $ $,~ $ $Revision: 1.210 $ $}
...
\midrule
  ...
  \var{bcz} [(\code{'p'}, \code{'p'}, \ldots)]
               & boundary conditions (for \file{start.csh}). 
                 \\
%
\midrule
  \multicolumn{2}{c}{Namelist \name{hydro_init_pars}} \\
...
$ $Id: manual.tex,v 1.210 2003-11-20 11:59:17 ajohan Exp $ $}
...
% LocalWords:  Raphson trans Ylm rrr Coord diag Stegun acroread cgs
% LocalWords:  visc walltime ekintot ethtot sedov Abramowitz Harri GNG
% LocalWords:  BJNRST Ferri
