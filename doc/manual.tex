...
%%%
\subsubsection{Compilation fails on MacOS Sonoma}
...
While running pc_build, I get an error message saying that \texttt{-ld\_classic} is not found.

{\bf A}:
Note that \texttt{ld\_classic} is \emph{not} specifying a library called \texttt{libd\_classic}, but is rather an option passed to Apple's \texttt{ld} binary.
\cmd{type ld} should say \texttt{/usr/bin/ld}.
...
\end{Verbatim}
when the commiting author is not Philippe?
