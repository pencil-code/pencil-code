...
%%  CVS: $Id: manual.tex,v 1.322 2006-02-22 19:34:05 mee Exp $
%%  Description:
...
%\date{$ $Date: 2006-02-22 19:34:05 $ $,~ $ $Revision: 1.322 $ $}
...
\begin{titlepage}
  ...
  \emph{$ $Date: 2006-02-22 19:34:05 $ $,~ $ $Revision: 1.322 $ $}\\
...
Copyright \copyright{} 2001--2005 Wolfgang Dobler \& Axel Brandenburg
\bigskip
...
Heinemann, T., Dobler, W., Nordlund, \AA., \& Brandenburg, A.\pana{2005}
{Radiative transfer in decomposed domains}
(\astroph{0503510})

Dobler, W., Stix, M., \& Brandenburg, A.\papj{2005}{683}
...
Johansen, A., Henning, Th., \& Klahr, H.\papj{2006}{643}
{Dust sedimentation and self-sustained Kelvin-Helmholtz turbulence in
protoplanetary disc mid-planes}{01 June 2006}
(\astroph{0512272})
...
specifications of the initial condition. Here is a sample of the
particle part \code{run.in} (also for dust particles):
\begin{verbatim}
/
&particles_run_pars
  ldragforce_gas=T
...
/
\end{verbatim}
The logical \code{ldragforce_gas} determines whether the dust particles
...
used to acquire this velocity outside the grid points.
...
gas).
\begin{equation}
  \frac{\dd \vec{v}_i}{\partial t} = \ldots -\frac{1}{\tau_{\rm s}}
  (\vec{v}_i-\vec{u}) \, .
\end{equation}
...
\begin{equation}
  \frac{\partial \vec{u}}{\partial t} = \ldots - \frac{\epsilon_{\rm
  d}}{\tau_{\rm f}} (\vec{u}-\vec{v}) \, .
\end{equation}
Here $\vec{v}$ is the average velocity of the particles in the grid cell that
is considered. The dust-to-gas ratio $\epsilon_{\rm d}=0.01$ is calculated by
counting the number of particles in the grid cell and multiplying by a fixed
mass density $\tilde{\rho_{\rm p}}$ that each particle represents. This is
again set by the global dust-to-gas ratio \cmd{eps_dtog} in
\cmd{particle_init_pars}.
...
of the source code is of the uttmost importance.  Well layed out
...
\item code in any delimited block eg. if statments, do loops, subroutines 
etc. is indented by precisely 2 spaces.
...
The centralised pencil calculation scheme is a guarantee that
\begin{itemize}
...
\begin{itemize}
  ...
  \item Request the new pencil based on the input parameters in any relevant
      physics module
\end{itemize}
Remember that the centralised pencilation scheme is partially there to force
...
\section*{APPENDIX
  \hfill\normalsize\rm$ $Date: 2006-02-22 19:34:05 $ $,~ $ $Revision: 1.322 $ $}
...
begine with the Fortran module name in all lowercase.  Where there
...
Fortarn is not case-sensitive but in almost all instances we prescribe
...
Comments should be written in sentetences using the usual capitalization 
...
of the Pencil Code: `f' the main state array and `p' the pencil case
...
character (usually in lowercase) eg. the velocity $u$ becomes `uu', 
...
since Fortran is not case sensitive the variables for example `iTT' and
...
\item idiag_ -- Denotes diganostic indices.
...
$ $Id: manual.tex,v 1.322 2006-02-22 19:34:05 mee Exp $ $}
