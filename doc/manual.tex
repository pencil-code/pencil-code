...
%%  CVS: $Id: manual.tex,v 1.99 2002-09-20 07:06:25 brandenb Exp $
%%  Description:
...
\newcommand{\env}[1]{\code{#1}\index[var]{#1@\emph{#1}}}
...
A set of test problems (listed in \file{\$PENCIL_HOME/bin/auto-test})
...
\cmd{setenv CVSROOT :pserver:\$USER@norserv.nordita.dk:/home/brandenb/CVS},
...
\item Set your environment variable \env{CVSROOT} (you probably want to
  ...
  \prompt{csh> } setenv CVSROOT \bs
          :pserver:anonymous@norserv.nordita.dk:/home/cvs/CVS \
  \end{alltt}
  or (if you are running Bourne shell or bash)
  \begin{alltt}
  \prompt{sh> } CVSROOT=:pserver:anonymous@norserv.nordita.dk:/home/cvs/CVS
...
\item Go to the top directory of the code:
  \begin{alltt}
  \prompt{unix> } cd \file{somewhere/pencil-code} \
  \end{alltt}
...
\begin{alltt}
  ...
  \prompt{unix> } setup-src \quad\#(from \file{../../../bin}, which by now should be in your path)
  \prompt{unix> } cd src
  \prompt{unix> } make
  \prompt{unix> } cd ..
...
individual run directory, like e.g.~\file{samples/conv-slab/src}.
...
are running the code you may want to check in out in your \file{pencil-code}
directory. In our case we have our own runs in a repository called
\file{pencil-runs}, so we say
\begin{alltt}
  \prompt{brandenb@fe1:/home/brandenb> } cd \$PENCIL_HOME
  \prompt{brandenb@fe1:/home/brandenb/pencil-code> } cvs co -d runs pencil-runs
\end{alltt}
In our case, \file{runs} contains individual run directories, grouped in
...
\file{src} directory (these links are set up by the script \cmd{setup-src}.
...
Bottom b.c.\ $F_{\rm bot}=-K\dd T/\dd z$,
...
$ $Id: manual.tex,v 1.99 2002-09-20 07:06:25 brandenb Exp $ $}
