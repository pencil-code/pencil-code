...
%%  CVS: $Id: manual.tex,v 1.110 2002-09-29 16:35:03 dobler Exp $
%%  Description:
...
There are two ways to obtain the code: you can download the tarball of a
stable release, or you can access the latest version via anonymous
\name{CVS}.
...
\item Get the password for anonymous \name{CVS} access to the Pencil Code
  ...
  continue to use it.\footnote{%
    ...
    assuming that \var[USER]{\$USER} is the userid registered in the original
...
take here.
(If you are lucky and use compileres similar to the ones we have, you may
...
Sec.~\ref{S-IDL-routines} for details)
\begin{alltt}
  \prompt{unix > } idl
  \prompt{IDL >  } .r start
  \prompt{IDL >  } .r r
...
are running the code, you may want to check them out into a subdircetory
of \file{pencil-code/}.
...
\end{alltt}
In this case, \file[runs/]{runs} contains individual run directories, grouped in
...
\begin{verbatim}
  1d-tests/     2d-tests/     OLD/
  buoy_tube/    convstar/     disc/         discont/
  forced/       kinematic/    pass_only/    radiation/
  rings/        slab_conv/
\end{verbatim}
...
\file[bin/]{bin} with their explicit path name, or adjust your \var{PATH}
manually.
...
Sec.~\ref{S-start-params} and \ref{S-run-params}), and the list of
...
To see whether the results of your run are OK, compare \file{n.dat} to
\file{reference.out}:
\begin{alltt}
  \prompt{unix> } diff tmp/n.dat reference.out
\end{alltt}
...
\subsubsection{\file{tmp/}}
This directory (whose name is actually set by the variable \var{datadir} in
\file{start.in}) contains the output from the code:

\paragraph{\file[dim.dat]{tmp/dim.dat}}
The global array dimensions.

\paragraph{\file[legend.dat]{tmp/legend.dat}}
The header line specifying the names of the diagnostic variables in
\file{n.dat}.\footnote{WD: Do we still need this?}

\paragraph{\file[n.dat]{tmp/n.dat}}
Time series of diagnostic variables (also printed to stdout).

\paragraph{\file[tsnap.dat]{tmp/tsnap.dat}, \file[tvid.dat]{tmp/tvid.dat}}
Time when the next snapshot \file{VAR$N$} or animation slice should be
taken.

\paragraph{\file[param.nml]{tmp/param.nml}}
...
\item[\file{seed.dat}] the random seed for the next time step (saved for
  ...
  For multi-procesor runs with velocity forcing, the files
...
\item[\file{$X$.xy}, \file{$X$.xy}, \file{$X$.xy}] two-dimensional
  sections of variable $X$. \footnote{WD: Axel, can you explain a bit more?}
\end{description}
...
Sect.~\ref{S-run-params}), the code writes monitoring output to \name{stdout}
...
\item[auto-test] Verify that the code compiles and runs in a set of run
  ...
  version of the code is inu a usable state.
\item[copy-snapshots] Copy snapshots from a processor-local directory to
  the global directory.
  Supposed to be started in the background before \file{run.x} is invoked.
  Specific to a Beowulf cluster in Odense.
\item[cvs-add-rundir] Add the current run directory to the CVS repository.
\item[cvsci_run] \footnote{WD: Need to see which one we keep:
    cvs-add-rundir or cvsci_run}
...
\item[mkcparam] Based on \file{Makefile} and \file{Makefile.local},
  ...
  \var{mvar} of fundamental variables.
  Called by the \file{Makefile}.
...
\item[nl2idl] Transform a Fortran \name{namelist} (normally the files
  ...
  or \file{run.pro}.
\item[run.csh] The script for running the code; see \S~\ref{start-run-getconf}
...
see Sect.~\ref{S-start-params}). [Occasionally, however, and only on some
...
output. The \file{run.csh} removes any \file{STOP} files before starting
...
!
&init_pars
  cvsid='$Id: manual.tex,v 1.110 2002-09-29 16:35:03 dobler Exp $',
  random_gen='nr_f90'
/
&hydro_init_pars
/
...
A full list of input parameter is given in Appendix~\ref{S-all-init-params}.
...
A full list of input parameter is given in Appendix~\ref{S-all-run-params}.
...
If you want to run two-dimensional problems, set
\var{nzgrid}=1 in \file{cparam.local}.

Similarly, for one-dimensional problems, set
\var{nxgrid}=1, \var{nygrid}=1 in \file{cparam.local}.

See \emph{need 1d and 2d samples} for examples.
...
Table~\ref{Tab-modules} listas the available physics and technical
modules.
...
$ $Id: manual.tex,v 1.110 2002-09-29 16:35:03 dobler Exp $ $}
