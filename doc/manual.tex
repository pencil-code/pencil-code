...
%%  CVS: $Id: manual.tex,v 1.54 2002-07-18 15:10:40 dobler Exp $
%%  Description:
...
31 June 2000 (to give an example), you can check out the version prior to
this by:
\begin{enumerate}
\item Go to the top directory of the code:
  \begin{alltt}
...
\begin{alltt}
  PENCIL_HOME = </home/dobler/f90/pencil>
  Adding /home/dobler/f90/pencil/bin to path
\end{alltt}
...
as your \dfn{run dirctory}, i.\,e~you want to run a three-layer slab model
of solar convection.
...
\begin{alltt}
  ...
  Thu Jul 18 16:25:20 CEST 2002
     src/start.x
  CVS: hydro.f90          v. 1.47         (dobler    ) 2002/07/16 08:29:44
  {\sl{}[\ldots]}
  CVS: start.in           v. 1.14         (dobler    ) 2002/07/09 18:38:17
   mx,my,mz=          38          38          38
   
   Adding random uu fluctuations
  ...
  0.030u 0.010s 0:00.08 50.0\%     0+0k 0+0io 122pf+0w
  
  Thu Jul 18 16:25:21 CEST 2002
...
\begin{alltt}
 ---it------t--------dt--------urms------umax------rhom------ssm-------dtc----
      0     0.00  7.603E-03   0.04939   0.08525  14.47076  -0.44601  4.21E-01
     50     0.37  7.423E-03   0.00585   0.02033  14.47033  -0.44748  4.11E-01
    100     0.74  7.520E-03   0.00597   0.01676  14.47050  -0.44789  4.17E-01
    150     1.11  7.220E-03   0.00604   0.01696  14.47026  -0.44677  4.00E-01
    200     1.47  7.093E-03   0.00412   0.01111  14.46917  -0.44648  3.93E-01
    250     1.82  7.094E-03   0.00323   0.00880  14.46890  -0.44717  3.93E-01
    300     2.18  7.094E-03   0.00293   0.00676  14.47013  -0.44768  3.93E-01
    350     2.53  7.025E-03   0.00285   0.00693  14.47027  -0.44765  3.89E-01
    400     2.88  6.943E-03   0.00233   0.00628  14.47022  -0.44772  3.85E-01
    450     3.23  6.932E-03   0.00280   0.00901  14.46989  -0.44807  3.84E-01
    500     3.57  6.927E-03   0.00280   0.00782  14.46906  -0.44796  3.84E-01
    550     3.92  6.827E-03   0.00235   0.00675  14.46849  -0.44750  3.78E-01
    600     4.26  6.770E-03   0.00202   0.00648  14.46838  -0.44798  3.75E-01
    650     4.60  6.908E-03   0.00274   0.00714  14.46884  -0.44949  3.83E-01
    700     4.95  6.992E-03   0.00175   0.00707  14.46894  -0.45078  3.88E-01
    750     5.29  6.980E-03   0.00245   0.00865  14.46868  -0.45115  3.87E-01
    800     5.64  6.899E-03   0.00400   0.01146  14.46840  -0.45052  3.82E-01
    850     5.98  6.771E-03   0.00410   0.01369  14.46800  -0.44956  3.75E-01
    900     6.32  6.687E-03   0.00352   0.01527  14.46770  -0.44909  3.71E-01
    950     6.65  6.687E-03   0.00347   0.01641  14.46781  -0.44927  3.71E-01
\end{alltt}
\end{small}
\bigskip
\end{minipage}
...
\item the mean entropy \var{rhom}$=\left<s/c_p\right>$;
\item the time step in units of the advective Courant step
  \var{dtc}$=\delta t/[\min(\delta x,\delta y,\delta z)\,cs0]$.
\end{enumerate}
...
\begin{alltt}
  ...
  \prompt{IDL >  } .r pvert \
\end{alltt}
and you should obtain somthing similar to Fig.~\ref{Fig-pvert1}, which
...
\begin{figure}[hbtp]
  ...
  \caption{Like in Fig.~\ref{Fig-pvert0}, but after 1000 time steps at
    time $t=6.99$.
    The dahsed lines represents the intitial condition.
  }
  \label{Fig-pvert1}
\end{figure}
% ---------------------------------------------------------------------- %
...
adapt \file{getconf.csh}, which is defines the commands and flags used to
...
\begin{table}[htb]
  \centering
  \caption{
    Possible coefficients for different $2N$-type third-order Runge-Kutta
    schemes.
    }\label{T-2N-RK3}
  \vspace{12pt}
  \begin{tabular}{l@{\quad}l@{\quad}ccccccc}
    \toprule
    scheme            & nodes $(t_i-t_0)/\delta t$
                                      & $\alpha_2$
                                                 & $\alpha_3$
                                                            & $\beta_1$
                                                                    &$\beta_2$
...
\begin{table}
  ...
  \begin{tabular}{lp{0.6\textwidth}}
    ...
    \midrule
    pscalar.f90    & Add variable $c$ (concentration) with
                     advection-diffusion equation for transport of a
                     passive scalar; \\
    nopscalar.f90  & no passive scalar, no variable $c$. \\
    \midrule
    \midrule
...
\end{alltt}
provided \file{\$PENCIL_HOME/bin} is in you path.
...
$\varepsilon \approx 5\times1.2\times10^{-7} = 6\times10^{-7}$; for
...
\midrule
  \var{inituu} [\code{'zero'}]
               & initialization of velocity. Currently valid choices are
                 \code{`zero'} ($\uv=0$), 
                 \code{`random-normal'} (random, normally-distributed $u_x$), 
                 \code{`sound=wave'} (sound wave in $x$ direction), 
                 \code{`shock-tube'} (polytropic standing shock). \\
  \var{ampluu} [$0.$]
...
\midrule
  \var{initlnrho} [\code{'zero'}]
               & initialization of density. Currently valid choices are
                 \code{`zero'} ($\ln\varrho=0$),
                 \code{`isothermal'} (isothermal stratification),
                 \code{`polytropic\_simple'} (polytropic stratification),
                 \code{`hydrostatic-z-2'} (hydrostatic vertical
                   stratification for isentropic atmosphere),
                 \code{`rho-jump'} (density jump of width \var{widthlnrho}),
                 \code{`piecew-poly'} (piecewise polytropic vertical
                               stratification for solar convection),
                 \code{`polytropic'} (polytropic vertical stratification),
                 \code{`sound-wave'} (sound wave),
                 \code{`shock-tube'} (polytropic standing shock).\\
  \var{gamma} [$5./3$]
...
\midrule
  \var{initss} [\code{'nothing'}]
               & initialization of entropy. Currently valid choices are
                 \code{`nothing'} (leaves the initialization done in the
                   density module unchanged),
                 \code{`zero'} (put $s=0$ explicitly; this may overwrite
                   the initialization done in the density module),
                 \code{`isothermal'} (isothermal stratification, $T=\const$),
                 \code{`isobaric'} (isobaric, $p=\const$),
                 \code{`isentropic'} (isentropic with superimposed hot [or
                   cool] bubble),
                 \code{`linprof'} (linear entropy profile in $z$),
                 \code{`piecew-poly'} (piecewise polytropic stratification
                   for convection),
                 \code{`polytropic'} (polytropic stratification,
                   polytropic exponent is \var{mpoly0}). \\
  \var{pertss} [\code{'zero'}]
               & additional perturbation to entropy. Currently valid
                 choices are
                 \code{'zero'} (no perturbation)
                 \code{'hexagonal'} (hexagonal perturbation for convection). \\
  \var{ampl_ss} [$0.$]
  ...
  \var{hcond2} [$0.$]
               & \label{hcond0123-init}%
                 ...
                 unstable layer, \var{hcond1} is the ratio
...
\midrule
  \var{initaa} [\code{'zero'}]
               & initialization of magnetic field (vector potential).
                 ...
                 \code{`zero'} (zero field),
                 \code{`gaussian-noise'} (white noise),
                 \code{`Beltrami-x'} ($x$-dependent Beltrami wave),
                 \code{`Beltrami-y'} ($y$-dependent Beltrami wave),
                 \code{`Beltrami-z'} ($z$-dependent Beltrami wave),
                 \code{`hor-fluxtube'} (horizontal flux tube),
                 \code{`hor-fluxlayer'} (horizontal flux layer),
                 \code{`uniform-Bx'} (uniform field in $x$ direction),
                 \code{`Bz(x)'} ($B_z\propto\cos(k x)$),
                 \code{`fluxrings'} (two interlocked magnetic fluxrings;
                   see Sect.~\ref{fluxrings}),
                 \code{`crazy'} (for testing purposes),
                 \code{`Alfven-circ-x'} (circularly polarized Alfvén wave).
                 \\
  \var{initaa2} [\code{'zero'}]
               & additional perturbation of magnetic field.
                 Currently valid choices are
                 \code{`zero'} (zero perturbation),
                 \code{`Beltrami-x'} ($x$-dependent Beltrami wave),
                 \code{`Beltrami-y'} ($y$-dependent Beltrami wave),
                 \code{`Beltrami-z'} ($z$-dependent Beltrami wave). \\
  \var{amplaa} [$0.$]
...
\midrule
  \var{initlncc} [\code{'zero'}]
               & initialization of passive scalar
                 ...
                 \code{`zero'} ($\ln c=0.$),
                 \code{`gaussian-noise'} (white noise),
                 \code{`wave-x'} (wave in $x$ direction),
                 \code{`wave-y'} (wave in $y$ direction),
                 \code{`wave-z'} (wave in $z$ direction). \\
  \var{initlncc2} [\code{'zero'}]
               & additional perturbation of passive scalar concentration $c$.
                 Currently valid choices are
                 \code{`zero'} ($\delta\ln c=0.$),
                 \code{`wave-x'} (add $x$-directed wave to $\ln c$). \\
  \var{ampllncc} [$0.1$]
...
\midrule
  \var{qshear} [$0.$]
               & \label{qshear-init}%
                 ...
                 are sheared in the $y$-direction). \\
\bottomrule
%\end{tabular}
\end{longtable}
% ---------------------------------------------------------------------- %
...
\midrule
  ...
  \var{ivisc} [$0$]
               & select form of viscous term:
                 \begin{description}
                 \item[\code{ivisc=0}] Simplified viscous force
                   $\Fv_{\rm visc}=\nu\Laplace\uv$
                 \item[\code{ivisc=1}] Viscous force for
                   $\mu\equiv\varrho\nu=\const$,
                   $\Fv_{\rm visc}=\mu/\varrho(\Laplace\uv+1/3\grad\Div\uv)$
                 \item[\code{ivisc=2}] Viscous force for $\nu=\const$,
                   $\Fv_{\rm visc}=\mu/\varrho(\Laplace\uv+1/3\grad\Div\uv
...
\midrule
  \var{iforce} [$2$]
               & select form of forcing in the equation of motion:
                 \begin{description}
                 \item[\code{iforce=1}] irrotational forcing
                 \item[\code{iforce=2}] helical forcing
                 \item[\code{iforce=3}] forcing of ``fountain flow'' (cf.\
                                        MNRAS 276, 651 for similar reference)
                 \item[\code{iforce=4}] forcing localized horizontal
                                        sinusoidal shear
                 \end{description}
                 \\
  \var{iforce2} [$0$]
               & select form of additional forcing in the equation of motion:
                 \begin{description}
                 \item[\code{iforce=1}]forcing of fountain flow (used in
                                       combination with helical forcing,
                                       for example.
                 \end{description}
                 \\
  \var{force} [$0.$]
...
diffusivities, use the run parameter \var{dt} (see p.~\ref{dt-run}).
...
\begin{description}
\item[Nl3:] 500\,MHz Pentium??? single CPU;
  ??? memory;
  RedHat Linux 6.2 ???
...
$\tilde{h}+\Phi=\tilde{h}_{\rm ext}=\const$.
For gravity potentials that vanish at infinity, we can have
\footnote{Axel, it might be better to rename $h_{\rm ext}$ to $h_0$.
  Only for some systems, there is really an external region that allows
  a direct interpretation of this value. ???
}%
...
Thus, when using \var{initlnrho='polytropic\_simple'}\footnote{
  Axel, is in this case $\tilde{h}_{\rm ext} = \tilde{h}(z_\infty)$???
}
 we calculate
\begin{equation}
  \ln \frac{c_{\rm s}^2}{c_{\rm s0}^2}
...
\subsubsection{Three-layered convection model}
...
\begin{figure}[hbtp]
  ...
  \includegraphics%
    [width=1\textwidth,keepaspectratio]%
    {pvert2}
  \caption{Like in Fig.~\ref{Fig-pvert0}, but at time $t=100$.
  }
  \label{Fig-pvert2}
\end{figure}
% ---------------------------------------------------------------------- %
...
horizontal sections for time $t=100$.
...
\begin{figure}[hbtp]
  ...
  \includegraphics%
    ...
    {hsect2}
  \caption{Horizontal section $z=0.352$ at $t=100$.
    Left: velocity field.
    Right: entropy (colour coded) and density (isocontours).
...
\begin{figure}[hbtp]
  ...
  \includegraphics%
    [width=0.8\textwidth,height=0.65\textheight,keepaspectratio]%
    {vsect2}
  \caption{Vertical section $y=0.516$ at $t=100$.
    Left: velocity field.
    Right: entropy (colour coded) and density (isocontours).
    Plots of this type can be produced by running the IDL scripts
...
$ $Id: manual.tex,v 1.54 2002-07-18 15:10:40 dobler Exp $ $}
...
% LocalWords:  SuSE SGI IRIX Tru noshear RedHat ethernet
