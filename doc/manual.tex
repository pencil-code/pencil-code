...
%%  CVS: $Id: manual.tex,v 1.140 2002-11-26 17:17:00 brandenb Exp $
%%  Description:
...
The calculation of the power spectra, the remeshing procedures,
...
\subsubsection{Internal compiler error with Compaqs/Dec fortran}
\ldots the Dec fortran optimiser has occasional problems with \var{nompicomm.f90}:
\begin{Verbatim}
  make start.x run.x read_videofiles.x
  f90  -fast -O5 -tune ev6 -arch ev6  -c cparam.f90
  f90  -fast -O5 -tune ev6 -arch ev6  -c cdata.f90
  f90  -fast -O5 -tune ev6 -arch ev6  -c general.f90
  f90  -fast -O5 -tune ev6 -arch ev6  -c deriv.f90
  f90  -fast -O5 -tune ev6 -arch ev6  -c nofft.f90
...
{\em
  The occurrence of this problem depends upon the gridsize;
  and the problem never seems to occur with \file{mpicomm.f90},
  except when \var{ncpus=1}.
  The problem can be avoided by switching off the loop transformation 
  optimisation (part of the \var{-O5} optimisation), via:
  \begin{Verbatim}
  ...
  with \var{nxgrid=nygrid=nzgrid=128}, \var{ncpus=16},  \var{nt=100},
  on the Mhd machine.
}
...
%    \begin{tabular}{rllrlrll}
  ...
  16 & fe1   & 0.79 & $128^3$ & magn/noentro & 4x4/ifc& 10-nov-02 & AB \\
...
\end{equation}
where $\kv(t)=(k_x,k_y,k_z)$ is a time dependent wavevector,
...
$ $Id: manual.tex,v 1.140 2002-11-26 17:17:00 brandenb Exp $ $}
