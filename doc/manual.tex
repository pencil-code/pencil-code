...
Lyra, W., Johansen, A., Klahr, H., \& Piskunov, N.\yana{2008}{491}{L41}{L44}{Embyros grown in the dead zone. Assembling the first protoplanetary cores in low-mass selfgravitating circumstellar disks of gas and solids}
...
fields, or testfields, see Section~\ref{RestartingFromLessPhysics}.
...
%
  &=&
  c_p\chi \left[ \gamma\Laplace s/c_p + (\gamma{-}1)\Laplace\ln\varrho \right] \nonumber\\
...
\frac{\Div(K\grad T)}
              {\varrho T} =
  \frac{K}{\varrho}\Bigl[ \Laplace\ln T + (\grad \ln T)^2 \Bigr] 
\end{eqnarray}
...
\begin{eqnarray}
  \pderiv{\rho_i}{t} &=& -\Div{(\rho_i \v{u}_i)} + G \label{ionized-continuity}\\
  \pderiv{\rho_n}{t} &=& -\Div{(\rho_n \v{u}_n)} - G \label{neutral-continuity}\\
  \pderiv{(\rho_i \v{u}_i)}{t} &=& -\Div{(\rho_i \v{u}_i:\v{u}_i)} - 
    \grad{\left(p_i + p_e + \frac{B^2}{2\mu_0}\right)} + F  \label{ionized-ns}\\
  \pderiv{(\rho_n \v{u}_n)}{t} &=& -\Div{(\rho_n \v{u}_n:\v{u}_n)} - 
    \grad{p_n} - F                                          \label{neutral-ns}\\
  \pderiv{\v{A}}{t} &=& \v{u}_i \times \v{B}
\end{eqnarray}where the subscripts $n$ and $i$ are for neutral and ionized, respectively. 
The terms $G$ and $F$, through which the two fluids exchange mass and momentum, are given by 

\begin{eqnarray}
  G &=& \zeta\rho_n - \alpha \rho_i^2 \\
  F &=& \zeta\rho_n\v{u}_n - \alpha \rho_i^2\v{u}_i + \gamma\rho_i\rho_n (\v{u}_n - \v{u}_i) \;.
...
In the code, \eq{ionized-continuity} and \eq{ionized-ns} are solved in \file{density.f90} and 
\file{hydro.f90} respectively. Equation~\ref{neutral-continuity} is solved un \file{neutraldensity.f90} 
and \eq{neutral-ns} in \file{neutralvelocity.f90}. The sample \file{1d-test/ambipolar-diffusion} has the 
current setup for a two-fluid simulation with ions and neutrals.
...
\subsection{Testfield equations}
\index{Testfield method}

The testfield method is used to calculate turbulent transport coefficients
...
\begin{Verbatim}
      setup-src
\end{Verbatim}
If this does not help, say first \cmd{make clean} and then \cmd{setup-src}.
}
...
%%%
\subsubsection{{\sc Pencil Code} discussion forum}
...
\begin{Verbatim}
   http://lists.albanova.se/AlbaNova/listinfo/pencilcode-discuss
   http://lists.albanova.se/pipermail/AlbaNova/pencilcode-discuss/
\end{Verbatim}
...
$\varphi(z) \equiv f(z)\,e^{\kappa z}$:
\begin{equation}
  \varphi_{N-j} = \varphi_{N+j} \; , \qquad j=1,2,3
\end{equation}
...
Being high order, {\sc Pencil} has much reduced numerical dissipation. 
...
the wavenumber. Another tool is hyper-viscosity, which replaces the $k^2$ 
...
to an operator $\nabla^n$ in real space. As {\sc Pencil} is of sixth order, 
three ghost cells are available in each direction, thus the sixth-order 
derivative is the highest we can compute. The hyper-dissipation we use is 
...
\begin{equation}
  \label{eq:hyperfick}
  {\mathcal J^{(n)}} = -D^{(n)} \grad^{2n-1} \rho,
\end{equation}In our case, we are interested in the case $n=3$, so that the hyper-diffusion term is
...
The hyper diffusion coefficient $D^{(3)}$ can be calculated from $D$ assuming that at the Nyquist frequency the two formulations (\ref{eq:diff}) and (\ref{eq:hyperdiff}) yield the same quenching. Considering a wave as a Fourier series in one dimension ($x$), one element of the series is expressed as

\begin{equation}
  \psi_k = A {\mathrm e}^{i(k x +  \omega t)} 
\end{equation}

Plugging it into the second order diffusion equation (\ref{eq:diff}) we have the dispersion condition $i\omega = -D k^2$. The sixth order version (\ref{eq:hyperdiff}) yields $i\omega = -D^{(3)} k^6$. Equating both we have  $D^{(3)} = D k^{-4}$. This condition should hold at the grid scale, where $k=\pi/\Delta x$, therefore
...
So, for the hyper-viscous force, we must replace the rate-of-strain tensor by a high order version 
...
This term, however, is only important when high compressibility is present (since it depends on the divergence of $\uv$). In practice we drop this term and use a simple hyper-viscosity 
...
\begin{equation}
  ...
  \mathbf{{\mathcal J}}=\left(D_x \pderivn{\rho}{x}{5}, D_y \pderivn{\rho}{y}{5}, D_z \pderivn{\rho}{z}{5}\right),
\end{equation}
...
\begin{equation}
  \label{eq:anisodiff}
  \Div\mathbf{{\mathcal J}}=D_x \pderivn{\rho}{x}{6}+ 
  D_y \pderivn{\rho}{y}{6}+
  D_z \pderivn{\rho}{z}{6}.
\end{equation}
...
/
\end{alltt}
because otherwise you won't read in your precious testfield data next
