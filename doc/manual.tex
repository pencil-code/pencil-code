...
%%  CVS: $Id: manual.tex,v 1.48 2002-07-14 21:16:40 dobler Exp $
%%  Description:
...
\item Localize the Makefile (see Sect.~\ref{adapt-mkfile})
\end{itemize}
\vspace{7cm}
...
\midrule
  \var{initlncc} [\code{'zero'}]
               & initialization of passive scalar (concentration $c$).
                 Currently valid choices are
                 \code{`zero'} ($\ln c=0.$),
                 \code{`gaussian-noise'} (white noise),
...
\midrule
  ...
  \var{ivisc} [--]
               & select form of viscous term:
                 ...
                 \item[\code{ivisc=2}] Mysterious viscous force
                   $\Fv_{\rm visc}=\mu/\varrho(\Laplace\uv+1/3\grad\Div\uv
...
\midrule
  \var{iforce} [$2$]
               & select form of forcing in the equation of motion:
                 ...
                 \item[\code{iforce=3}] fountain forcing (???)
                 \item[\code{iforce=4}] forcing horizontal shear (???)
                 \end{description}
                 \\
  \var{iforce2} [$0$]
               & select form of additional forcing in the equation of motion:
                 \begin{description}
                 \item[\code{iforce=1}] fountain forcing (???)
                 \end{description}
                 \\
  \var{force} [$0.$]
...
$\chi = \gamma\lambda/(c_p\varrho)$ (a quantity proportional to) thermal
...
different machines.
Shown is (amon other quantities) the wall clock time per mesh point
...
\begin{description}
\item[Nl3:] 500\,MHz Pentium??? single CPU; Linux;
  ???> memory per (dual) node
\item[Nq$N$:] 924\,MHz Pentium??? cluster; Linux;
  ???> memory per (dual) node
\item[Kabul:] 1.9\,GHz Athlon dual-CPU cluster;
  ...
  SuSE Linux 8.0;
  LAM-MPI
\item[Ukaff:] ???\,MHz SGI Origin 3000;
  IRIX ???;
  native MPI
\item[Mhd:] ???\,MHz EV? Compaq cluster with 4 CPUs per node;
  OSF1 (Tru64);
  native MPI
\end{description}
...
\begin{equation}
  -\nab h+T\nab s
  = -\frac{1}{\rho}\nab p
  = -\nab\left(\frac{\Gamma K}{\Gamma{-}1}\rho^{\Gamma{-}1}\right)
\equiv-\nab\tilde{h},
\end{equation}
...
$ $Id: manual.tex,v 1.48 2002-07-14 21:16:40 dobler Exp $ $}
...
% LocalWords:  endif lreset iname nname cname cform enddo Pentium GHz Athlon
% LocalWords:  SuSE SGI IRIX Tru
