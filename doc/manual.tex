...
%%  CVS: $Id: manual.tex,v 1.119 2002-10-02 17:29:05 brandenb Exp $
%%  Description:
...
\item[{\file[X.xy]{$X$.xy}, \file[X.xz]{$X$.xz}, \file[X.yz]{$X$.yz}}]
  two-dimensional sections of variable $X$.
  \footnote{WD: Axel, can you explain a bit more?}
\end{description}
...
\begin{verbatim}
  &run_pars
    cvsid='\${}Id:\$',                 \textsl{! identify version of start.in}
    nt=10, it1=2, cdt=0.4, cdtv=0.80, isave=20, itorder=3,
    dsnap=50., dvid=0.5,
    random_gen='nr_f90'
  /
  &hydro_run_pars
    nu=5e-3, ivisc='nu-const',
  /
  &density_run_pars
  /
  &forcing_run_pars
...
\subsection{A comment regarding the viscosity}
\label{viscosity}

We emphasise that the code works with constant diffusion coefficients
(viscosity $\nu$, thermal diffusivity $\chi$, magnetic diffusivity $\eta$,
or passive scalar diffusivity ${\cal D}$). If any of these numbers are
too small, you would need to have more meshpoints; otherwise the code
develops wiggles (`ringing') and blows up. A useful criterion is based
on the mesh Reynolds number based on the maximum velocity,
\begin{equation}
  \mbox{Re}_{\rm mesh}=\max(|\uv|)\max(\delta x,\delta y,\delta z)/\nu,
\end{equation}
which should not exceed a certain value with can be problem-dependent.
Often the largest possible value of $\mbox{Re}_{\rm mesh}$ is around 5.
Similarly there exist also mesh Peclet and mesh magnetic Reynolds numbers.
...
coefficients \var{cdt} ($c_{\delta t}$) and \var{cdtv} ($c_{\delta t,{\rm v}}$).
...
\item[\option{she}] shearing-sheet boundary condition (default when )
\end{description}
...
\subsubsection{Data explorer networks}
\label{S-openDX}

A few networks for \name{DX} (\name{data explorer}) are available.
It is quite easy to read in a snapshot file from \name{DX} (the only
tricky thing are four extra bytes at the beginning of the file,
representing a Fortran record marker), and we should
document this here and have the Fortran code write a \texttt{.general}
file.\footnote{WD: Do this\ldots}
...
\begin{equation}
  \frac{D\uv}{Dt}
   =  -\cs^2\grad\biggl(\frac{s}{c_p} + \ln\varrho\biggr)
      ...
      + 2\Strain\cdot\grad\ln\varrho\right) \; .
\end{equation}
Here, $\cs^2 = \gamma p/\varrho$ is the squared sound speed,
$\gamma=c_p/c_v$ the ratio of specific heats or \emph{adiabatic index},
$\Phi_{\rm grav}$ is the gravity potential,
...
$\Strain$ is the traceless rate-of-strain tensor.
...
\bibitem{BNST95} Brandenburg, A., et al.,
  \emph{Astrophys. J.} \textbf{446}, 741 (1995)
\bibitem{Ref-3} Brandenburg, A., et al.,
  \emph{J. Fluid Mech.} \textbf{306}, 325 (1996)
...
\bibitem{NR} Press, W., Teukolsky, S., Vetterling, W., \& Flannery, B.,
  \emph{Numerical Recipes in Fortran 90}, Cambridge (1988)
...
\bibitem{Ref-4} Brandenburg, A., Dobler, W.,
  \emph{Astron. Astrophys.} \textbf{369}, 329 (2001)
...
\bibitem{Ref-2} Brandenburg, A.,
  \emph{Astrophys. J.} \textbf{550}, 824 (2001)
...
and the power spectrum of $\vec{\omega}$ (or $\Bv$) increases like $k^4$.
...
\mbox{Ra}={gd^4\over\overline{\nu}\;\overline{\chi}}
\left(-{\mbox{d}s/c_{\rm p}\over\mbox{d}z}\right)_{\rm hydrostat},
\end{equation}
...
$\overline\chi=K/(\overline\rho c_{\rm p})$ and either $\overline\nu=\nu$
...
the definition of Hurlburt \& Toomre (1984), but their definition
differs slightly from Eq.~(\ref{Radef}). Since the Rayleigh number is
proportional to $\rho^2$, their definition included the extra factor
$[(z_{\infty}-z_{\rm m})/d]^2$. Therefore
\begin{equation}\label{Rdef}
\mbox{Ra}_{\rm HT}=\left({z_{\infty}-z_{\rm m}\over d}\right)^{2m}
\left({\overline\rho\over\rho_{\rm top}}\right)^2\mbox{Ra}
\end{equation}

%\subsubsection{Relation to the setup of Hurlburt \& Toomre}
...
(\gamma-1){\dd\ln\varrho\over\dd z}+\gamma{\dd s/c_{\rm p}\over\dd z}\right),
\end{equation}
which gives
\begin{equation}\label{Fbot2}
{\dd s/c_{\rm p}\over\dd z}=-{\gamma-1\over\gamma}\left(
...
\begin{table}[htb]
  \begin{center}
    \caption{
	Values on the different sides of the shock.
    }
    \label{Tshock1}
    \begin{tabular}{lll}
    \toprule
  ...
  \var{iforce} [$2$]
               & select form of forcing in the equation of motion;
                 ...
                 \item[\code{'fountain'}] (forcing of ``fountain flow'';
                   cf.~\emph{MNRAS} \textbf{276}, 651 (1995)
                   for a reference),
...
\midrule
  \var{it} & number of time step (since beginning of job only)\\
  \var{t} & $t$ (since start.csh)\\
  \var{dt} & $\delta t$ \\
...
\midrule
  ...
  \var{ekin} & $\left<{1\over2}\varrho\uv^2u_x\right>$ \\
%
\midrule
  \multicolumn{2}{c}{Module \name{entropy.f90}} \\
\midrule
  \var{ssm} & $\left<s/c_{\rm p}\right>$ \\
%
\midrule
  \multicolumn{2}{c}{Module \name{magnetic.f90}} \\
\midrule
  ...
  \var{bmx} & $\left<\left<\Bv\right>_{yz}^2\right>^{1/2}$ \\
  \var{bmy} & $\left<\left<\Bv\right>_{xz}^2\right>^{1/2}$ \\
  \var{bmz} & $\left<\left<\Bv\right>_{xy}^2\right>^{1/2}$ \\
%
\midrule
  \multicolumn{2}{c}{Module \name{pscalar.f90}} \\
\midrule
...
$ $Id: manual.tex,v 1.119 2002-10-02 17:29:05 brandenb Exp $ $}
...
% LocalWords:  setenv cvs cvspass code's dP sourceme mkdir tmp tmp struct Weyl
...
% LocalWords:  wcool iforce relhel tsnap isothermality initaa jbm
