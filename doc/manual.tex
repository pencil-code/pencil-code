...
%%  CVS: $Id: manual.tex,v 1.387 2008-03-31 12:54:00 nilshau Exp $
%%  Description:
...
%\date{$ $Date: 2008-03-31 12:54:00 $ $,~ $ $Revision: 1.387 $ $}
...
\begin{titlepage}
  ...
  \emph{$ $Date: 2008-03-31 12:54:00 $ $,~ $ $Revision: 1.387 $ $}\\
...
For \code{grid_function='duct'} a grid profile for turbulent flow in ducts
are used.
Since most gradients are steepest close to the walls, 
it is required to have 
very fine resolution near the walls in a duct flow. Here we follow the method of 
\cite{kim87} 
and use a cosine distribution of the grid point such that
in the y direction the grid points are found at
\begin{equation}
\label{deltay}
y_j=h\cos \theta_j
\end{equation}
where
\begin{equation}
\theta_j=\frac{(N_y-j)\pi}{N_y-1}, \;\;\; j=1,2,...N_y.
\end{equation}

Currently this method is only adapted for ducts where the $x$ is in
the streamvise direction, $z$ is in the spanwise direction and the walls
are at $y=y_0$ and $y=y_0+L_y$.

In order to have fine enough a resolution the first grid point should be
a distance $y_{\rm first}=0.05l_w$ from the wall,
when
\begin{equation}
l_w=\frac{\nu}{u_{\tau}},
\end{equation}
...
This is accomplished by using a total of 
\begin{equation}
N_y=\frac{\cos^{-1}\left(s_1\right)-2\pi}{\cos^{-1}\left(s_1\right)-\pi},
\end{equation}
grid points in the $y$-direction when
\begin{equation}
s_1=\frac{y_{\rm first}-h}{h}.
\end{equation}
...
\section*{APPENDIX
  \hfill\normalsize\rm$ $Date: 2008-03-31 12:54:00 $ $,~ $ $Revision: 1.387 $ $}
...
$ $Id: manual.tex,v 1.387 2008-03-31 12:54:00 nilshau Exp $ $}
