...
%%  CVS: $Id: manual.tex,v 1.104 2002-09-27 07:28:14 brandenb Exp $
%%  Description:
...
\begin{titlepage}
  ...
  {\sffamily Documentation and Test Problems}
...
own set of parameters. In particular, you need to be careful in choosing
...
you can more efficiently \emph{read the source code}, which contains lots
of comments.
For further enhancements that you may want to add to the code, you can follow
-- as an example -- the lines in the code that deal with related variables,
functions, diagnostics, equations etc., which whave already been implemented.
...
were implemented by Nils Erland Haugen (University of Trondheim).
...
\begin{enumerate}
  ...
  \item Absolutely needed:
    \begin{itemize}
    \item F90 or F95 compiler (note that there is the free Intel compiler, ifc)
    \end{itemize}
...
We refrain from making any `stable releases' available at this point.
We are still improving the code which enhances functionality.
A set of test problems (listed in
\file[auto-test]{\$PENCIL_HOME/bin/auto-test})
are routinely tested. This set of test problems includes those in
...
%  \path{http://www.nordita.dk/data/brandenb/pencil-code/download/pencil-code.tgz}
%\item Put it into a convenient directory and unpack it:
%  \begin{alltt}
%  \prompt{unix> } mv pencil-code.tgz somewhere/; cd somewhere
%  \prompt{unix> } gunzip pencil-code.tgz
%  \prompt{unix> } tar xf pencil-code.tar \
%  \end{alltt}
%\end{enumerate}
...
\item Many machines have already \name{CVS} (just say \code{which cvs}).
...
\item Get the password for anonymous \name{CVS} access to the Pencil Code by
going to the web address \url{http://www.nordita.dk/data/brandenb/pencil-code}.
Alternatively, just email \email{Wolfgang.Dobler@kis.uni-freiburg.de}
or \email{Axel.Brandenburg@nordita.dk}.
Priviledged users with write access to the original repository should
continue to use it.\footnote{In that case you'd say
\code{setenv CVSROOT :pserver:\$USER@norserv.nordita.dk:/home/brandenb/CVS},
assuming that \cmd{\$USER} is the userid registered in the original
...
\item Go to wherever you want the code (otherwise the code will land
in your home directory, which will also be ok):
  \begin{alltt}
  \prompt{unix> } cd \file[somewhere/]{somewhere} \
  \end{alltt}
  and check the code out:
  \begin{alltt}
  \prompt{unix> } cvs checkout pencil-code \
  \end{alltt} 
  This creates a subdirectory \file[pencil-code/]{somewhere/pencil-code} and populates it with
...
Independent of how you installed the code in the first place (tarball or
...
\item Go to the top directory of the code:
  \begin{alltt}
  \prompt{unix> } cd \file[pencil-code/]{somewhere/pencil-code} \
  \end{alltt}
...
(we will assume here that it is called \file[pencil/]{pencil}) and source one of
...
can set (or change, in case of tests!) the environment variable
...
thinking twice.
If, however, at the first souring, the setting of \env{PENCIL_HOME} was
...
\file[tmp/]{tmp} of the run directory.
...
NFS.
The recommended way to set up a \file[tmp/]{tmp} data directory is to generate
a corresponding directory on the local disc of the computer you are
running on and (soft-)link it to \file[tmp/]{./tmp}.
Even if the link is part of an NFS directory, all the IO operations will
be local.
E.\,g.~if you have a local disc \file[/scratch/]{/scratch}, you can do the
following
\begin{alltt}
...
This initial condition is stored in \file[var.dat]{tmp/proc0/var.dat} (and
in \file[var.dat]{tmp/proc1/var.dat}, etc. if you run the multiprocessor
version).
...
Since \file{getconf.csh} is always linked, your changes will be
...
\begin{alltt}
  ...
  \prompt{unix> } make
  \prompt{unix> } cd ..
  \prompt{unix> } mkdir tmp \quad{\sl\# (or [better] link directory on local disc to \file[tmp/]{tmp})}
...
separately for each run in a local \file[src/]{src} directory below the
...
this would usually be a different repository. On the machine where you
are running the code you may want to check in out in your
\file[pencil-code/]{pencil-code} directory.
In our case we have our own runs in a repository called
\file[pencil-runs/]{pencil-runs}, so we say
\begin{alltt}
  \prompt{brandenb@fe1:/home/brandenb> } cd \$PENCIL_HOME
  \prompt{brandenb@fe1:/home/brandenb/pencil-code> } cvs co -d runs pencil-runs
\end{alltt}
In our case, \file[runs/]{runs} contains individual run directories, grouped in
...
\begin{verbatim}
   4 1d-tests/    4 OLD/          4 convstar/    4 forced/       4 radiation/
   4 2d-tests/    4 README        4 disc/        4 kinematic/    4 rings/
   4 CVS/         4 buoy_tube/    4 discont/     4 pass_only/    4 slab_conv/
\end{verbatim}
...
\file[pencil-runs/]{pencil-runs} directory also contains done some
time ago. Occasionally, we try to update these, especially if we
...
\file[src/]{src} directory (these links are set up by the script
\cmd{setup-src}.

General purpose visualization routines for \name{IDL} or \name{DX} are in the
\file[idl/]{idl} and \file[dx/]{dx} directories, respectively.
There are additional and more specialized \name{IDL} directories in the
different branches under \file[pencil-runs/]{pencil-runs}.

The directory \file[doc/]{doc} contains this manual; \file[bin/]{bin} a
number of utility scripts (mostly written in \name{csh} and \name{Perl}),
and in particular the \file{start.csh}, \file{run.csh}, and
\file{getconf.csh} scripts.
...
\file[bin/]{bin} with their explicit path name.
...
long arrays (good on vector processors). This approach also implies
...
In all such cases, we depend on your feedback for fixing problems
...
These files will contain mainly parameters that are different from their
default values. You need to check very carefully that the code is doing
what is desired. In the beginning of the run, it will show you normally a
selected amount of parameters that are set.  You can get more information
by lowering the \var{ip} parameter, but this tends to become messy,
so it is better to check in \name{IDL} that you are doing the right thing.

\subsubsection{param.nml, param2.nml; param[2].pro}

These are written automatically and used by other routines.
...
options \option{-lmpi} \option{-llam} for \name{MPI}.
...
and, parallel to this, to the file \file[n.dat]{tmp/n.dat}.
...
They are written to the file \file[zaverages.dat]{tmp/zaverages.dat}.
At the moment we
...
see Sect.~\ref{start-params}). [Occasionally, however, and only on some
...
output. The \file{run.csh} removes and \file{STOP} files before starting
...
modifying a particular example that is closest to your application.
...
!
&init_pars
  cvsid='$Id: manual.tex,v 1.104 2002-09-27 07:28:14 brandenb Exp $',
  random_gen='nr_f90'
/
&hydro_init_pars
/
...
/
\end{verbatim}
For reproducibility, we use not the system random number generator,
but the f90 procedure that is given in the Numerical Recipies \cite{NR}.
$\gamma=1$ is set explicitly (the default would have been 5/3).
The magnetic vector potential is initialized with gaussian random noise
with amplitude $10^{-4}$.

A full list of input parameter is given in Appendix~\ref{Sall-init-params}.

\subsection{Run parameters}
\label{Run-params}

As an example, we give here the run parameters for
\file{samples/helical-MHDturb}
\begin{verbatim}
&run_pars
  cvsid='$Id: manual.tex,v 1.104 2002-09-27 07:28:14 brandenb Exp $',
  nt=10, it1=1, cdt=0.4, cdtv=0.80, isave=10, itorder=3,
  dsnap=50, dvid=0.5,
  random_gen_tmp='nr_f90'
/
&hydro_run_pars
  nu=5e-3, ivisc='0',
/
...
/
\end{verbatim}
Here we run for 10 timesteps, give output every timestep, use the 3-step scheme
(1-step schemes only useful for tests), use again a deterministic random number
generator\footnote{The reason we have the \_tmp in the name has to do with a
compiler problem on \name{IRIX} machines!}. Viscosity and magnetic diffusivity
are set to $5\times10^{-3}$ (so the mesh Reynolds number is about
$u_{\rm rms}\delta x/\nu=0.3\times(2\pi/32)/5\times10^{-3}\approx12$,
which is rather a bit to high!)

A full list of input parameter is given in Appendix~\ref{Sall-run-params}.
...
diffusivities, set the run parameter \var{dt} to overwrite the Courant
value (see p.~\pageref{dt-run}).
...
sheet boundary conditions and have stress-free instead).\footnote{Remember
that in the parameter list you can have the same line twice, and only the
last one is used.}
...
\begin{itemize}
\item \file[n.dat]{tmp/n.dat} --- output diagnostics, depends on what is
    put in print.in
\item \file{tsnap.dat}, \file{tvid.dat}
\item \file[dim.dat]{tmp/dim.dat}
\item \file[param.dat]{tmp/param.dat} \file[param2.dat]{tmp/param2.dat}
\item \file[var.dat]{tmp/proc\emph{X}/var.dat} --- detailed format
\item \file[VAR$N$]{tmp/proc\emph{X}/VAR\emph{N}} --- same format as
      \file[var.dat]{tmp/proc\emph{X}/var.dat}
\item \file[seed.dat]{tmp/proc\emph{X}/seed.dat} --- seed field for forcing
      procedure: must be the same for all processors, because globally
      coherent waves of given wavenumber are used
\item \file[index.pro]{tmp/index.pro}
      ...
      \name{entropy} or \name{noentropy}, etc, are invoked.
\item \file[param.nml]{tmp/param.nml}
\end{itemize}
...
(perhaps too much) for very large runs.
...
\item setup (include \file[idl/]{../idl} and \file[idl/]{../../idl} (?) in
  \var{!path})
...
(For us this hasn't been a serious restriction.)

\subsubsection{Bandwidth-limited clusters (e.g.\ Beowulf machines)}
\label{Bandwidth}

On big Beowulf machines, a group of nodes is often connected with a switch
of modest speed, and all these groups are connected via a $n$ times
faster uplink. When bandwidth-limited, it is important to make sure that
consecutive processors are mapped onto the mesh such that the load on
the uplink is indeed approximately $n$ times larger than the load on the
modestly fast switch within each group. We are currently running on a 512
node cluster where groups of 24 processors are linked via fast ethernet,
which in turn are connected via a Gigabit uplink ($\sim10$ times faster).
This means that invariably \var{nprocy}=4 is optimal. For 128 processors,
for example we find that $\var{nprocy}\times\var{nprocz}=4\times32$ is
optimal. For comparison, $8\times16$ is 3 times slower and $16\times8$
is even 17 times slower.
...
\begin{equation}
  \frac{D\uv}{Dt}
   =  -\cs^2\grad\biggl(\frac{s}{c_p} + \ln\varrho\biggr)
      ...
      + \Strain\cdot\grad\ln\varrho\right) \; .
\end{equation}
...
the magnetic flux density, $\nu$ is kinematic viscosity, and $\Strain$ is
...
this option could be a useful approximation during an initial relaxation phase;
...
and $\nu$ is then replaced by $\mu/\varrho$.
...
$K$ is the radiative (thermal) conductivity, and
...
\subsection{Processor layout when using MPI}

It often pays (and is easy) to adapt to the particular hardware.
In \S\ref{Bandwidth} we mentioned to extreme sensitivity to
the precise layout of processors. If you have control over
the design of the machine, you may want to chose a torus-like
arrangement that matches best with your mesh. In general you
won't be alone on the machine, so you have to find a good
compromise. A two-layered switch arrangement is probably a
reasonable compromise, in which case it is important to minimize
the contact surface between the nodes on one switch and the
outside nodes; see \S\ref{Bandwidth}.
...
in the code, and we were only now adding it in, we would need to do the following
\begin{enumerate}
...
rather big file \file[zaverages.dat]{tmp/zaverages.dat}.
...
{\em
  ...
  (Note that \file{cparam.inc} is automatically generated.)\footnote{Wolfgang,
    I just added \file{*.inc} to the \file{Makefile}.}
}
...
{\em
  You don't have the subdirectory \file[tmp/]{tmp} in your IDL variable
  \var{!path}.
...
\bibitem{NR} Press, W., Flannery, B., Teukolsky, S. \& Vetterling, W. 1988
...
In this appendix we give timing results, discuss in greater detail
some initial conditions, list ?all possible input parameters, and
discuss some numerical aspects of the code.
...
Note that the timer currently used will overflow on some machines (after
...
\begin{table}[htb]
 ...
 128 & fe1   & 0.166& $512^3$ & magn/noentro & 4x32   &  9-sep-02 & AB \\
...
\subsection{Random velocity or magnetic fields: \code{inituu} or \code{initaa='gaussian-noise'}}
...
\subsection{Beltrami fields: \code{inituu} or \code{initaa='Beltrami-z'}}
...
$\nu_{\rm g}=0.5\Omega$ is respresentative of the solar neighborhood.
...
(\gamma-1){\dd\ln\varrho\over\dd z}+\gamma{\dd s\over\dd z}\right),
\end{equation}
which gives
\begin{equation}\label{Fbot2}
{\dd s\over\dd z}=-{\gamma-1\over\gamma}\left(
...
\section{List of possible input parameter for \file{start.in}}
\label{Sall-init-params}.
...
This list is likely not to be up to date; also, in many cases it can only
...
\midrule
  ...
  \var{random_gen} [\code{system}]
               & If set to \code{system}, the systems random number generator
                 is used. If set to \code{min_std}, a Numerical Recipes
                 routine is used.\\
%
\midrule
  \multicolumn{2}{c}{Namelist \name{hydro_init_pars}} \\
...
\section{List of possible input parameter for \file{run.in}}
\label{Sall-run-params}.
...
\midrule
  ...
  \var{Fbot} [start]
               & heat flux for bottom boundary condition \option{c1}.
                 ...
                 be calculated from the value of \var{hcond0} in \cmd{start.x},
...
$ $Id: manual.tex,v 1.104 2002-09-27 07:28:14 brandenb Exp $ $}
...
% LocalWords:  setenv cvs cvspass code's dP sourceme mkdir tmp tmp struct
...
% LocalWords:  ttransient fe aug Gridur isentropy Eqs
