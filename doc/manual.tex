...
Additionally, more complicated computations outside of the RHSs are not included like the Navier-Stokes Characteristic Boundary Condition (NSCBC) and granulation calculations in the corona modules.
To see which of the commited samples are known to run on GPUs, check the list gpu\_test in \file{bin/pc_autotest}.
...
\begin{verbatim}
Integration of PDEs and ODEs. Boundary conditions that can be formulated
as local operations. (For not yet covered boundary conditions requires
explicit use of code tools to generate the GPU code.) Calculations
of averages or other reductions before time integration. (For not
yet covered cases requires explicit use of code tools to generate the
GPU code.) Fourier transforms (requires handwritten modifications to
add new physics).
\end{verbatim}
% ---------------------------------------------------------------------- %
