...
the popular parametrizations for radiative losses, Kramers opacity,
...
for an entire OS-X login session, google environment.plist. That won't be
needed here.
...
In Cartesian geometry, this is generally achieved by setting
the two tangential components of $A$ to be zero.
In the case of spherical coordinates, we can do the same, as we will show
now.

Let us first consider a radial boundary.
In that case we must have
\begin{equation}
  B_r = \frac{1}{r} \partial_{ \theta} A_{\phi}
        - \frac{1}{r\sin\theta} \partial_{\phi} A_{\theta}
        + \frac{\cot\theta}{r} A_{\phi}
      = 0 \; .
\end{equation}
...
Next, on a boundary with constant $\theta$ we must have
\begin{equation}
...
The stress-free boundary condition for the velocity is generally
implemented by setting the normal component of the velocity at the
boundary to zero.
...
$\partial_r u_{\theta} - \frac{1}{r} u_{\theta} = 0$.
This is best done by writing the above condition in the following way,
$\partial_r(u_{\theta} / r) = 0$.
We then set the symmetric condition on $u_{\theta} / r$.
Actually a more general but related boundary condition can be set, which
is
\begin{equation}
  \partial_r u_{\theta} -\frac{\alpha}{r} u_{\theta} = 0
\end{equation}
where $\alpha$ is a constant.
This can be set by setting the symmetric condition on
$\partial_r(u_{\theta} / r^{\alpha}) = 0$

At the boundary with constant $\theta$, the stress-free boundary
...
\end{eqnarray}
with $u_{\theta} = 0$.
The first condition can be satisfied by $u_r=0$.
The second condition can be satisfied by
\begin{equation}
  \frac{1}{r}\partial_{\theta} u_{\phi}
  - \dfrac{\cot\theta}{r} u_{\phi}
  = \dfrac{\sin\theta}{r}
    \partial_{\theta} \left(\dfrac{u_{\phi}}{\sin\theta}\right)
  = 0 \; .
\end{equation}
This is implemented as a symmetric boundary condition on
$u_{\phi} / \sin\theta$.
