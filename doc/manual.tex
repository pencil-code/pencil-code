...
(as opposed to compact) derivatives.
Typical targets include driven MHD turbulence in a periodic box,
convection in a slab with non-periodic upper and lower boundaries,
a convective star embedded in a box, accretion disc turbulence in
...
The code is non-conservative. Instead, conserved quantities should be
conserved up to the discretization error of the scheme. Fully conservative
...
our knowledge not guaranteed to be conserved by ordinary conservative
schemes.
...
The parameter input file (run.in) is read in regular time intervals
to one can change parameters during the fly. This can be useful if one
wants to stop the in a graceful way, if one wants to change the time step,
the viscosity, or (in forced calculations) the sound speed. Obviously, in
...
\vfill\bigskip\noindent{\tiny\it
$ $Id: manual.tex,v 1.6 2002-03-21 17:09:59 brandenb Exp $ $}
