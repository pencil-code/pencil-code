...
\begin{tabular}{lll}
  ...
  dhruba & \htmladdnormallink{Dhrubaditya Mitra}{http://www.maths.qmul.ac.uk/~dhruba} &  Queen Mary College, London\\
  ...
  ajohan & \htmladdnormallink{Anders
  Johansen}{http://www.strw.leidenuniv.nl/~ajohan} & Leiden Observatory, Leiden
  University
\end{tabular}

\vfill

Copyright \copyright{} 2001--2009 Wolfgang Dobler \& Axel Brandenburg
\bigskip
...
\item Unless you are a privileged users with write access, you can
  ...
  \code{svn checkout http://pencil-code.googlecode.com/svn/trunk/ pencil-code}
  Privileged users with write access to the original repository should use
  something like
  \code{svn checkout https://pencil-code.googlecode.com/svn/trunk/ pencil-code --username USERNAME},
  where \name{USERNAME} is to be replaced by {\it your} googlecode user
  name.
...
% {\sc Pencil Code} directories.  This for example avoids the problem of having
% a partially checked out \file{runs} directory under your {\sc Pencil Code}
...
%A new line of averages is written every time \file{var.dat} is updated
%(i.e.\ every \var{it1}th time steps).
%AB: it1 and isave are in general not the same.
A new line of averages is written every \var{it1}th time steps.
See Sec.~\ref{S-new-output-diagnostics} on how to add new averages.
...
%\subsubsection{Vertical averages}
%AB: a vertical average would be zaver.in, which also exists.
%AB: So I changed it to "planar averages". OK?
\subsubsection{Other planar averages}
\label{S-ver-averages}

%There is the possibility to output also vertical $xz$- and $yz$- averages.
%AB: planar.
...
\item[copy-VARN-to-var] Used to restart a run from a particular snapshot 
...
fields, or test fields, see Section~\ref{RestartingFromLessPhysics}.
...
will now be calculated every 2.3 (\code{dspec}) time units. 
...
%$\mu/\rho_0$ (\var{rho0} is another input parameter).
%The term involving the
%rate-of-strain tensor does not appear in the equation,
%and $\nu$ is then replaced by $\mu/\rho$, which is no longer constant.
%
...
By setting \code{pretend_lnTT=T} the logarithmic temperature is used
...
\begin{eqnarray}
  \pderiv{\rho_i}{t} &=& -\Div{(\rho_i \v{u}_i)} + {\mathcal G} \label{ionized-continuity}\\
  \pderiv{\rho_n}{t} &=& -\Div{(\rho_n \v{u}_n)} - {\mathcal G} \label{neutral-continuity}\\
  \pderiv{(\rho_i \v{u}_i)}{t} &=& -\Div{(\rho_i \v{u}_i:\v{u}_i)} - 
    \grad{\left(p_i + p_e + \frac{B^2}{2\mu_0}\right)} + {\mathcal F}  \label{ionized-ns}\\
  \pderiv{(\rho_n \v{u}_n)}{t} &=& -\Div{(\rho_n \v{u}_n:\v{u}_n)} - 
    \grad{p_n} - {\mathcal F}                                          \label{neutral-ns}\\
  \pderiv{\v{A}}{t} &=& \v{u}_i \times \v{B}
...
\begin{eqnarray}
  ...
  {\mathcal F} &=& \zeta\rho_n\v{u}_n - \alpha \rho_i^2\v{u}_i + \gamma\rho_i\rho_n (\v{u}_n - \v{u}_i) \;.
...
\begin{equation}
  \frac{d{\v{v}}_{p_i}}{dt} = {\v{F}}_{g_i} -\sum_{j\neq i}^{N}\frac{G M_j}{{\mathcal R}_{ij}^2} \hat{\v{{\mathcal R}}}_{ij}
  \label{eq:dust-dynamics}
\end{equation}where ${\mathcal R}_{ij}=|\v{r}_{p_i}-{\v{r}}_{p_j}|$ is the
distance between particles $i$ and $j$, and $\hat{\v{{\mathcal R}}}_{ij}$ is
...
\begin{equation}
  {\v{F}}_{g_i} = -G\int_{V} \frac{[\rho_g(\v{r})+\rho_p(\v{r})]\v{{\mathcal R}}_i}{({\mathcal R}_i^2 + b_i^2)^{3/2}} \de V, 
...
inertial frame - with origin at the baricenter of the $N$-body ensemble - 
...
to one part in $\ttimes{5}$ within the timespan of 100 orbits.
...
For technical details, see also Section~\ref{RestartingFromLessPhysics}.
...
{\em
This is because somebody added a new module (together with a corresponding
\name{nomodule.f90} and a \name{module.h} file (chemistry in this case).
These files didn't exist before, so you need to say:
\begin{Verbatim}
      pc_setupsrc
\end{Verbatim}
...
{\em
  The {\sc Pencil Code} should compile successfully with \name{ifc}~6.x,
...
{\em
Then don't use the old LAM-MPI. It is long superseded by open-mpi now.
...
{\em
Thanks for letting me know about the status, and congratulations
...
Macs work well for Linux stuff, except that the file structure is slightly
different. Problems when following Linux installs can usually be traced to
the PATH. For general reference, if you need to set an environment variable
for an entire OSX login session, google environment.plist. That won't be
needed here.
...
changes to the code via svn submit code
...
Sec.~\ref{S-hor-averages} describes how to output horizontal averages
...
document. Punctuation at the end of a \emph{short} comment may be omitted.
...
With the addition of spherical and cylindrical coordinate systems more
complicated boundary conditions has now been implemented in the code. 
So specific examples are discussed below. 

\subsection{Prefect-conductor boundary condition}

This boundary condition is often needed to be implemented for the magnetic
field. This implies that 
\begin{equation}
  B_n = 0
\end{equation}
at a boundary, where the subscript $n$ denotes the normal component. 
Implementing this in cartesian case is generally achieved by setting
the two tangential components of $A$ to be zero. In the case of
spherical coordinates, let us first consider a radial boundary. In that
case we must have
\begin{equation}
B_r = \frac{1}{r}\partial_{ \theta}A_{\phi} - 
      \frac{1}{r\sin(\theta)}\partial_{\phi}A_{\theta} 
      + \frac{\cot(\theta)}{r} A_{\phi} = 0 \/.
\end{equation}
This can be achieved by 
\begin{equation}
A_{\phi}=A_{\theta}=0
\end{equation}
Note that this does not set any condition on the radial component of the 
vector potential.  Next, on a boundary with constant $\theta$ 
we must have
\begin{equation}
...
This needs to be often imposed on the velocity. This is generally accomponied
by setting the normal component of the velocity at the boundary
to be zero. At the radial boundary
the relevent component of the strain tensor that is to be set to 
zero are:
\begin{equation}
S_{r\theta}=\frac{1}{r}\partial_{\theta}U_r + \partial_r U_{\theta}
           -\frac{1}{r}U_{\theta} = 0
S_{r\phi}=\frac{1}{r\sin(\theta)}\partial_{\phi}U_r + \partial_r U_{\phi}
           -\frac{1}{r}U_{\phi} = 0
\end{equation}
With $U_r=0$ we need to set 
$\partial_r U_{\theta} -\frac{1}{r}U_{\theta} = 0$. This is best done
by writing the above condition in the following way, 
$\partial_r(\frac{U_{\theta}}{r}) = 0$. We then set the symmetric 
condition on $ \frac{U_{\theta}}{r}$. Actually a more general but related
boundary condition can be set, which is 
\begin{equation}
\partial_r U_{\theta} -\frac{\alpha}{r}U_{\theta} = 0
\end{equation}
where $\alpha$ is a constant. This can be set by setting the symmetric
condition on 
$\partial_r(\frac{U_{\theta}}{r^{\alpha}}) = 0$

At the boundary with consant $\theta$ the stress-free boundary 
condition will take the form, 
\begin{equation}
S_{r\theta}=\frac{1}{r}\partial_{\theta}U_r + \partial_r U_{\theta}
           -\frac{1}{r}U_{\theta} = 0
S_{\theta\phi}=\frac{1}{r\sin(\theta)}\partial_{\phi}U_{\theta} + 
                \frac{1}{r}\partial_{\theta}U_{\phi}
              -\frac{\cot(\theta)}{r}U_{\phi}
\end{equation}
with $U_{\theta} = 0$. The first case can be satisfied by,
$U_r=0$. The second case can be satisfied by 
$\frac{1}{r}\partial_{\theta}U_{\phi}-\frac{\cot(\theta)}{r}U_{\phi}$ 
which we write as, $\partial_{\theta}\frac{U_{\phi}}{\sin(\theta)}=0$
And thus is implemented as a symmetric boundary condition on
$\frac{U_{\phi}}{\sin(\theta)}$

\subsection{Normal-field-radial boundary condition}
This boundary condition is mostly used to set a boundary condition on
the magnetic field. This implies that the normal component of the
magnetic field at the boundary is left unconstrained, and the two
tangential components of the field is set to zero. At the radial boundary,
in terms of the vector potential, this is implemented as:
\begin{equation}
B_{\theta} = \frac{1}{r\sin(\theta)}\partial_{\phi} A_r - \partial_r A_{\phi} 
            - \frac{1}{r}A_{\phi} = 0 
\end{equation}
and 
\begin{equation}
B_{\phi} = \partial_{r}A_{\theta} - 
            \frac{1}{r}\partial_{\theta}A_{r} 
      + \frac{1}{r} A_{\theta} = 0 .
\end{equation}
Which are set by setting $A_r=0$ and 
$\partial_r A_{\phi} + \frac{1}{r}A_{\phi}=0 $ and 
$\partial_r A_{\theta} + \frac{1}{r}A_{\theta}=0 $ which are set by 
setting anti-symmetric condition for $A_r$ and symmetric condition
for $rA_{\theta}$ and $rA_{\phi}$. 

On the boundary with constant $\theta$ is condition is set by 
\begin{equation}
B_{r} = \frac{1}{r}\partial_{\theta}A_{\phi}- 
    \frac{1}{r\sin(\theta)}\partial_{\phi}A_{\theta} 
      + \frac{\cot(\theta)}{r} A_{\phi} = 0 \/.
\end{equation}
and 
...
B_{\phi} = \partial_{r}A_{\theta} - 
      ...
      + \frac{1}{r} A_{\theta} = 0 .
\end{equation}
which can be achieved by setting $A_{\theta} = 0$ and
$\partial_{\theta}A_{\phi}+\frac{\cot(\theta)} A_{\phi} = 0$
and 
$\partial_{\theta}A_{r}=0$. These can be achieved by setting the
antisymmetric condition for $A_{\theta}$, symmetric condition for
$A_r$ and symmetric condition on $\sin(\theta)A_{\phi}$. 
...
\end{equation}i.e., proportional to the density gradient, in the opposite direction. This leads to the usual Laplacian diffusion 

\begin{equation}
  \label{eq:diff}
  \pderiv{\rho}{t} = D\Laplace{\rho}
\end{equation}under the assumption that the diffusion coefficient $D$ is isotropic. Higher order hyperdiffusion of order $2n$ involves a generalization of Eq.~(\ref{eq:fick}), to 


\begin{equation}
  \label{eq:hyperfick}
  \vec{\mathcal J}^{(n)} = (-1)^n D^{(n)} \grad^{2n-1} \rho,
\end{equation}In our case, we are interested in the case $n=3$, so that the hyperdiffusion term is
...
quenching. Considering a wave as a Fourier series in one dimension ($x$),
...
we have the dispersion condition $i\omega = -D k^2$. The sixth order
version (\ref{eq:hyperdiff}) yields $i\omega = -D^{(3)} k^6$. Equating
both we have  $D^{(3)} = D k^{-4}$. This condition should hold at the
grid scale, where $k=\pi/\Delta x$, therefore
...
\end{equation}where we used the Coulomb gauge $\Div{\Av}=0$. The algebra is the same as above, also yielding $\eta^{(3)}=\eta({\Delta}x/\pi)^4$. For entropy, the heat conduction term is 

\begin{equation}
  \pderiv{S}{t}=\frac{1}{\rho T}\Div\left(K\grad{T}\right),
\end{equation}and requiring that $K$ be constant, we substitute it by 

\begin{equation}
  \pderiv{S}{t}=\frac{K^{(3)}}{\rho T}\nabla^6T.
\end{equation}also with $K^{(3)}=K({\Delta}x/\pi)^4$.
...
\begin{equation}
  \label{eq:strain}
S_{ij} = \frac{1}{2}\left( \pderiv{u_i}{x_j} + \pderiv{u_j}{x_i} - \frac{1}{3}\delta_{ij}\Div{\uv}\right)
\end{equation}which only allows the viscous acceleration to be reduced to the simple formulation $\nu\Laplace{\uv}$ under the condition of incompressibility and constant dynamical viscosity $\mu=\nu\rho$. Due to this, the general expression for conservative hyperviscosity involves more terms. In some cases, it is no great overhead, but for others, simpler formulations can be applied.
...
\end{equation}where the $n^{\rm th}$-order rate of strain tensor is 
...
\begin{equation}
S^{(3)}_{ij} = \frac{1}{2} \left( \pderivn{u_j}{x_i}{5} + \pderivn{}{x_i}{4}\left(\pderiv{u_i}{x_j}\right) - \frac{1}{3}\pderivn{}{x_i}{4}\left(\Div{\uv}\right) \right)
\end{equation}

Plugging it into Eq.~(\ref{eq:hypervisc}), and assuming $\mu_3={\rm const}$

\begin{equation}
  \label{eq:hyper-mu-const}
f_{\rm visc}^{({\rm hyper})} = \nu_3 \left(\nabla^6\uv + \frac{1}{3}\nabla^4({\grad({\Div{\uv}})}) \right) 
\end{equation}


For $\nu_3={\rm const}$, we have to derivate the density as well

\begin{equation}
  \label{eq:hyper-nu-const}
f_{\rm visc}^{({\rm hyper})} = \nu_3 \left(\nabla^6\uv + \frac{1}{3}\nabla^4({\grad({\Div{\uv}})}) + 2\Strain^{(3)}\cdot\grad{\ln\rho} \right) 
\end{equation}


\subsubsection{Non-conservative cases}

Equations (\ref{eq:hyper-mu-const}) and (\ref{eq:hyper-nu-const}) explicitly conserve linear {\em and} angular momentum. Although desirable properties, such expressions are cumbersome and numerically expensive, due to the fourth order derivatives of $\grad({\Div{\uv}})$. 


This term, however, is only important when high compressibility is present (since it depends on the divergence of $\uv$). In practice we drop this term and use a simple hyperviscosity 

\begin{equation}
  f_{\rm visc} = \left\{ 
  \begin{array}{l r}
    \nu_3 \nabla^6\uv   &\mbox{if $\mu={\rm const}$}                                                  \\
    \nu_3 \left(\nabla^6\uv + 2\Strain^{(3)}\cdot\grad{\ln\rho} \right)   &\mbox{if $\nu={\rm const}$} 
  \end{array}
\right.  
\end{equation}
...
\end{equation}so it does conserve linear momentum. It does {\it not}, however, conserve {\it angular} momentum, since the symmetry of the rate-of-strain tensor was dropped. Thus, vorticity sinks and sources may be spuriously generated at the grid scale.
...
S_{ij} = \frac{1}{2} \left(\pderivn{u_i}{x_j}{5} + \pderivn{u_j}{x_i}{5} \right)
\end{equation}

%S5 = this plus that 

This tensor, however, is not traceless, and therefore accurate only
for weak compressibility. It should work well if the turbulence is
subsonic. Major differences are not expected, since the spectral range
in which hyperviscosity operates is very limited: as a numerical tool,
only its performance as a high-frequency filter is needed. This also
supports the usage of the highest order terms only, since these are
the ones that provide quenching at high $k$. Momentum conservation is
a cheap bonus. Angular momentum conservation is perhaps playing it too
safe, at great computational expense. \index{Hyperviscosity}
...
Eq.~(\ref{eq:diffrho-hyper}). Because of this, {\it isotropic}
hyperdissipation only gives equal quenching in all spatial directions 
if $\Delta x$=$\Delta y$=$\Delta z$, i.e., if the cells are cubic. For 
non-cubic cells, anisotropic dissipation is required as different 
directions may be better/worse sampled, thus needing less/more numerical 
smoothing. Such generalization is straightforward. For that, we 
replace Eq.~(\ref{eq:hyperfick}) by

\begin{equation}
  \label{eq:general-fick}
  \vec{\mathcal J}=\left(D_x \pderivn{\rho}{x}{5}, D_y \pderivn{\rho}{y}{5}, D_z \pderivn{\rho}{z}{5}\right),
\end{equation}
so that different diffusion operates in different directions. Since $D_x$,
$D_y$ and $D_z$ are constants, the divergence of this vector is 
...
\begin{equation}
  ...
  D_z \pderivn{\rho}{z}{6}.
\end{equation}
The formulation for resistivity and heat conductivity are strictly the same. For viscosity it also
assumes the same form if we consider the simple non-conservative rate-of-strain tensor (\ref{eq:weird-strain}). 

Mathematically, these operations can be written compactly by noticing that the
coefficients in Eq.~(\ref{eq:anisodiff}) transform like diagonal tensors $\chi_{ij}^{(3)}=\chi_k^{(3)} \delta_{ijk}$,
where $\delta_{ijk}$ is the unit diagonal third order tensor, $\mathbf{\chi}^{(3)}$ is the
...
&testfield_run_pars
  ...
  itestfield='B11-B22', leta_rank2=T
  etatest=1e-4
  lsoca=F
/
\end{alltt}
...
\item Once it works and it running, you want to remove (or uncomment)
like this:
\begin{alltt}
&run_pars
  ...
  ! lread_oldsnap_notestfield=T
/
\end{alltt}
...
\midrule
  \var{aa}    & magnetic vector potential $\Av$;
                writes \file[]{a[xyz].\{xz,yz,xy,xy2\}}  \\
  \var{bb}     & magnetic flux density $\Bv$;
                writes \file[]{b[xyz].\{xz,yz,xy,xy2\}}  \\
...
% LocalWords:  fF Xemacs fortran el failsafes daainit etatest FBCX yx
