...
{Use of more than one special module} at a time requires that the environment
...
with its details depending on the Fortran compiler used. They can be learned by employing the {\it nm} command, say, by {\it nm src/general.o | more}.
The environment variables are most conveniently set in the users {\tt .bashrc} or a proper configuration file of the {\sc Pencil Code}.
In the {\tt Makefile.local} file the requested special modules are simply specified as a list of names as\\
{\tt Special = special/$\langle${\it module 1}$\rangle\langle${\it module 2}}$\rangle\ldots}$.
