...
%%  CVS: $Id: manual.tex,v 1.68 2002-07-23 13:53:11 dobler Exp $
%%  Description:
...
%WD: Moved it up again, as it belongs to the same group as dampu
  \var{rdamp} [$1.2$], \\
  \var{wdamp} [$0.2$]
...
interface}) for a simple domain decomposition (data-pararellism), which is
...
\item Edit \file{src/cparam.local} and replace
  ...
  and $z$ direction.
  The second line increases the resolution of the run because on most
  architectures you won't gain much by running a problem as small as
  $32^3$ on 16 CPUs (even $64^3$ may still be quite small in that
  respect).
...
For weakly compressible flows, there is little difference between the
simplified viscous operator (\code{ivisc='simplified'}; see
\S\ref{S-Eqn-of-motion}) and the full operator
(\code{ivisc='nu-const'}), while the latter is numerically considerably
more expensive.

Strictly speaking, however, the equation of motion with the simplified
...
$ $Id: manual.tex,v 1.68 2002-07-23 13:53:11 dobler Exp $ $}
