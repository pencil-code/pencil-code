...
%%  CVS: $Id: manual.tex,v 1.300 2005-08-23 16:15:31 bingert Exp $
%%  Description:
...
%\date{$ $Date: 2005-08-23 16:15:31 $ $,~ $ $Revision: 1.300 $ $}
...
\begin{titlepage}
  ...
  \emph{$ $Date: 2005-08-23 16:15:31 $ $,~ $ $Revision: 1.300 $ $}\\
...
Heinemann, T., Dobler, W., Nordlund, \AA., \& Brandenburg, A.\sana{2005}
{Radiative transfer in decomposed domains}
(\astroph{0503510})

Dobler, W., Stix, M., \& Brandenburg, A.\sapj{2005}
...
\begin{equation}
\zeta_{\rm shock}=c_{\rm shock}\left<\max_5[(-\Div\uv)_+]\right>*min(\delta x,\delta y,\delta z),
\end{equation}
...
at the position of the particle. A simple first order interpolation scheme is
used to aquire this velocity outside the grid points.
...
\begin{equation}
  \frac{\partial \vec{x}_i}{\partial t} = \vec{v}_i \, ,
\end{equation}
...
gas).
\begin{equation}
  \frac{\partial \vec{v}_i}{\partial t} = \ldots -\frac{1}{\tau_{\rm s}}
  (\vec{v}_i-\vec{u}) \, .
\end{equation}
Here $\tau_{\rm s}$ is the stopping time of the dust particle. A first order
interpolation scheme is used to calculate the gas velocity $\vec{u}$ at the
position of the particle. The particle description is the proper description of
...
The sample run \file{samples/random_uu_particles} contains the latest setup for
dust particles.
...
the same time (hopefully), there is some memory optimisation in making
...
\begin{enumerate}
  ...
  \item The index number of the receiving processor is then calculated. This
      ...
      to neighbouring processors as the global boundary conditions may send
  ...
  \item Once the number of migrating particles is known, this information is
      shared between the processors so that they all know what they have to
      receive.
  ...
  \item Once a particle has been received, the code checks if it really belongs
      to the local $(x,y,z)$-interval. If not, the code dies with a warning.
      This has turned out to be valuable for debugging.
\end{enumerate}
...
\section*{APPENDIX
  \hfill\normalsize\rm$ $Date: 2005-08-23 16:15:31 $ $,~ $ $Revision: 1.300 $ $}
...
$ $Id: manual.tex,v 1.300 2005-08-23 16:15:31 bingert Exp $ $}
