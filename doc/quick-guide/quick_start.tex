...
Some 5--10 years ago, this was the procedure (nowadays you either have gfortran or install it with brew):
...
If you get complaints about sed at the start of the compilation try installing the gnu version of sed:
\begin{verbatim}
  brew install gnu-sed
\end{verbatim}
and add it to the path so it takes presedence:
\begin{verbatim}
...
Similarly, this is incorporated inside \code{pc\_start/pc\_run} and you do not need to remember to source the modules when using them.
...
In this case, try to turn off CUDA-aware MPI by adding the following to your build config
\begin{verbatim}
  CUDA_AWARE_MPI=OFF
\end{verbatim}
.
