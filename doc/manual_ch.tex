...
%   Process with:
%     This file needs to be handled by full version of Texlive, which takes the package of Chinese. xelatex is required. Normally, one may need to complie twice to make the contents complete.
%
...
这一代码包一开始是在一个暑期学校 Turbulence Summer School of the
Helmholtz Institute in Potsdam (2001) 期间发展起来的.
虽然一些流体力学和吃流体力学的 SPH 和 PPM 的代码都是公开的,但这不代表高阶微分法或谱分析代码也被公开了.
已经有不少对此感兴趣的朋友接触过我们,基于此,我们决定让其使用起来更灵活并尽可能地对用户友好一些,于是这个程序包便被放到了公有 \name{CVS} 库上.
自从 2008年9月21号起,它便通过 \url{https://github.com/pencil-code/pencil-code} 发布了出来.
\url{https://github.com/pencil-code/pencil-code}.
这段代码不应该被视作黑箱,为了用一种较为优化的方式解决新出现的问题,用户将会需要自己配置合适的参数.
特别的,你需要小心地对一些参数选择正确的数值,比如说粘度,磁扩散率和辐射热传导率等.


{\sc Pencil Code} 主要用来解决弱可压缩的湍流问题,这也是为什么我们用了高阶的一次和二次导数.
为了实现较好的并行化预算,我们使用了显式有限差分法(与之相对的是紧致的有限差分法).
本程序包典型的科学目标包括周期边界条件长方体内受迫的 MHD 湍流;平面板上下边界条件非周期性,研究对流运行;嵌在完全非周期盒中的对流星(翻译存疑a convective star embedded in a fully nonperiodic box);吸积盘湍流的剪切片近似等等.
此外,我们还有非局域辐射输运,惯性粒子,尘埃凝聚,自引力,化学反应网络等多种物理元件,其数量在稳步增加.
除了笛卡尔坐标系外,该代码还可以处理球极坐标系和柱极坐标系.


磁场是通过磁矢势来实现的,这可以保证磁场维持环形,而不夹杂任何散度.
同时,用磁矢势还有一个巨大的好处,例如,当研究者想要控制磁场螺旋度时,磁矢势会非常方便. 从而使得本程序包非常适合于各类 dynamo (翻译存疑)问题.


{\sc Pencil Code} 一般来说不保证守恒量守恒;所以,守恒量的稳定性将会依赖于算法的离散误差(不是指机器精度).
...
输入参数(在 \file{start.in},
\file{run.in} 中配置好的)修改后无需再编译.甚至用户还可以在数值模拟过程中修改输出的诊断变量种类,还有相关程序可以实现运行中重新载入参数或者热退出(exit gracefully).你可以通过命令 \code{pc_configtest} 来检验这些文件的正确性.

使用 Pencil-MPI 代码的要求不高: 你可以在任何一个带了 \name{F95} 和 \name{C} 编译器的 Linux 或者 Unix 系统上运行.比如说 \name{GNU gcc} 和 \name{gfortran} 编译器就可以.另外还需要 shell \name{CSH} 和 \name{Perl} 解释器.

尽管  {\sc Pencil Code}  主要设计在超级计算机上运行,但也有超过 50\%  的用户直接在 Macs 上运行代码,另一半用户在他们的
 Linux 笔记本电脑系统上运行或者在 Windows 上安装 Ubuntu Linux 虚拟机.
If you have \name{IDL} as
...
如果你也有 IDL,你就能够便捷地可视化运算结果(我们有大量的样例),但其他的工具比如说 也能用,
相关的工具和程序和 放在一起.


\bigskip

如果你想要充分地,富有开创性地利用这个程序包,这份文档的内容还不充分.
它主要的目标是告诉你程序的组织架构,让你能够有效率地 \emph{阅读源码},源码中包含了更多详尽的注释.
你可能会需要看一遍样例文件夹里面的文件.
选择一个最贴近你应用的样例并着手修改.
如果你希望给 {\sc Pencil Code} 一些新功能,你可以把代码中已经实现的相关变量,函数,诊断,方程等拿来当例子.
记住一点: 当你想要理解一些特定的变量或者是函数时, \cmd{grep} 能给你很大的帮助.

\bigskip

我们将会非常开心地接纳来自用户的代码改进,欢迎任何形式的反馈.
...
已经有许许多多的人对 {\sc Pencil Code} 做出了贡献. 我们在此首先感谢 {\AA}ke Nordlund (Copenhagen Observatory)
和 Bob Stein (University of Michigan),他们首先提出了将高阶差分方法用在可压缩流体里,并且他们还教导了其他人许多关于模拟的知识.

功率谱,结构函数,再生成网格,改变处理器数量的程序以及
剪切片近似和辐射传输的极限扩散近似的工作都是 Nils Erland L.\ Haugen (University of Trondheim) 完成的.
Tobi Heinemann added the long characteristics method for
radiative transfer as well as hydrogen ionization.
Tobi Heinemann 添加了针对辐射传输的长距离特性方法(翻译存疑)以及氢元素电离方法.
...
Philippe-A. Bourdin 实现了 HDF5 的支持并且为高性能运算多样的数据传输过程撰写了输入输出模块.
他还贡献了日冕模块并且做了 IDL GUI 方面的工作,包括让 IDL 读入并处理大规模数据的程序.
...
\part{使用代码 Using the {\sc Pencil Code}}

\section{系统要求 System requirements}

为了能够运行这个程序包,你需要有一些下面列出的东西:
...
\item 经常需要用到的(如果你没有其中之一,你需要手动修改许多细节):
  \begin{itemize}
...
\item 接下来的是一些可有可无的东西,但或多或少会增强代码功能:
  \begin{itemize}
...
\section{获得源码 Obtaining the code}
\index{Download}
\label{Download}
...
\index{svn}
不管你第一次安装程序包通过什么方式(from tarball or via \name{svn/git}),你都可以在之后通过 \name{svn/git} 更新你的程序版本,
如果你对你的程序做了实质上的改变,那就要小心这个升级过程产生矛盾.尽管 \name{svn/git} ,是一个极好的分布式编程工具. 但因为我们中的每一个都在发展代码的过程中接触大量的源码,所以依然会时不时的会出现毛病. 尽管这份源码是通过 \name{svn/git} 来发展,你也应该在每次更新之前备份好你说做出的重要的改变.
...
如果你已经做出了一些有意义的修改,那就可以联系贡献者列表中的任何一人了,他们在这个网址下列出来了 ,他们可以给你 push 或者 check-in 的权限.
...
请务必在代码中写上足够多的注释,并遵循我们标注的代码风格.我们的代码风格请参见 .
...
\subsection{取得最新的有效版本 Getting the last validated version}
\index{svn}
...
\subsection{取得更老版本的代码 Getting older versions}
\index{svn}
...
\section{开始吧! Getting started}
\label{S-getting-started}

如果你要开始的话,你需要先拿 \file{samples/} 内的样例下手.
注意你只有将数据用\name{IDL}, \name{DX} 或其他工具(see
Sect.~\ref{S-Visualization})可视化,你才能得到一个对数值计算结果充分展现的效果.
...
\subsubsection{环境设置 Environment settings}
...
\end{alltt}
对于 \name{tcsh} 或 \name{csh} 使用者而言; 对\name{bash}, \name{Bourne shell}, or similar shells的使用者而言,需要以下语句:
\begin{alltt}
...
\end{alltt}
除了 \env{PATH} 变量以外,环境变量\env{IDL_PATH} 也被设置为类似这样的形式:
...
来作为你的 \dfn{run directory}(即运行三层的层状结构来模拟太阳对流).
...
\subsubsection{调整\file{Makefile.src} Adapting \file{Makefile.src}}
\index{Makefile}

这一步骤需要一些你的输入,但它仅仅需要在电脑上看见一次就足够了.
请参看 Sect.~\ref{adapt-mkfile} 这里所需要进行的步骤的描述,

\paragraph{Note:} 如果你幸运地和我们用的类似的编译器,那么这一步你很有可能可以跳过去.但如果不能编译的话,就需要你自己检查一下了. 如果用的不是类似的编译器的话,你可以运行 \cmd{make} 并指定一些标记,这部分请参看 Sect.~\ref{S-make-flags} ,具体的标记会在出现在表格  Table~\ref{Tab-compiler-options} 内.

\subsubsection{执行 \cmd{make} 命令 Running \cmd{make}}

下一步,在你的运行目录的子目录 \file[src/]{src} 里面运行 \cmd{make}.
因为你正在用一个之前定义好的问题来做测试,在 \file[Makefile.local]{src/Makefile.local} 里面的设置和 \file[cparam.local]{src/cparam.local} 都是有效的.那么你现在只需要运行下面的代码.

\begin{alltt}
  \prompt{unix> } make \
\end{alltt}

如果已经正确的配置好了这个编译器的标记(flags).那边编译将会成功完成.


\subsubsection{选定存储数据的文件夹Choosing a data directory}

默认情况下程序会将数据结果切片写入到工作目录下的子目录 \file[data/]{data}.
因为这涉及到大量的输入输出操作(至少对于大尺度网格是这样的),一般我们会建议接近尽量避免用 NFS 手段写入数据.推荐的配置数据文件夹 \file[data/]{data} 的方法是生成一个磁盘中对应的目录,将它软链接到你的工作目录下的 \file[data/]{./data} 文件夹.即使这个链接是 NFS 目录的一部分,所有的输入输出操作也会是局部的.

例如,如果你有一个本地磁盘 \file[/scratch/]{/scratch} 你可以运行下面的代码:
...
这是由 \cmd{pc_mkdatadir} 命令自动完成.而 \cmd{pc_mkdatadir} 又会在比如说新建一个工作文件夹 \cmd{pc_newrun} 的时候被调用.

即使你不想用 NFS 型的文件夹(比如在你的笔记本电脑上),我们仍然推荐你将代码和数据通过一个类似上述的模式来分.

符号链接的一个替代选项是设置一个文件 \file{datadir.in} 在工作目录的根目录下,这个文件应该包含一行文本说明绝对或相对的数据目录存放的地址.如果使用者希望将它的工作目录在不同的数据目录下切换的时候,这将非常有用.
在这样的情况下符号链接将会在不同的工作目录下重复生成,所以 \file{datadir.in} 应该只包含一个简短的相对路径.


\subsubsection{运行代码 Running the code}
...
\subsection{更多的测试 Further tests}

在 \file{samples/} 文件夹内有大量的其他测试.你可以用脚本 \file[auto-test]{bin/auto-test} 来自动运行这些测试程序并且将输出与参考结果作对比.
...
所有代码的目录结构已经展示在 Fig.~\ref{Fig-Structure} 内.
在 \file[pencil-code/]{pencil-code} 目录下, 现在有以下文件和文件夹:
\begin{verbatim}
bin/   config/  doc/  idl/  license/  perl/   samples/      sourceme.sh  utils/
bugs/  dx/   lib/  misc/     README  sourceme.csh  src/         www/
\end{verbatim}

几乎所有的代码都已经包含在文件夹 \file{src/} 内.但为了对单独的程序设置依然能方便地处理,运行的程序代码会复制到运行文件夹下的\file[src/]{src}内,
 如 e.\,g.~\file[conv-slab/src/]{samples/conv-slab/src}.
...
目录 \file{forced/} 包含了一些受迫湍流的运行代码(有磁化的也有未经磁化的);
\file{gravz/} 包含了垂直重力场下的运行代码;
\file{rings/} 包含一些衰减磁流体问题 (如互锁的流环作为初始条件 interlocked flux rings);
...
\subsubsection{Pencils 中的数据访问 Data access in pencils}
\index{Pencil design}

不像 80 年代和 90 年代早期作为超级计算领域主角的 CRAY 计算机,所有的现代计算器都有缓存,而这原是引起许多程序运行的瓶颈.

这意味着在有缓存的条件下,如果大型三维数组每一步都会进行计算,那会带来巨大的处理超长数组和矢量运算的优势.
这一设计也使程序设计更为简化,扩展了符合 F90 数组设计的应用.然而,一个更有效利用缓存的方法是一次沿着一个维度pencil的数据计算完一个时步(或者一个多时步计划中的一小步).
...
\begin{verbatim}
  ...
  FOURIER   = nofourier
\end{verbatim}
This file will be processed byand the settings are thus
assignments of \name{make} variables.
...
除了物理模块之外(以上样例子中,空行上方的为物理模块),
一些技术模块也可以被选择为使用或停用；在上面的例子中,这些是MPI(关闭),附加的全局变量(没有),输入/输出(分布式)和FFT(未使用).教派的桌子.？？？在附录中

列出所有当前可用的模块.

请注意,大多数这些make变量必须被设置,但它们通常会在“MaMaFILE”中获得合理的默认值(因此,您只需要在“MaxField.Load”中设置非标准的值).

这是通过使用这种切换机制,使我们获得高灵活性,而不诉诸过多的密码预处理器指令或其他代码内的开关.

模块的许多可能组合已经被测试,实例是分布的一部分,但是您可能感兴趣的是以前从未尝试过并且可能还没有工作的组合,因为模块不是完全正交的.在这种情况下,我们依赖于用户反馈来修复问题并记录他人的更改.


% ---------------------------------------------------------------------- %

\subsection{run 文件夹内的文件}

\subsubsection{\file{start.in}, \file{run.in}, \file{print.in}}

这些文件指定启动和运行时的参数(参见 Sects.~\ref{S-start-params} 和 \ref{S-all-run-params}),
并设置需要诊断的变量输出 (参见 \ref{diagnostic-IO}).
这些参数明确了所需模拟的配置并且放在 \file[samples/]{samples} 文件夹内并用 \name{svn}\index{svn} 同步.


\subsubsection{\file{datadir.in}}
\label{Ss-datadir-in}

如果这个文件存在的话,它必须包含一个现有文件夹的名字,而该文件夹将被作为 \dfn{data directory} 文件夹使用,换句话说,所有数据输出的地方.如果 \file{datadir.in} 不存在, 数据文件夹默认为 \file{data/}.

\subsubsection{\file{reference.out}}
如果存在的话,\file{reference.out}指定您应该在run目录中获得的输出,
只要您没有更改任何参数.如果想检查你的运行结果是否成功,可以比较\file{time_series.dat} 和\file{reference.out}:
\begin{alltt}
...
这些文件是通往 \file[bin/]{\$PENCIL_HOME/bin} 的链接.
你会常常用到 \file{start.csh} 和
\file{run.csh} 来使程序初始化.
...
\file[src/]{src} 文件夹包含两个本地(local,局部？)文件,
\file[Makefile.local]{src/Makefile.local} 和
\file[cparam.local]{src/cparam.local}, 让使用者可以方便地选择模块(see \ref{S-modularity})并设置参数,比如设置网格大小和每个方向上用到的处理器
这两个文件是一个模拟的设置参数,他们在 \file[samples/]{samples} 文件夹内,通过  \name{svn} 同步.

 \file[cparam.inc]{src/cparam.inc} 是通过脚本 \file{mkcparam} 自动生成的,它包含了一个明确的设置所需要的基本变量数.

所有 \file{src/} 内的其他文件要不就是到位于\file[src/]{\$PENCIL_HOME/src}内的源文件的链接(and \file{Makefile.src}) ,要不就是编译器产生的对象或模块文件.

\subsubsection{\file{data/ }}

这个文件夹存储程序的运行结果：(但如果\file{datadir.in}存在的话,该文件夹名会被\file{datadir.in}第一行指定,参见
\S\ref{Ss-datadir-in})

\paragraph{\file[dim.dat]{data/dim.dat}}
全局数组的维度 The global array dimensions.

\paragraph{\file[legend.dat]{data/legend.dat}}
首行指定了输出在\file{time_series.dat}中的诊断变量名字.

\paragraph{\file[time_series.dat]{data/time_series.dat}}
诊断变量的时序表 (also printed to stdout).
你可以直接使用这个文件绘图,通过 \name{Gnuplot},
\name{IDL}, \name{Xmgrace} 或其他类似工具 (see also
...
% ====================================================================== %
    ...
    \subsection{开始并运行参数 Start and run parameters}
    \label{S-start-params}
    所有的\file{start.in} 和\file{run.in}里面的输入参数都集合在 Fortran \name{namelists}内.
    ...
    \file[helical-MHDturb]{samples/helical-MHDturb} 文件内的运行参数如下
    \begin{alltt}
      &run_pars
        cvsid='\${}Id:\$',                 \textsl{! identify version of start.in}
    ...
    这里我们运行 \var{nt}$=10$ 个时间步长, 每隔一步输出一次诊断; 
    我们要求时间步长能使得对流
    \name{Courant 数} $\le 0.4$ 和扩散 \name{Courant 数}
    ...
    我们会写入 \file{VAR$N$} 一些需要记录的参数,每隔 \var{dsnap}$=50$ 个时间单位
    和可供视频化的二维数据记录每 \var{dvid}$=0.5$ 个时间单位.
    并且,我们使用具有确定性的随机数生成器(deterministic random number generator).
    粘度 $\nu$ 和磁场扩散系数 $\eta$
    设为 $5\times10^{-3}$ (所以网孔雷诺数(mesh Reynolds number)将近为
    $u_{\rm rms}\delta x/\nu=0.3\times(2\pi/32)/5\times10^{-3}\approx12$,
    这个数值有些偏高).
    \name{forcing_run_pars} 里的参数指明了螺旋的力的幅度.
    
    \bigskip
    
    完整的输入参数在附录 \ref{S-all-parameters} 内.
    ...
    许多计算并不依赖于单位,也就是说所有结果都依赖于你用于解释计算结果的单位制.
    例如,如果你在一个长度 $L=1$ 箱子里模拟一束简单的流体并且在 $t=3$ 个时间单位后
    得到了它的最大速度 $u_{\rm max}=0.5$.这可以解释为
    $L=\unit[1]{m}$, $u_{\rm max}=\unit[0.5]{m/s}$, $t=\unit[3]{s}$,
    也可以是 $L=\unit[1]{pc}$, $u_{\rm max}=\unit[0.5]{pc/Myr}$, $t=\unit[3]{Myr}$,
    ,取决于你用的单位制.你所使用的单位需要保持一致,所以在上面的第二个例子中,扩散系数的单位应该是
    $\unit{pc^2/Myr}$.
    
    磁感应强度和温度的单位会通过在内部设置 $\mu_0=1$ , $c_p=1$来得到\footnote{
    ...
    \begin{table}[htbp]
      \centering
      \caption{通过 Eqs.~(\ref{unit-B}) and
      (\ref{unit-T}) 和确定的$[\rho]$, $[v]$ 得到的磁感应强度和温度单位.
    ...
    对一些密度和速度单位,表 \ref{Tab-units-B-T}
    已经展示了最后得到的磁感应强度和温度单位.
    
    另一方面,一旦使用了材料方程,例如:
    辐射损耗的常用参数,Kramers 不透明度,
    Spitzer 导电性或电离度(这表征良定义的电离能),在代码中对应的例程需要知道你正在使用的单位.
    这些信息在 \file{start.in} 或 \file{run.in} 里面可以通过 \var{unit_system},
    \var{unit_length},  \var{unit_velocity}, \var{unit_density} 和
    ...
    Note: 默认的单位制是 \code{unit_system='cgs'},它也可以写为 \code{unit_system='Babylonian cubits'}.
    ...
    \subsection{粘度存在最小允许值}
    \index{Viscosity}
    \index{Shock viscosity}
    \label{viscosity}
    
    我们强调,默认条件下,这个程序都会将扩散系数视为常数(粘度 $\nu$, 热扩散系数 $\chi$, 磁扩散系数 $\eta$
    及被动标量扩散系数 passive scalar diffusivity ${\cal D}$).
    如果这些量中某些量过小,那么你需要更多的格点以使之得到合理的数值计算结果,否则最后程序运算结果就会越来越不稳定.
    基于最大速度的网格雷诺数可以给出一个有用的判据,
    \begin{equation}
      \mbox{Re}_{\rm mesh}=\max(|\uv|)\max(\delta x,\delta y,\delta z)/\nu,
    \end{equation}
    根据具体的问题,网格雷诺数不能超过某个具体的值.
    $\mbox{Re}_{\rm mesh}$ 网格雷诺数最大的值常常在 5 左右.
    类似的 mesh P\'eclet 和 mesh magnetic Reynolds numbers 也不能太大.
    
    Note that in some cases, `wiggles' in $\ln\rho$ will develop despite
    sufficiently large diffusion coefficients, 本质上是因为连续性方程不包含耗散项.
    我们对于包括但不限于对流的情形可以通过\name[Upwinding]{upwinding} 来抑制这种现象, 参见 Sect.~\ref{S-upwind}.
    ...
    \subsection{时间步长 The time step}
    \label{time-step}
    \index{Runge-Kutta time step}
    \index{Time step}
    
    \subsubsection{常规 RK-2N 时间步长 The usual RK-2N time step}
    
    RK-2N refers to the third order Runge-Kutta scheme by Williamson (1980)
    with a memory consumption of two chunks.
    RK-2N 指的是三阶龙格-库塔算法(Williamson 1980),它有着两个代码块(翻译存疑)的空间复杂度,所以名字里有2N.
    ...
    \end{equation}
    $\cs$ 和 $\vA$ 分别表示声速 Alfv\'en 速度;
    \begin{equation} \label{Dmax}
      D_{\rm max} = \max(\nu,\gamma\chi,\eta,D) ,
    \end{equation}
    公式中 
    $\nu$ 表示动粘度(翻译存疑) denotes kinematic viscosity,
    $\chi = K/(c_p\rho)$ 表示热扩散率而 $\eta$表示磁扩散率;
    and
    ...
    helps to recognize the problem. 下面有个图片展示的例子
    Fig.~\ref{Ftimestepoverviscous}.
    
    \begin{figure}[h!]\begin{center}\includegraphics[width=.99\textwidth]
    {timestepoverviscous}\end{center}\caption[]{时间步长过长的速度数据切片例子.
    ...
    $\nu_3=5\times10^{-12}$. 流体粘度的解释参见 Appendix~\ref{high-freq-filters}.}\label{Ftimestepoverviscous}\end{figure}
    
    通过 Runge-Kutta 2N scheme 时步法得以实现.细节参见 Sect.~\ref{S-2N-scheme}.
    
    \subsubsection{The Runge-Kutta-Fehlberg time step}
    \index{Runge-Kutta-Fehlberg time step}
    
    五阶的 Runge-Kutta-Fehlberg 时步法是可以使用的.它通常用来解决化学问题,常常需要双精度选项.
    为了使之运行,你需要调整 \file{src/Makefile.local}  里的
    \begin{verbatim}
    TIMESTEP  =   timestep_rkf
    \end{verbatim}
    另外还需要在 \file{run.in}  里面设置\cmd{itorder=5}
    一个使用它的样例是 \file{samples/1d-tests/H2_flamespeed}.
    但这种方法还在测试中.
    ...
    \subsubsection{指定边界条件的文件}
    \label{S-boundconds-where}
    
    自然地,在大部分 {\sc Pencil Code}  的测试中,边界条件常常在 \file{run.in} 内设定好.
    ...
    zones assuming periodicity (默认的边界条件) in all three
    directions.
    这些 ghost data 在计算中从不会被调用,而 \file{run.x} 会在调用 ghost-zone 的数据之前调用边界条件.
    
    为了避免你所设置的初始条件出现在你的输出文档内,你可以将它们设置在文件 \file{start.in} 内.
    这样,它们会被 \file{run.x} 所运行, 除非你又在\file{run.in}内进行设置,否则不会出现问题.
    
    
    \subsubsection{如何指定边值条件}
    \index{Ghost zones}
    
    对MPI程序来说,用 ghost zones 来代表相邻单元处理器所模拟区域的数值是很自然的,于是边界条件可以通过
    指定每个边界上的三层 \name{ghost points} 来实现(three layers of
    \name{ghost points} on either boundary).
    合适的边界条件选择是在\file{run.in}里设置
    \var{bc\{x,y,z\}}
    \index[var]{bcx@\emph{bcx}}%
    \index[var]{bcy@\emph{bcy}}%
    \index[var]{bcz@\emph{bcz}}%
    ; 术语使用说明如下.
    给 \var{bc\{x,y,z\}} 设置一串字符来设置周期性的边界条件
    \begin{alltt}
      bcx = 'p','p','p', 'p',  'p'
    \end{alltt}
    或者
    ...
    设置了非周期性的边界条件.
    每一个字母对应着
    $u_x$, $u_y$, $u_z$, $\ln\rho$, $s/c_p$, $A_x$, $A_y$, $A_z$, $\ln c$ 里其中之一的变量边界条件
    that are actually used \emph{in this order}.
    以下字符是可用的:
    \begin{description}
    \item[\option{p}] 周期边界条件 periodic boundary condition
    \item[\option{a}] 关于边界反对称,即边界数值为零 antisymmetric condition w.\,r.\,t.~the
      boundary, i.\,e.~vanishing value
    \item[\option{s}] 关于边界对称,即一阶导为零  symmetric condition w.\,r.\,t.~the
      boundary, i.\,e.~vanishing first derivative
    \item[\option{a2}] 关于边界上任意值的反对称,即二阶导为零antisymmetry w.\,r.\,t.~the
      arbitrary value on the boundary, i.\,e.~vanishing
      second derivative
    \item[\option{c1}] $\ln\rho$ 和 $s$ 的特殊边界条件: 边界的稳恒热流 special boundary condition for
      $\ln\rho$ and $s$: constant heat flux through the
      boundary
    \item[\option{c2}] $s$的特别边界条件:边界上稳定的温度,该条件需要$\ln\rho$的\code{a2}边界条件 special boundary condition for
      $s$: constant temperature at the boundary --- requires boundary
      condition \code{a2} for $\ln\rho$
    \item[\option{cT}] $s$ 或 $\ln\ T$ 的特殊边界条件:边界常温(对任意设置的$\ln\rho$成立) special boundary condition for
      $s$ or $\ln\ T$: constant temperature at the boundary (for arbitrarily set
      $\ln\rho$)
    \item[\option{ce}] $s$的特殊边界条件: 在边界的 ghost points 上设置温度(对任意设置的$\ln\rho$成立) special boundary condition for
    ...
    \item[\option{she}] 剪切片边界条件(这在\name{Shear}模块被使用时是默认的) shearing-sheet boundary condition (default when the
      module \name{Shear} is used)
    ...
    \item[\option{hs}] 对$\ln\rho$ 和 $s$的强制要求使之在垂直边界上满足流体静力学上的平衡 special boundary condition for $\ln\rho$ and $s$
    which enforces hydrostatic equilibrium on vertical boundaries
    \end{description}
    $a$:$b$ (e.\,g.~`\code{c1:c2}') 这种形式的边界条件意味着:在左/低边界上使用 $a$条件,在右/上边界使用$b$条件.

    
    如果你从一个老的 \file{run.in} 文件出发,修改并另存为了一个新文件,变量的数目改变了(比如计算熵和不计熵的时候, (\name{noentropy} vs.~\name{entropy}),你需要记得调整数组\var{bcx},\var{bcy},\var{bcz}的长度.
    这样做的好处是通过在一个方向上设置新的边界条件,很容易就可以交换所有的边界条件(例如,让所有边界都周期化,或者不要剪切片边界条件切到无压力条件)
\clearpage


% ====================================================================== %

\section{使用程序 Using the code}
\label{Input-params}

\subsection{配置代码以使之编译并运行,Configuring the code to compile and run on your computer}
\label{S-configuration}

\paragraph{Note:} 我们建议不要参考 Sects.~\ref{adapt-mkfile} 和 \ref{start-run-getconf} 中比较老的文档内容,
本章节的内容更合适.
...
这行命令会告诉你正在使用的系统的 \emph{host-ID} :
\begin{alltt}
  \prompt{unix> } pc_build -i
\end{alltt}


\subsubsection{配置文件的结构 Structure of configuration files}
...
\subsubsection{编译源码 Compiling the code}

用 \cmd{pc_build} 命令来编译代码:
\begin{alltt}
...
\subsubsection{跑代码 Running the code}
...
% ---------------------------------------------------------------------- %
\subsection{改变分辨率 Changing the resolution}
...
% ---------------------------------------------------------------------- %
\subsection{使用非等距网格 Using a non-equidistant grid}
...
\subsection{诊断输出 Diagnostic output}
\label{diagnostic-IO}
...
\subsection{视频和截断图片 Video files and slices}
\label{S-slices}
...
\paragraph{另一个例子 Another example}
...
\subsubsection{时间平均 Time averages}
...
\subsection{帮助脚本 Helper scripts}
\index{Scripts}
...
\subsection{重启模拟 Restarting a simulation}
\index{Restarting}


如果你的运行程序跑到一半中断了,你还可以重新运行,从中断的地方继续,它可以从\file[var.dat]{data/proc*/var.dat}里你保存的最新帧的数据处再开始.
最新帧的数据保存在一个单独的文件中 \file[time.dat]{data/proc*/time.dat}.
...
如果你要到另一台计算机上接着运行了,你只需要复制
\file[var.dat]{data/proc*/var.dat} (为了以防万一加上 \file[time.dat]{data/proc*/time.dat}) 到一个新的目录下面.
你也许还需要 \file[seed.dat]{data/proc*/seed.dat}
作为你的随机数生成器. The easiest way to get all these
...
\subsection{一维和二维的运行 One- and two-dimensional runs}
...
\subsection{在多核处理器上跑吧 Running on multi-processor computers}
\label{MPI}
...
\subsection{双精度运算 Running in double-precision}
...
\subsection{结构函数 Structure functions}
...
\subsubsection{并行的粒子 Particles in parallel}
...
\subsubsection{大规模粒子群 Large number of particles}
...
\subsubsection{随机数生成器 Random number generator}
...
\subsection{非笛卡尔坐标系 Non-cartesian coordinate systems}
...
\section{方程 The equations}

 {\sc Pencil Code} 求解的方程基本上都是标准的可压缩磁流体力学方程.不过模块化的代码结构能够允许用户做出一些磁流体力学方程的改变.
你可以关掉某些方程,或者将方程中的某些项消掉(比如说 nomagnetic,
noentropy 等等).

在本章节中方程都会以尽可能完整的形式展现出来.
{\sc Pencil Code} 团队希望能够将这些方程尽可能地实现,或者退而求其次实现其化简版.

% ---------------------------------------------------------------------- %

\subsection{连续性方程 Continuity equation}

在本代码中,连续性方程
$\partial\rho/\partial t+\Div\rho\uv=0$,
是以 $\ln\rho$ 代替 $\rho$ 来写的,
\begin{equation}
  \frac{\De\ln\rho}{\De t}
  = - \Div\uv \; .
\end{equation}
这里 $\rho$ 表示密度, $\uv$ 是流体速度, $t$ 是时间而
$\De/\De t \equiv \partial/\partial t + \uv\cdot\grad$ 是对流导数.(翻译存疑) the convective
derivative.

% ---------------------------------------------------------------------- %

\subsection{动量方程 Equation of motion}
\label{S-Eqn-of-motion}


在动量方程汇总,如果是理想气体的话,压强项可以写作
...
\label{EOSsimple}
\end{equation}
式中 $\gamma=c_p/c_v$ 是比热比,或者说 \emph{绝热指数}.
注意 $\cs^2$ 和温度成正比,因为
$\cs^2=(\gamma-1)c_p T$.
...
\end{equation}
是剪切速率张量,它的矩阵迹等于零,因为它可以写成广义应变率张量减去其自身迹的形式. 如果是在曲线坐标系下,这里的偏微分就需要用斜变微分来代替(用分号表示),可以写成 
${\mathsf S}_{ij}=\frac{1}{2}(u_{i;j}+u_{j;i})-\frac{1}{3}\delta_{ij}\Div\uv$.

对于两个粘滞系数项的解释会因为所使用的粘性模型而异,并依赖于给数值计算对应模块的具体参数.参见 \S\ref{Bulkviscosity}.

对于恒温流体力学,参见 \S\ref{entropy} .

% ---------------------------------------------------------------------- %

\subsection{磁感应方程 Induction equation}
...
此处 $\Av$ 是磁矢势\index{Vector potential},
$\Bv = \curl\Av$ 是磁感应强度, $\eta = 1/(\mu_0\sigma)$ 磁扩散率
($\sigma$ 为电导率), 以及 $\mu_0$ 为 
真空磁导率.
这种形式的磁感应方程能够确保 \name{Weyl gauge}
$\Phi=0$, 而 $\Phi$ 意味着标量势.
...
\subsection{熵方程 Entropy equation}
\index{Entropy}%
\label{entropy}%

目前的热力学模块  \name{entropy} 计划用 \emph{entropy} $s$ 来作为热力学物理量中的基石,而不是通过读者更熟悉的热能密度 thermal
energy $e$.于是 $\ln\rho$
和 $s$ 便成为两个基础的热力学变量. 
我们这么做的原因是因为熵(至少)于对流过程而言是更自然的物理量;熵梯度的正负号帮助确定对流的(不)稳定性; \emph{Rayleigh 数} 在流体静力学的实例中和熵的梯度成正比等(翻译存疑the \emph{Rayleigh number} is proportional to the entropy gradient
...
其中, $T$ 为温度, $c_p$ 恒温比热,
$\Heat$ 和 $\Cool$ 是显示的加热和降温项,
$K$ 为(热)辐射传导率, $\zeta$ 描述的是体粘滞系数, 以及 $\Strain$ 为迹为零的剪切速率张量(翻译存疑traceless).

熵模块用来求解比熵, $s$(翻译存疑specific entrop).
右侧的热传导项可以用下面的形式写
\begin{eqnarray}
...
\end{eqnarray}
 $\chi = K/(\rho c_p)$ 是热扩散率.
 后一等式表明, $s$ 的扩散率是 $\gamma\chi$,我们在方程 Eq.~(\ref{Dmax}) 中已经用到了它.

在对常数 $K$ 的另一种表示中, 右侧的热传导率也可以写为
\begin{eqnarray}
\frac{\Div(K\grad T)}
              {\rho T} =
...
\subsubsection{粘滞加热 Viscous heating}

我们将粘滞系数在这里写成一个张量的散度形式 $\tau_{ij,j}$,
\begin{equation}
  \rho \frac{\partial u_i}{\partial t} = ...+\tau_{ij,j} \, ,
\end{equation}
其中 $\tau_{ij}=2\nu\rho{\sf S}_{ij}$ 为应力张量. 粘滞加热功率密度为 $P$ 
\begin{eqnarray}
  P &=& u_i\tau_{ij,j}\\
...
\end{equation}
但由于 ${\sf S}_{ij}$ 的迹为零, 我们可以添加正比于 $\delta_{ij}$ 的数给它, 特别的一种加法是这样的
\begin{eqnarray}
...
\subsubsection{可选的描述?? Alternative description}
\index{pretend_lnTT}
...
\subsection{被动标量的输运方程 Transport equation for a passive scalar}
...
times.
然而,这样做的缺点是我们无法确保 $c=0$.
出于这个原因,我们最后通过设置  \code{PSCALAR=pscalar_nolog} 采用了非对数的形式.

% ---------------------------------------------------------------------- %

\subsection{体粘滞系数 Bulk viscosity}
\label{Bulkviscosity}
\index{Viscosity}

对单原子气体而言,它的体粘滞系数作用是可以忽略不计的,所以我们在大多数运算中并不会用到这个系数.然而,对于超声速气流或其他类似情况而言,研究者可能会想要加入激波粘性(在最简单的表示中,它表现为体粘滞系数)


\subsubsection{激波粘性 Shock viscosity}
\index{Shock viscosity}
\label{ShockViscosity}

激波粘性,它在本代码和 {\AA}ke Nordlund 的 Stagger Code 中都有使用.
...
在程序包中这个无量纲系数变量名为 \code{nu_shock}, 它的取值通常在 1 左右.
假设粘性系数仅作为体粘滞系数而作用,那么整个应力张量可以表示为
...
\subsection{气体状态方程 Equation of state}
\index{Equation of state}
\label{S-eos}

在目前的代码构造中,只有氢的电离程度在代码中实现了.
其他成分 (目前支持 He 和 H$_2$) 有着固定的电离程度参数.
压强正比于粒子总数,可以写成
\begin{equation}
...
在下面的章节中,电离度用 $y=n_{\rm e}/n_{\rm H}$ 来表示.在存在 H$_2$ 的情况下,它和  $x_{\rm e}$ 略有不同. 如果我们用 $n_{\rm Htot}$ 替换掉  $n_{\rm H}$,就能得到 $y=x_{\rm e}/(1-2x_{\rm H_2})$.

% ---------------------------------------------------------------------- %

\subsection{电离 Ionization}
\index{Ionization}
\label{S-ionization}

这一部分的代码可以通过在 \file{Makefile.local} 中设置 \code{EOS=eos_ionization}
(或者 \code{EOS=eos_temperature_ionization}) 来调用.
...
is now no longer valid.  作为替代措施,研究者可以用
\code{EOS=eos_fixed_ionization}, 这允许研究者手动设置电离程度.
  它的熵的归一化(翻译存疑normalization)和 \code{EOS=eos_ionization}的是一样的.  下面会将这一情况讲述地更清晰.\footnote{We omit here the contribution of H$_2$.}

这里提到的气体是由部分电离的氢和中性的氦组成的. 其中有四种不同的粒子种类,均可以被单独视为理想气体.

电离度 $y$ 表示电离的氢在氢的总数 $n_{\rm H}$ 中的占比, 可以从 Saha 方程中求得. Saha 方程可以写成以下形式
\begin{equation}
...
温度  $T$ 不可以直接从 {\sc Pencil Code} 的独立变量 $\ln\rho$ 和  $s$ 中获得,但它依赖于 $y$  . 
所以, $y$ 的计算实质上是一个求根问题.

由 $i$ 粒子构成的理想气体的熵可以通过 Sackur-Tetrode 方程获得
\begin{equation}
...
这里 $N_i$ 表示单一类型的粒子的数目,而 $n_{\rm tot}$ 表示所有粒子的总的数密度.
...
\begin{figure}[htb]
  ...
  \includegraphics%
    ...
    {pTTss}
  \caption{在不同的密度下,温度对熵的依赖关系}
  \label{pTTss}
\end{figure}
% ---------------------------------------------------------------------- %

只要给定 $\ln\rho$ 和 $s$ ,我们就可以通过找到下列方程的根计算电离百分比 $y$
...
在电离的情况下,一些气体的热力学量会受电离百分比 $y$ 的影响.比如说压强
$P\!=(1\!+y+x_{\rm He})n_{\rm H}k_{\rm B}T$, 内能
$E=\frac{3}{2}(1+y+x_{\rm He})n_{\rm H}k_{\rm B}T+y\chi_{\rm H}$, 其中
$x_{\rm He}$ 给出的是中性氦占氢的总数的百分比.
在不同的密度条件下,温度对熵的依赖关系已经在 Fig.~\ref{pTTss} 中刻画出来了.

更多的关于求解熵的信息附在了附录里的 Sect.~\ref{S-Ioni}.


% ---------------------------------------------------------------------- %

\subsubsection{双极性扩散 Ambipolar diffusion}

{\sc Pencil code}  中另外一种处理电离的方法是通过使用  {\it neutrals} 模块.
这一模块能够通过双流体模型,求解中性气体和电离气体的耦合方程 
...
\end{eqnarray}
这里的下标 $n$ 和 $i$ 分别表示中性或电离了的粒子.  ${\mathcal G}$ 和 ${\mathcal F}$ 是关于两种流体间交换质量和动量的物理量,通过以下公式给出
...
\end{eqnarray}
在上面的方程中, $\zeta$ 是电离系数, $\alpha$ 是复合系数(翻译存疑recombination coefficient) 以及 $\gamma$ 为碰撞阻力强度.
...
在本程序包中,两个方程 \eq{ionized-continuity} 和 \eq{ionized-ns} 分别在 \file{density.f90} 和 \file{hydro.f90} 中求解. 方程 Equation~\ref{neutral-continuity} 在
\file{neutraldensity.f90} 中求解, 而 \eq{neutral-ns} 则在文件 \file{neutralvelocity.f90}内. 样例 \file{1d-test/ambipolar-diffusion} 已经有了现成的给双流体(离子和中性粒子)模型的参数配置.

% ---------------------------------------------------------------------- %

\subsection{辐射传输 Radiative transfer}
\index{Radiative transfer}

这里我们仅列出基础的方程,更详细的代码实现说明在 Sect.~\ref{S-radi-trans} 和 Heinemann et al.\ (2006)的初始论文中呈现.
...
\subsection{不可压缩流体和非弹性流体方程 Incompressible and anelastic equations}
...
\subsection{尘埃方程 Dust equations}
...
\subsection{扩散近似下的宇宙线压力 Cosmic ray pressure in diffusion approximation}
\index{Cosmic rays}
...
%JS added (Is this too long?):
\subsection{手性磁流体力学 Chiral MHD}
\index{Chiral MHD}
...
\end{equation}
Also the value of $\lambda$ should not be choosen too small, since it scales 
...
粒子是那些有着空间坐标和速度矢量的实体,而一个流体只有速度矢量场(流体的连续性方程某种意义上对应着粒子的空间坐标). 本程序包中,粒子有两种存在形式,一种是示踪粒子,另外一种是尘粒.

\subsubsection{示踪粒子 Tracer particles}
...
\subsubsection{尘粒 Dust particles}
...
\end{equation}
这些粒子速度遵循着和流体类似的栋梁方程,只是没有平流项.尘粒也会受到空气的阻力(通常正比于尘粒和气体的速度差)
\begin{equation}
...
\subsection{$N$体 求解器 $N$-body solver}
...
\subsection{测试场方程 Test-field equations}
...
\subsection{引力波方程 Gravitational wave equations}
...
${\cal H}=0$.
We use the {\sc Pencil Code}; for the numerical treatment of \Eq{d2hdt}
and equations~(\ref{dlnrhodt})--(\ref{mhd3}).
For most of the simulations, we use $1152^3$ meshpoints on $1152$ cores
of a Cray XC40 system with 2.3 GHz processors.
...
\section{问题解决方案/经常提到的问题 Troubleshooting / Frequently Asked Questions}
\label{FAQ}
\index{FAQ}\index{Frequently Asked Questions}

\subsection{下载和安装 Download and setup}

\subsubsection{禁止下载 Download forbidden}
...
\subsection{Pencil 检查 Pencil check}
...
\subsection{一般问题 General questions}
...
\subsubsection{Pencil Code 讨论论坛 discussion forum}
...
\part{编写 {\sc Pencil Code} Programming the {\sc Pencil Code}}
...
\begin{figure}[htb]
  ...
  \includegraphics
    ...
    {pfile-size}
  \caption{自从 2001 年末以来,代码函数和子程序数目的变化. 2005 年的突增是由于写在另外一个分支(eos branch)上的改进合并到了主程序中.注意代码的增长和时间近似呈现线性增长关系.}
  \label{pfile-size}
\end{figure}
% ---------------------------------------------------------------------- %
...
\section{理解代码 Understanding the code}
...
\section{改进代码 Adapting the code}

\subsection{编程规范 The {\sc Pencil Code} coding standard}
\label{ProgrammingStyle}
\index{Programming style}
\index{Coding standards}
\index{Style}

就如同任何超过聊聊几行的代码一样,代码的书写风格和布局对本程序包非常重要.布局优秀的代码更加容易阅读和理解,能够减少犯错的机会.

{\sc Pencil Code} 已经发展出了统一的书写风格,并且我们也要求其他贡献者能够尽可能地保持一致,这对所有人都有好处.特别的,如果贡献者对现有的代码进行风格上的统一,那将是值得赞赏的.

我们没有太多的规则,简单地遵循已有代码的风格通常来说就已经足够了.简单来说,最重要的规则就是:
\begin{itemize}
...
所有的子程序/函数都开头加上一段标准的注释描述这个玩意是干什么的,什么时候需要调用,它的创造者是谁.任何有意义的修改都需要在后面注释上他们是什么时候被改动的,以及改动人是谁.
\item Lines longer that 78 characters should be explicitly wrapped using
  ...
  the \& character.
如果一行超过了 78 个字符,那他就要用 \& 来换行,除非不换行的时候更加有助于理解.记得在 \& 前加上空格.

\end{itemize}

这些事项在 Appendix~\ref{coding-standard} 中有着更多的叙述和例子,特别是在 Sect.~\ref{CodingStandards} 内.

\subsection{增加诊断输出 Adding new output diagnostics}
\label{S-new-output-diagnostics}

随着新物理规律的加入和新机制的发展,输出一些新的诊断变量来对其进行观察将会是非常有必要的.
在下文中.我们将会描述一些设置新的诊断变量所需要的步骤.
...
如果你不想要平均值,而是想要最大值,那就需要用 \code{max\_mn\_name()} 去替代 \code{sum\_mn\_name()}. 

Sect.~\ref{S-1d-averages} 讲述了如何输出磁场和速度场的水平平均值. 通过借鉴现有的平均值计算流程 \code{calc\_bmz()} 或 \code{calc\_jmz()},类似的平均值也可以实现.

\subsection{The f-array}
\label{f-array}
\index{f-array}
The `f' array is the largest array in the {\sc Pencil Code} and its primary role
is to store the current state of the timestepped PDE variables. 
 `f' 数组是 {\sc Pencil Code} 里面最大的数组,它主要的作用是储存当前阶段的时进(timestepped)偏微分方程变量.
f 数组和稍微比它小一点的类似的 df 数组是本数据包内仅有的三维数组.

The f-array is of type real but PDEs for a complex variable may
...
\label{df-array}
`df'  {\sc Pencil Code}里面第二大的数据就是 `df' 数组了 .
...
states of all the particles. 
和 `f' 数组类似的是, `fp' 数组保存了所有粒子的当前状态.
Like the f-array the fp-array also has a time
derivative part, the dfp-array. 就好像 f 数组一样, fp 数组也有时间微分的对应数组 dfp. The dimension of the fp-array is
...
\subsection{Pencil 样例 The pencil case}
\label{pencil-case}
...
\begin{itemize}
  \item 所有的 pencil 只计算一次
  \item Pencil 总是由合适的物理模块来计算
\end{itemize}
...
\subsubsection{添加新的 pencils, Adding new pencils}
...
\subsection{加入新物理机制: 特殊模块 Adding new physics: the Special module}
...
now {\it discourage} such practice. 
尽管程序包已经设置了很多的初始条件,但{\it 尽量不要}再让它的数量增长了. We aim to eventually removed
most of them. 我们希望最后能够将大部分初始条件移除. The recommended course of action is to make use of
the InitialCondition module.推荐的做法是利用好 InitialCondition 模块. 
...
以下的变量都定义在 \file{cdata.f90} 内,并且可以在所有的调用了 \cmd{use} \name{Cdata}模块的程序中都可以使用.
...
%%%
\subsection{子程序和函数 Subroutines and functions}
...
\subsubsection{熵边界条件 Entropy boundary condition}
...
\subsubsection{温度边界条件(翻译存疑at the top)Temperature boundary condition at the top}
...
\section{一些特定的边界条件 Some specific boundary conditions}
\label{s-boundary-conditions}

在这一章节中,我们讨论了一些常用的球坐标系或柱坐标系中的边界条件.

\subsection{理想导体边界条件 Perfect-conductor boundary condition}

这是一个常用的磁场的边界分布条件,它意味着在边界上
\begin{equation}
...
from the f-array of the snapshop, instead of the default, which is to
...
\section{数值方法 Numerical methods}

\subsection{六阶精度的空间导数 Sixth-order spatial derivatives}
...
\subsection{避免振荡的逆风导数 Upwind derivatives to avoid `wiggles'}
\index{Upwinding}
\label{S-upwind}
...
\section{启动选项和运行中参数 Startup and run-time parameters}
\label{S-all-parameters}

Chinese translation team plans to make the parameters file standalone, but with low priority. 中文翻译的团队计划将这一部分单独列做出一个文件,这一步还在计划中.
