...
本程序包典型的科学目标包括周期边界条件长方体内受迫的 MHD 湍流;平面板上下边界条件非周期性,研究对流运行;嵌在完全非周期盒中的对流星(翻译存疑);吸积盘湍流的剪切片近似.
...
尽管 主要设计在超级计算机上运行,但也有超过 的用户直接在 Macs 上运行代码,另一半用户要不直接在他们的
...
它主要的目标是告诉你程序的组织架构,让你能够有效率地阅读源码,源码中包含了更多详尽的注释.
...
记住一点: 当你想要理解一些特定的变量或者是函数时, 能给你很大的帮助.
...
% ====================================================================== %
    ...
    $\nu_3=5\times10^{-12}$. 流体年度的解释参见 Appendix~\ref{high-freq-filters}.}\label{Ftimestepoverviscous}\end{figure}
...
在熵模块中, In the entropy module we solve for the specific entropy, $s$.
右侧的热传导项可以用下面的形式写
\begin{eqnarray}
...
对单原子气体而言,它的体粘滞系数作用是可以忽略不计的,所以我们在大多数运算中并不会用到这个系数.然而,对于超声速气流或其他类似情况而言,研究者可能会想要加入
...
\subsection{状态方程 Equation of state}
...
\section{启动选项和运行时参数 Startup and run-time parameters}
\label{S-all-parameters}

中文翻译的同学计划将这一部分单独列做出一个文件,这一步还在计划中.
