...
for ivar = 0,3 do begin
  ...
  case ivar of
    ...
    3: begin
      cp = par.cp
      mpoly = par.mpoly
      var = 1./gamma1/cp * cs0^2*exp(gamma*ss/cp+gamma1*(lnrho-lnrho0))  ;; from T=cs^2/gamma1/cp 
      print,var[12,3,3],var[12,3,53]
      print,Tinit[3],Tinit[53]
...
;; overplot initial profiles
  if (n_elements(Tinit) le 0) then begin
    ...
    case ivar of
      0: oplot, lnrhoinit, z, LINE=2, COLOR=red, THICK=3
      1: ;nothing to overplot
      2: oplot, ssinit, z, LINE=2, COLOR=red, THICK=3
      3: oplot, Tinit, z, LINE=2, COLOR=red, THICK=3
    endcase
  endelse
