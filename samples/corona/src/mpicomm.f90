...
!
      if (nprocy>1) then
  ! Debug code for the g95 daily auto-test of samples/2d-tests/selfgravdisk-planet-dust:
  write (*,*) 'size(lbufyo,1), size(lbufyo,2), size(lbufyo,3), size(lbufyo,4)'
  write (*,*) size(lbufyo,1), size(lbufyo,2), size(lbufyo,3), size(lbufyo,4)
  write (*,*) 'size(f,1), size(f,2), size(f,3), size(f,4)'
  write (*,*) size(f,1), size(f,2), size(f,3), size(f,4)
  write (*,*) 'm1, m1i, n1, n2, ivar1, ivar2'
  write (*,*) m1, m1i, n1, n2, ivar1, ivar2
  write (*,*) 'mcom, maux_com, mvar'
  write (*,*) mcom, maux_com, mvar
