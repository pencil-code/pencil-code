program magic_emul
  ...
  implicit none
  include 'mpif.h'
  integer:: mpierr,ncpus,i,iproc,iproc_save,MPI_COMM_EULAG, &
            nprocs,tag,nprocx_penc,nprocy_penc,nprocx,nprocy,nprocz,ip
...
!print*, 'EULAG: iapp,flag=', iapp, flag
! New comm MPI_COMM_EULAG which comprises only the procs of the Pencil
! application. iproc becomes rank in MPI_COMM_EULAG.
! Attention: If there is more than one application envisaged, Pencil needs to be
! compiled
! with FPPFLAGS=-fpp -DMPI_COMM_WORLD=MPI_COMM_EULAG.
! If there is only one application, iproc is unchanged and
! MPI_COMM_EULAG=MPI_COMM_WORLD.
!
      iproc_save=iproc
      call MPI_COMM_SPLIT(MPI_COMM_WORLD, iapp, iproc, MPI_COMM_EULAG, mpierr)
      call MPI_COMM_RANK(MPI_COMM_EULAG, iproc, mpierr)
      call MPI_COMM_SIZE(MPI_COMM_EULAG, ncpus, mpierr)
      nprocy=sqrt(real(ncpus)) ! for EULAG only
      nprocz=nprocy
...
!
          call MPI_SEND('EULAG',5,MPI_CHARACTER,0,tag_foreign, MPI_COMM_WORLD, mpierr)
!
!  Send processor numbers of foreign code.
!
          call MPI_SEND((/1,nprocy,nprocz/),3,MPI_INTEGER,0,tag_foreign, MPI_COMM_WORLD, mpierr)
!
...
!
!  Send domain extents of foreign code.
!
...
!
          ...
          call MPI_RECV(nprocy_penc,1,MPI_INTEGER,ncpus,tag_foreign,MPI_COMM_WORLD,stat,mpierr)
!
        endif
print*, 'EULAG: successful'
...
!print*, 'EULAG: iproc_save, iproc=', iproc_save, iproc
print*, 'EULAG: successful'
      call MPI_BARRIER(MPI_COMM_WORLD, mpierr)
      call MPI_FINALIZE(mpierr)
stop
