...
for ((i=1; i<=EPOCHS; i++)); do
    ...
    echo "=== Finished Epoch $i ==="
    	...
    	cp train_sample_0.csv \
      	 "${data_src}/training/$ml_model/train_sample_0${i}.csv"
		fi
		...
		srun -n 1 python3 -c "from build_files import avg_stats; avg_stats('all_stats')"
		
		cp all_stats/stats_current_output.pt \
			"${data_src}/training/stats_current_output.pt"

		cp all_stats/stats_current_input.pt \
