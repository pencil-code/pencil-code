# fiel_integration_e3.py
#
# Integraates queentities along field lines for e3.
...
p = pc.pClass()
p.Lx = 8.; p.Ly = 8.; p.Lz = 48.
p.Ox = -4.; p.Oy = -4.; p.Oz = -24.
...
# Field parameters.
B0 = 1
e3_xc = np.array([1, -1, 1, -1, 1, -1], dtype = 'float32')
e3_yc = np.array([0, 0, 0, 0, 0, 0], dtype = 'float32')
e3_zc = np.array([-20, -12, -4, 4, 12, 20], dtype = 'float32')
e3_k = np.array([1, -1, 1, -1, 1, -1], dtype = 'float32')
e3_a = np.sqrt(np.array([2, 2, 2, 2, 2, 2], dtype = 'float32'))
e3_l = np.array([2, 2, 2, 2, 2, 2], dtype = 'float32')
...
for i in range(len(e3_xc)):
    bb[0,...] += 2*B0*e3_k[i]*np.exp((-(xx-e3_xc[i])**2-(yy-e3_yc[i])**2)/e3_a[i]**2-(zz-e3_zc[i])**2/e3_l[i]**2) \
                 *(-(yy-e3_yc[i]))
    bb[1,...] += 2*B0*e3_k[i]*np.exp((-(xx-e3_xc[i])**2-(yy-e3_yc[i])**2)/e3_a[i]**2-(zz-e3_zc[i])**2/e3_l[i]**2) \
                 *(xx-e3_xc[i])
...
np.save('e3_jp.dat', jp)
np.save('e3_xi.dat', xi)
np.save('e3_yi.dat', yi)
np.save('e3_xi.dat', xf)
np.save('e3_yf.dat', yf)
np.save('e3_m.dat', m)
