# fiel_integration_e1.py
#
# Integraates queentities along field lines for e1.
...
p = pc.pClass()
p.Lx = 8.; p.Ly = 8.; p.Lz = 16.
p.Ox = -4.; p.Oy = -4.; p.Oz = -8.
...
# Field parameters.
B0 = 1
e1_xc = np.array([1, -1], dtype = 'float32')
e1_yc = np.array([0, 0], dtype = 'float32')
e1_zc = np.array([-4, 4], dtype = 'float32')
e1_k = np.array([1, -1], dtype = 'float32')
e1_a = np.sqrt(np.array([2, 2], dtype = 'float32'))
e1_l = np.array([2, 2], dtype = 'float32')
...
for i in range(len(e1_xc)):
    bb[0,...] += 2*B0*e1_k[i]*np.exp((-(xx-e1_xc[i])**2-(yy-e1_yc[i])**2)/e1_a[i]**2-(zz-e1_zc[i])**2/e1_l[i]**2) \
                 *(-(yy-e1_yc[i]))
    bb[1,...] += 2*B0*e1_k[i]*np.exp((-(xx-e1_xc[i])**2-(yy-e1_yc[i])**2)/e1_a[i]**2-(zz-e1_zc[i])**2/e1_l[i]**2) \
                 *(xx-e1_xc[i])
...
np.save('e1_jp.dat', jp)
np.save('e1_xi.dat', xi)
np.save('e1_yi.dat', yi)
np.save('e1_xi.dat', xf)
np.save('e1_yf.dat', yf)
np.save('e1_m.dat', m)
