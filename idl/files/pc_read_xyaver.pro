; $Id: pc_read_xyaver.pro,v 1.1 2005-07-18 16:25:37 ajohan Exp $
;
...
cmd='var=fltarr(nz)*one'
if (execute(cmd,0) ne 1) then message, 'Error defining data read array'
;
;  Prepare for read
;
GET_LUN, file
...
for it=0,nit-1 do begin
  for i=0,nvar-1 do begin
    readf, file, t
    cmd=varnames[i]+'[*,it]='+strtrim(t,2)
    if (execute(cmd,0) ne 1) then message, 'Error putting time in array'
...
makeobject="object = CREATE_STRUCT(name=objectname,[" + $
    arraytostring(varnames,QUOTE="'",/noleader) + "]," + $
    arraytostring(varnames,/noleader) + ")"
