pro rvid_line,field,mpeg=mpeg,png=png,tmin=tmin,tmax=tmax,max=amax,min=amin,$
  ...
  test=test,proc=proc,exp=exp,map=map,tt=tt,noplot=noplot
;
; $Id: rvid_line.pro,v 1.2 2004-03-15 12:49:31 brandenb Exp $
;
  ...
  if t ge tmin and t le tmax then begin
    if ijump eq njump then begin
      if not keyword_set(noplot) then begin
        if keyword_set(exp) then plot,exp(axz),yr=[amin,amax] else plot,axz,yr=[amin,amax]
      endif
