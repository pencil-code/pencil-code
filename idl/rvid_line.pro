pro rvid_line,field,mpeg=mpeg,png=png,tmin=tmin,tmax=tmax,max=amax,min=amin,$
  ...
  transp=transp,global_scaling=global_scaling,points=points,nsmooth=nsmooth, $
  log=log,_extra=_extra
;
; $Id: rvid_line.pro,v 1.15 2006-08-27 11:13:09 mee Exp $
;
    ...
    if ijump eq njump then begin
      if not keyword_set(noplot) then begin
        if keyword_set(points) then begin
          if keyword_set(exponential) then plot,zaxisscale,exp(axz),ps=-2,yr=[amin,amax] else plot,zaxisscale,axz,ps=-2,yr=[amin,amax],_extra=_extra
        endif else begin
          if keyword_set(exponential) then plot,zaxisscale,exp(axz),yr=[amin,amax] else plot,zaxisscale,axz,yr=[amin,amax],_extra=_extra
;plot,zaxisscale,axz
        endelse
      endif
