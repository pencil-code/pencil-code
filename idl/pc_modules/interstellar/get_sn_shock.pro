...
;
; $Id: get_sn_shock.pro,v 1.3 2006-06-30 08:47:36 mee Exp $
;
...
;
  if t ge tmin and t le tmax then begin
    if ijump eq njump then begin
      find_SN_shock,axz[*,32], zaxisscale,leftpnt=shock_left, rightpnt=shock_right, $
                             tol=tolerance
      shockinfo[it,0]=t
      ...
      if not keyword_set(noplot) then begin
        if keyword_set(exp) then begin
          plot,zaxisscale,exp(axz[*,32]),yr=[amin,amax] 
          print,islice,t,exp(min([axz])),exp(max([axz])),shock_left,shock_right
        endif else begin
          plot,zaxisscale,axz[*,32],yr=[amin,amax]
