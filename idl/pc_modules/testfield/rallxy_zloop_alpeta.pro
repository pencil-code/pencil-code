;
;  Similar to rallxy_zloop.pro, this routine loops through all
;  horizontal processor layers to calculate alpij and etaij.
;
;  15-jun-08/axel: adapted from rallxy_zloop.pro
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;   rallxy_zloop.pro   ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;  define
;
uxbtestm=reform(uxbtestm,nzgrid,3,2,2)
alpij=fltarr(nzgrid,2,2)
etaij=fltarr(nzgrid,2,2)
;
...
;
  alpij(*,0,0)=cz*uxbtestm(*,0,0,0)+sz*uxbtestm(*,0,1,0)
  alpij(*,0,1)=cz*uxbtestm(*,0,0,1)+sz*uxbtestm(*,0,1,1)
  alpij(*,1,0)=cz*uxbtestm(*,1,0,0)+sz*uxbtestm(*,1,1,0)
  alpij(*,1,1)=cz*uxbtestm(*,1,0,1)+sz*uxbtestm(*,1,1,1)
;
  etaij(*,0,0)=-(k1sz*uxbtestm(*,0,0,1)-k1cz*uxbtestm(*,0,1,1))
  etaij(*,0,1)=+(k1sz*uxbtestm(*,0,0,0)-k1cz*uxbtestm(*,0,1,0))
  etaij(*,1,0)=-(k1sz*uxbtestm(*,1,0,1)-k1cz*uxbtestm(*,1,1,1))
  etaij(*,1,1)=+(k1sz*uxbtestm(*,1,0,0)-k1cz*uxbtestm(*,1,1,0))
;
;  save result
;
save,file=varfile+'.alpeta',t,zzz,bm,alpij,etaij
;
end
