pro rvid_box,field,mpeg=mpeg,png=png,tmin=tmin,tmax=tmax,max=amax,min=amin,$
  ...
  qswap=qswap,bar=bar,nolabel=nolabel
;
; $Id: rvid_box.pro,v 1.31 2005-04-08 02:13:20 brandenb Exp $
;
  ...
  if t ge tmin and t le tmax then begin
    if ijump eq njump then begin
      if not keyword_set(shell) then begin
        ...
        if keyword_set(nolabel) then begin
          if (label ne '') then begin
            xyouts,xlabel,ylabel,label,col=1,siz=size_label
          endif
        endif else begin
          if (label eq '') then begin
            xyouts,xlabel,ylabel,'!8t!6='+string(t/tunit,fo=fo)+'!c'+title, $
      ...
      if keyword_set(bar) then begin
        default,barcharsize,1.5
        default,barformat,'(f5.2)'
        colorbar,/v,pos=[.82,.15,.84,.85],col=1,div=2,$
           range=[amin,amax],/right,format=barformat,charsize=barcharsize
      endif
      ;
      if keyword_set(png) then begin
