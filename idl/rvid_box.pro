pro rvid_box,field,mpeg=mpeg,png=png,tmin=tmin,tmax=tmax,wait=wait,amax=amax,amin=amin,$
  nrepeat=nrepeat,njump=njump,datadir=datadir,OLDFILE=OLDFILE
;
; $Id: rvid_box.pro,v 1.6 2003-01-05 21:50:02 brandenb Exp $
;
...
if keyword_set(png) then begin
  ...
  ;device, SET_RESOLUTION=[400,300] ; set window size
...
if keyword_set(OLDFILE) then begin ; For files without position
  ...
  if t ge tmin and t le tmax then begin
    if ijump eq njump then begin
      ...
      if keyword_set(png) then begin
        istr2 = strtrim(string(itpng,'(I20.4)'),2) ; 4 digits: only up to 9999 frames ;-)
        image = tvrd()
