pro rvid_box,field,mpeg=mpeg,png=png,TRUEPNG=png_truecolor,tmin=tmin,$
  ...
  newwindow=newwindow, quiet_skip=quiet_skip
;
; $Id: rvid_box.pro,v 1.52 2007-08-03 16:28:50 ajohan Exp $
;
...
;
;  Perform preset mathematical operation on data (if requested).
;
...
;
  if (keyword_set(OLDFILE)) then begin ; For files without position
    ...
    if ( (t ge tmin) and (t le tmax) and $
         (islice mod (njump+1) eq 0) ) then begin
      readu, 1, xy2, t, slice_z2pos
      readu, 2, xy, t, slice_zpos
      readu, 3, xz, t, slice_ypos
      readu, 4, yz, t, slice_xpos
...
;
      if (keyword_set(bar)) then begin
        ...
        colorbar, pos=[.82,.15,.84,.85], color=1, div=divbar,$
            range=[amin,amax], /right, /vertical, $
            format=bformat, charsize=bsize, title=field
        !p.title=''
      endif
;
;  Save as png file.
