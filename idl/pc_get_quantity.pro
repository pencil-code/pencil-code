...
function pc_get_quantity, vars, index, quantity, unit=unit, dim=dim, grid=grid, param=param, run_param=run_param, datadir=datadir
	...
	if (strcmp (quantity, 'log_Temp', /fold_case)) then begin
		if (any (strcmp (sources, 'lnTT', /fold_case))) then begin
			...
			return, alog10 (vars[l1:l2,m1:m2,n1:n2,index.TT] * unit.temperature)
		end
		message, "Can't compute '"+quantity+"' without 'lnTT' or 'TT'"
	end
	if (strcmp (quantity, 'ln_Temp', /fold_case)) then begin
		if (any (strcmp (sources, 'lnTT', /fold_case))) then begin
			...
			return, alog (vars[l1:l2,m1:m2,n1:n2,index.TT] * unit.temperature)
		end
		message, "Can't compute '"+quantity+"' without 'lnTT' or 'TT'"
	end
	...
	if (strcmp (quantity, 'rho', /fold_case)) then begin
		...
		message, "Can't compute '"+quantity+"' without 'lnrho' or 'rho'"
	end else if (strcmp (quantity, 'ln_rho', /fold_case)) then begin
		if (any (strcmp (sources, 'lnrho', /fold_case))) then begin
